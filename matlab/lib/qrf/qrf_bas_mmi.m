function [UN,QN,p2opt]=qrf_bas_mmi(f,M,MR,MM,MM1,ZZ,ZM,BB,K,F,N,mu,v,rt)
...
if resNlp > 1e-6
    line_error(mfilename, ...
        ['fmincon did not reach a feasible point (max constraint residual ' ...
         '%.3e). The qrf_bas_mmi bound is not defined for this instance.'], resNlp);
end
...
%         end, end
        ...
        % THE STALE AMPL-LITERAL TWIN OF THIS FAMILY WAS DELETED (2026-09-03).
        % The five THM30-group families are implemented ABOVE, phase-aggregated
        % and with the population-to-index shift applied once. This file also
        % carried a second, literal transcription of each, emitting every one of
        % them TWICE with different index conventions -- so the system was
        % over-constrained and qrf_bas_mmi died in qrf_noblo_start with a maximum
        % equality residual of 2.469e-01 on cqn_bas_blocking, which is what made
        % 'qrf.bas.mmi' unusable. qrf_bas_mem.m had commented the duplicates out;
        % deleting them here puts all three BAS objectives on ONE polytope, the
        % same family set the JAR, python and C++ ports assemble.
