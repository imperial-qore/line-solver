...
lp = struct('A',A,'b',b,'Aeq',Aeq,'beq',beq,'lb',x0*0,'ub',x0*0+1);
if nargin >= 5 && ~isempty(lponly) && lponly
    UN = []; QN = []; p2opt = [];
    return
end
    ...
    function [A,b,Aeq,beq] = sub_qrfcon(q,M,MR,MM,BB,F,N)
        ...
        for i = 1:M, for k =1:K(i)
                ...
                for ni = 1+(1:F(i)), for m = 1:MR
                    ...
                    for j = 1:M, for h = 1:K(i)
                       Aeq(end,:) = Aeq(end,:) + q(i,j,k,h,ni)*deltap2(i,ni,k,i,ni,k,m);
                    end, end
                    % RHS
                    for j = 1:M, for h = 1:K(i)
                       Aeq(end,:) = Aeq(end,:) - q(i,j,h,k,ni)*deltap2(i,ni,h,i,ni,h,m);
                    end, end
                end, end
        end, end
        ...
        for i = 1:M, for u = 1:K(i)
                if i~=f
                    ...
                    for j = 1:M, for nj = 1+(1:F(j)), for k = 1:K(j), for h = 1:K(j), for m = 1:MR 
                        if j~=i && j~=f && BB(m,j)==0 
                            Aeq(end,:)= Aeq(end,:) + q(j,i,k,h,nj)*deltap2(j,nj,k, i,1+0,u, m);
                        end
                    end, end, end, end, end     
                    ...
                    for j = 1:M, for nj = 1+(0:F(j)), for k = 1:K(i), for h = 1:K(j), for m = 1:MR 
                        if j~=i && j~=f && BB(m,i)==0 
                            Aeq(end,:)= Aeq(end,:) - q(i,j,k,u,1+1)*deltap2(j,nj,h, i,1+1,k, m); 
                        end
                    end, end, end, end, end
                    ...
                    for j = 1:M, for nj = 1+(0:F(j)), for y = 1:K(j), for m = 1:MR 
                        if j~=i && j==f && BB(m,i)==1 && (nj-1)==F(j) && MM(m,1)==i  % rescaled back nj 
                            for p = 1:K(f), for w = 1:M
                                if w~=f && w~=i 
                                    Aeq(end,:)= Aeq(end,:) - q(f,w,y,p,nj)*deltap2(f,nj,y, i,1+1,u, m);
                                end
                            end, end
                        end
                    end, end, end, end
        ...
        for i = 1:M, for ni = 1+(0:(F(i)-1))
                ...
                for j = 1:M, for nj = 1+(1:F(j)), for k = 1:K(j), for h = 1:K(j), for u = 1:K(i), for m = 1:MR 
                    if j~=i && j~=f && BB(m,j)==0 
                        Aeq(end,:)= Aeq(end,:) + q(j,i,k,h,nj)*deltap2(j,nj,k, i,ni,u, m); 
                    end                   
                end, end, end, end, end, end
                for j = 1:M, for nj = 1+(1:F(j)), for k = 1:K(j), for h = 1:K(j), for u = 1:K(i), for m = 1:MR 
                    if j~=i && j==f && MM(m,1)~=i 
                ...
                for j = 1:M, for nj = 1+(0:F(j)), for k = 1:K(i), for u = 1:K(j), for m = 1:MR 
                    if j~=i && j~=f && BB(m,i)==0 
                        for h = 1:K(i) 
                            Aeq(end,:)= Aeq(end,:) - q(i,j,k,h,ni+1)*deltap2(j,nj,u, i,ni+1,k, m); 
                        end
                    end
                end, end, end, end, end
                for j = 1:M, for nj = 1+(0:F(j)), for k = 1:K(i), for u = 1:K(j), for m = 1:MR 
                ...
                for j = 1:M, for nj = 1+(0:F(j)), for k = 1:K(i), for u = 1:K(j), for m = 1:MR 
                    if j~=i && j==f && BB(m,i)==1 && (nj-1)==F(j) && MM(m,1)==i  % rescaled back nj 
                        for p = 1:K(f), for w = 1:M 
                            if w~=f && w~=i 
                                Aeq(end,:)= Aeq(end,:) - q(f,w,u,p,nj)*deltap2(f,nj,u, i,ni+1,k, m);
                            end
                        end, end
                    end
                end, end, end, end, end
                ...
                if i==f
                    ...
                    for j = 1:M, for nj = 1+(0:F(j)), for k = 1:K(i), for h = 1:K(i), for u = 1:K(j)
                        if j~=i && (ni-1) < F(i)  % rescaled back ni
                            Aeq(end,:) = Aeq(end,:) - q(i,j,k,h,ni+1)*deltap2(j,nj,u, i,ni+1,k, 1);
                        end
                    end, end, end, end, end
                end % if
        end, end
