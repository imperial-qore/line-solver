...
%   exitflag - Solver exit flag
    ...
    % pb(i): blocking-probability variables, AMPL `var pb{i in 1..M} >=0, <=1`
    pbidx = zeros(M, 1);
    for i = 1:M
        varCount = varCount + 1;
        pbidx(i) = varCount;
    ...
    nVars = varCount;
    if verbose; fprintf('Total variables: %d (p2: %d, U/Ueff/pb: %d)\n', nVars, nP2Vars, nVars - nP2Vars); end
    ...
    if verbose; fprintf('  UCLASSIC constraints...\n'); end
    for i = 1:M
        for ki = 1:K(i)
            for ni = 1:F(i)
                row = zeros(1, nVars);
    ...
    if verbose; fprintf('  UEFFS constraints...\n'); end
    for i = 1:M
        for ki = 1:K(i)
            for ni = 1:F(i)
                row = zeros(1, nVars);
    ...
    if verbose; fprintf('  PBLOCK constraints...\n'); end
    for i = 1:M
        row = zeros(1, nVars);
        row(pbidx(i)) = 1;
        for ki = 1:K(i)
            for ni = 1:F(i)
                row(getUIdx(i, ki, ni))    = row(getUIdx(i, ki, ni))    - 1;
                row(getUeffIdx(i, ki, ni)) = row(getUeffIdx(i, ki, ni)) + 1;
            end
        end
        Aeq = [Aeq; row];
        beq = [beq; 0];
    ...
    if verbose; fprintf('  PBB constraints...\n'); end
    for i = 1:M
        row = zeros(1, nVars);
        row(pbidx(i)) = 1;
        for j = 1:M
    ...
    if verbose; fprintf('  THM1 (Population) constraints...\n'); end
    % THM1 is AGGREGATED over nj >= 1: one row per (j,kj), matching the
    % authoritative AMPL model (qrboundsrsrd_skel.mod / example_rsrd.mod):
    %   THM1 {j,k}: sum_i sum_{nj>=1} sum_{ni>=1} sum_h ni*p2[j,nj,k,i,ni,h]
    %               = N * sum_{nj>=1} p2[j,nj,k,j,nj,k]
    % Emitting this per-nj instead (one row for each nj) is STRICTLY STRONGER --
    % the per-nj equalities imply the aggregate but not conversely -- and it
    % over-tightens the polytope: on the paper's M=5,N=20 RS-RD example the
    % per-nj form gives U1min=0.92508 against the published 0.87058.
    for j = 1:M
        for kj = 1:K(j)
            row = zeros(1, nVars);
            for nj = 1:F(j)
                idx_diag = getIdx(j, nj, kj, j, nj, kj);
                row(idx_diag) = row(idx_diag) - N;
                for i = 1:M
                    for ni = 1:F(i)  % ni >= 1
                        for hi = 1:K(i)
                            idx = getIdx(j, nj, kj, i, ni, hi);
    ...
    %% THM1c: population balance for the empty-queue case nj = 0 (i ~= j)
    %   THM1c {j,k}: sum_{i<>j} sum_{ni>=1} sum_h ni*p2[j,0,k,i,ni,h]
    %                = N * p2[j,0,k,j,0,k]
    if verbose; fprintf('  THM1c (nj=0 population) constraints...\n'); end
    for j = 1:M
        for kj = 1:K(j)
            row = zeros(1, nVars);
            idx_diag = getIdx(j, 0, kj, j, 0, kj);
            row(idx_diag) = row(idx_diag) - N;
            for i = 1:M
                if i == j
                    continue;
    ...
    if verbose; fprintf('  THM3a (Marginal balance) constraints...\n'); end
    for i = 1:M
        for ni = 1:(F(i)-1)
            row = zeros(1, nVars);
    ...
    if verbose; fprintf('  THM3b (Marginal balance ni=0) constraints...\n'); end
    for i = 1:M
        for ui = 1:K(i)
            row = zeros(1, nVars);
