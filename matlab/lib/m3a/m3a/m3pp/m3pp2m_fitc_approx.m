function [FIT] = m3pp2m_fitc_approx(a, bt1, bt2, binf, m3t2, ...
    t1, t2, ...
    ai, dvt3, t3)
...
%m3a_printf('Fitting per-class counting process...\n');
options = optimset('Algorithm','interior-point-convex ',...
    'Display','none');
%[x,fx] = quadprog(H, f, A, b, Aeq, beq, [], [], [], options);
% The free variables are variance DIFFERENCES, which are routinely negative
% (a minority class has less variance than all the others combined) and whose
% sum is fixed by the equality rows, so the former lb = 1e-6 made the feasible
% set empty on ordinary inputs. Left unbounded, as the sibling
% m3pp2m_fitc_approx_ag_multiclass does with its quadprog call.
lb = -inf(size(A,2),1);
ub =  inf(size(A,2),1);
[x,fx]=QP(H, f, A, b, Aeq, beq, lb, ub, options);
%fit_error = fx + m;
%m3a_printf('Per-class fitting error: %f\n', fit_error);
...
%
% for i = 1:m
%     m3a_printf('Rate class %d: input = %.3f, output = %.3f\n', ...
%             i, ai(i), Ai(i));
% end
% for i = 1:m
%     m3a_printf('DV%d(t3): input = %.3f, output = %.3f\n', ...
%             i, dvt3(i), Dvt3(i));
% end
