function [fit] = m3pp2m_fitc_trace(T, A, method, t1, tinf)
...
m3a_printf('Computing counting process at resolution %f\n', t1);
...
if strcmp(method,'exact_delta') == 1
    fit = m3pp2m_fitc(a, bt1, bt2, binf, m3t2, t1, t2, ai, dvt3, t3);
elseif strcmp(method,'approx_delta') == 1
    fit = m3pp2m_fitc_approx(a, bt1, bt2, binf, m3t2, t1, t2, ai, dvt3, t3);
elseif strcmp(method,'approx_cov') == 1
    V = var(sum(mNt3,2));
    vi = [var(mNt3(:,1)), var(mNt3(:,2))];
    s = 0.5 * (V - sum(vi));
    fit = m3pp22_fitc_approx_cov(a, bt1, bt2, binf, m3t2, t1, t2, ai, s, t3);
elseif strcmp(method,'approx_ag') == 1
    ...
    gt3 = vt3 + st3;
    fit = m3pp2m_fitc_approx_ag(a, bt1, bt2, binf, m3t2, t1, t2, ai, gt3, t3);
else
