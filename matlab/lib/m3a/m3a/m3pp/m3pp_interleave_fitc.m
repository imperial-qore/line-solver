function [fit,m3pps] = m3pp_interleave_fitc(av, btv, binfv, ...
                                                 acc, gtcc, t, tinf, ...
                                                 mapping, reorder)
...
for o = 1:k
    ...
    gtcj = gtcc{j};
    % fit m3pp(2,mj)
    m3pps{o} = m3pp2m_fitc_approx_ag_multiclass(mmpp, acj, gtcj, t);
end
...
m3a_printf('Rate: input = %f, %f\n', a, fa);
for i = 1:m
    m3a_printf('Class %d, rate: input = %f, output = %f\n', i, ac(i), fac(i));
end
fbtv = zeros(k,1);
for j = 1:k
    m3pp2 = cell(1,4);
...
for j = 1:k
    m3a_printf('Partition %d, IDC(%.2f): input = %f, output = %f\n', j, t, btv(j), fbtv(j));
end
fbinfv = zeros(k,1);
...
for j = 1:k
    m3a_printf('Partition %d, IDC(inf = %.2f): input = %f, output = %f, output_limit = %f\n', j, tinf, binfv(j), fbinfv(j), fbinfv_limit(j));
end
...
for j = 1:k
    ...
    for i = 1:m
        if mapping(i,j) == 1
            ...
            m3a_printf('gamma(%d): ', i);
            m3a_printf('input = %f, output = %f\n', gtcc{j}(n), fvtc(i) + covji(1,2));
            n = n + 1;
        end
    end
end
