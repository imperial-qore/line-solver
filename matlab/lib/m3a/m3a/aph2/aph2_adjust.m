...
elseif strcmp(method, 'opt_param') || strcmp(method, 'opt_param_gads')
    ...
    function [p1,l1,l2,tmp0] = aph2_fit1(xM2,xM3)
        tmp0 = (8*M1^3*xM3)/3 - 3*M1^2*xM2^2 - 2*M1*xM2*xM3 + 2*xM2^3 + xM3^2/9;
        % Clamped: tmp0 < 0 means this inversion has no real solution, which
        % is exactly what constraint c(1) = -tmp0 reports. Taking the raw
        % sqrt there made l1, l2 and p1 COMPLEX, so the whole constraint
        % vector was complex and fmincon stopped at its (infeasible) start
        % point, returning the input moments unchanged.
        tmp1 = 3*sqrt(max(tmp0,0));
        tmp2 = xM3 - 3*M1*xM2;
        tmp3 = (6*xM2 - 12*M1^2);
        l1 = (tmp2 + tmp1)/tmp3;
        l2 = (tmp2 - tmp1)/tmp3;
    ...
    function [p1,l1,l2,tmp0] = aph2_fit2(xM2,xM3)
        tmp0 = (8*M1^3*xM3)/3 - 3*M1^2*xM2^2 - 2*M1*xM2*xM3 + 2*xM2^3 + xM3^2/9;
        % Clamped: tmp0 < 0 means this inversion has no real solution, which
        % is exactly what constraint c(1) = -tmp0 reports. Taking the raw
        % sqrt there made l1, l2 and p1 COMPLEX, so the whole constraint
    ...
    function [c, ceq] = nonlcon1(x)
        xM2 = x(1);
        xM3 = x(2);
        % tmp0 is returned explicitly: it is assigned inside aph2_fit1, a
        % SIBLING nested function, so it is not in scope here otherwise and
        % every call used to raise an undefined-variable error.
        [p1,l1,l2,tmp0] = aph2_fit1(xM2,xM3);
        c = zeros(7,1);
        c(1) = -tmp0; % non-negative square root argument
        c(2) = -l1 + 1e-6; % non-negative l1
        c(3) = -l2 + 1e-6; % non-negative l2
        c(4) = -p1; % non-negative p1
        % CLOSED-FORM APH(2) FEASIBILITY, redundant with the inversion rows
        % above but not implied by them at solver tolerance: the inversion
        % constraints alone are satisfied to ~1e-9 by driving p1 to a tiny
        % negative value with l2 growing without bound, which fmincon's
        % default ConstraintTolerance = 1e-6 accepts although the pair admits
        % no APH(2). scv >= 1/2, plus the Telek-Heindl third-moment interval.
        scv = (xM2 - M1^2)/M1^2;
        c(5) = 1/2 - scv;
        if scv <= 1
            c(6) = 3*M1^3*(3*scv-1+sqrt(2)*(1-scv)^(3/2)) - xM3;
            c(7) = xM3 - 6*M1^3*scv;
        else
            c(6) = 3/2*M1^3*(1+scv)^2 - xM3;
            c(7) = 0;
        end
        ceq = [];
    end
    ...
    function [c, ceq] = nonlcon2(x)
        xM2 = x(1);
        xM3 = x(2);
        [p1,l1,l2,tmp0] = aph2_fit2(xM2,xM3);
        c = zeros(7,1);
        c(1) = -tmp0; % non-negative square root argument
        c(2) = -l1 + 1e-6; % non-negative l1
        c(3) = -l2 + 1e-6; % non-negative l2
