...
if abs(1-r1) < degentol
    ...
    % DEGENERATE
    m3a_printf('Fitting MAPH(2,m): detected degenerate form\n');
    ...
    for c = 1:k
        m3a_printf('Fitting MAPH(2,m): B(%d) = %f -> %f\n', c, B(c), fB(c));
    end
...
% the degenerate branch above matches the class probabilities only and never
% runs the solver, so fB is still unset there: report the backward moments the
% fitted MAPH actually realizes, which is what the caller ranks the forms by
if ~exist('fB','var')
    fB = mmap_backward_moment(maph, 1);
end
    ...
    function x = solve()
        m3a_printf('Fitting MAPH(2,m): running quadratic programming solver...\n');
        ...
        m3a_printf('Fitting MAPH(2,m): error = %f\n', fit_error);
    end
