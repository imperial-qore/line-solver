...
m3a_printf('Fitting MAMAP(2,2) B+S: form = %d\n', form);
...
if (form == 1 && (r1 < degentol || r2 > 1-degentol || abs(h2-h1*r2) < degentol )) || ...
    ...
    m3a_printf('Fitting MAMAP(2,2) B+S: perturbing degenerate Poisson to second-order\n');
    if r1 < degentol
        r1 = degentol;
    end
    if r2 > 1-degentol
...
elseif form == 2 && r2 < degentol && abs(1-r1) < degentol
    ...
    m3a_printf('Fitting MAMAP(2,2) B+S: detected degenerate phase-type form\n');
...
elseif form == 1 && r2 < degentol
    ...
    m3a_printf('Fitting MAMAP(2,2) B+S: detected canonical phase-type form\n');
...
elseif (form == 1 && abs(1-r1) < degentol) || ...
    ...
    m3a_printf('Fitting MAMAP(2,2) B+S: detected non-canonical phase-type form, converting to canonical form\n');
...
elseif form == 2 && r2 < degentol
    ...
    m3a_printf('Fitting MAMAP(2,2) B+S: detected degenerate MMAP form\n');
    
    if bsWeights(1) > bsWeights(2)
        m3a_printf('Fitting MAMAP(2,2) B+S: fitting B\n');
        ...
        m3a_printf('Fitting MAMAP(2,2) B+S: fitting S\n');
    ...
    if (form == 1 && abs(h1 - h2 + h2*r1) < degentol) || (form == 2 && abs(h1 - h2 + h2*r1) < degentol)
        m3a_printf('Fitting MAMAP(2,2) B+S: detected fittable Poisson process\n')
    end
    ...
    if isfeasible(q1) && isfeasible(q2) && isfeasible(q3)
        m3a_printf('Fitting MAMAP(2,2) B+S: exact fit found\n');
        exact = 1;
    elseif adjust
...
fS = mmap_sigma(mmap);
    ...
    function [q1,q2,q3,obj] = solve_nonlinear()
        ...
        m3a_printf('Fitting MAMAP(2,2) B+S: running approximate fitting (nonlinear)\n');
        ...
        m3a_printf('Fitting MAMAP(2,2) B+S: B1 == M1, objective = %e\n', eobj);
        % left solution
        [lfeas,lobj,lF1,lS11] = solve_nonlinear_func('left');
        if lfeas
            m3a_printf('Fitting MAMAP(2,2) B+S: B1 < M1, objective = %e\n', lobj);
        else
            m3a_printf('Fitting MAMAP(2,2) B+S: B1 < M1 infeasible\n');
        end
        % right solution
...
% used for non-degenerate cases: hybrid space
    function [feas,bobj,bB1,bS11] = solve_nonlinear_hybrid(side)
        ...
        if hsol.problem == 0
            m3a_printf('Fitting MAMAP(2,2) B+S: solver (hybrid space) objective = %f\n', double(hobj));
            feas = 1;
            bobj = double(hobj);
            bB1 = double(vB1);
            bS11 = double(vS11);
        elseif hsol.problem == 1 || hsol.problem == 12
            m3a_printf('Fitting MAMAP(2,2) B+S: program is infeasible\n');
            feas = 0;
            bobj = nan;
            bB1 = nan;
            bS11 = nan;
    ...
    function x = solve_quadprog()
        m3a_printf('Fitting MAMAP(2,2) B+S: running quadratic programming solver...\n');
        ...
        m3a_printf('Fitting MAMAP(2,2) B+S: error = %f\n', fit_error);
    end
