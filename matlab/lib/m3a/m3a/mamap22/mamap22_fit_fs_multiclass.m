...
m3a_printf('Fitting MAMAP(2,2) F+S: form = %d\n', form);
...
if (form == 1 && (r1 < degentol || r2 > 1-degentol || abs(h1 - h2 + h2*r1) < degentol )) || ...
    ...
    m3a_printf('Fitting MAMAP(2,2) F+S: perturbing degenerate Poisson to second-order\n');
    if r1 < degentol
        r1 = degentol;
    end
    if r2 > 1-degentol
...
elseif form == 2 && r2 < degentol && abs(1-r1) < degentol
    ...
    m3a_printf('Fitting MAMAP(2,2) F+S: detected degenerate phase-type form\n');
...
elseif form == 1 && r2 < degentol
    ...
    m3a_printf('Fitting MAMAP(2,2) F+S: detected canonical phase-type form\n');
    ...
    scv = map_scv(aph);
    if scv < 1.0 + degentol
        m3a_printf('Fitting MAMAP(2,2) F+S: hypoexponential detected (SCV=%.4f), converting to non-canonical form\n', scv);
...
elseif (form == 1 && abs(1-r1) < degentol) || ...
    ...
    m3a_printf('Fitting MAMAP(2,2) F+S: detected non-canonical phase-type form\n');
    ...
    if ~(isfeasible(q1) && isfeasible(q2) && isfeasible(q3))
        ...
        A = zeros(4,1);
        b = zeros(4,1);
        Aeq = []; % no equality constraints in this one-variable repair
        beq = [];
        % coefficients
        q2_F = - p(1) / (h1 * (r2-1));
        q2_0 = + p(1) * h2 / (h1 * (r2-1));
...
elseif form == 2 && r2 < degentol
    ...
    m3a_printf('Fitting MAMAP(2,2) F+S: detected degenerate MMAP form\n');
    
    if fsWeights(1) > fsWeights(2)
        m3a_printf('Fitting MAMAP(2,2) F+S: fitting F\n');
        ...
        m3a_printf('Fitting MAMAP(2,2) F+S: fitting S\n');
        ...
        if ~(isfeasible(q1) && isfeasible(q2) && isfeasible(q3))
            ...
            if sol.problem ~= 0
                ...
                m3a_printf('Fitting MAMAP(2,2) F+S: YALMIP objective = %f\n', double(obj));
            end
            % compute coefficient
    ...
    if (form == 1 && abs(h2-h1*r2) < degentol) || (form == 2 && abs(h1 - h2 - h1*r1 + h1*r1*r2) < degentol)
        m3a_printf('Fitting MAMAP(2,2) F+S: detected fittable Poisson process\n')
    end
    ...
    if isfeasible(q1) && isfeasible(q2) && isfeasible(q3)
        m3a_printf('Fitting MAMAP(2,2) F+S: exact fit found\n');
        exact = 1;
    elseif adjust
        % YALMIP is only needed by the nonconvex repair, so its options are
        % built here rather than above: an EXACT closed-form fit must not
        % require a YALMIP installation.
...
fS = mmap_sigma(mmap);
    ...
    function [q1,q2,q3,obj] = solve_nonlinear(optim_space)
        ...
        m3a_printf('Fitting MAMAP(2,2) F+S: running approximate fitting (nonlinear)\n');
        ...
        if strcmp(optim_space, 'param')
            ...
            m3a_printf('Fitting MAMAP(2,2) F+S: F1 == M1, objective = %e\n', eobj);
            % left solution
            [lfeas,lobj,lF1,lS11] = solve_nonlinear_func('left');
            if lfeas
                m3a_printf('Fitting MAMAP(2,2) F+S: F1 < M1, objective = %e\n', lobj);
            else
                m3a_printf('Fitting MAMAP(2,2) F+S: F1 < M1 infeasible\n');
            end
            % right solution
...
% used for non-degenerate cases: parmeter space
    function [q1,q2,q3,obj] = solve_nonlinear_param()
        ...
        if zsol.problem == 0
            ...
            q3 = double(vq(3));
            obj = double(zobj);
            m3a_printf('Fitting MAMAP(2,2) F+S: solver (parameter space) objective = %e\n', obj);
        else
            fname = tempname;
...
% used for non-degenerate cases: hybrid space
    function [feas,bobj,bF1,bS11] = solve_nonlinear_hybrid(side)
        ...
        if hsol.problem == 0
            m3a_printf('Fitting MAMAP(2,2) F+S: solver (hybrid space) objective = %f\n', double(hobj));
            feas = 1;
            bobj = double(hobj);
            bF1 = double(vF1);
            bS11 = double(vS11);
        elseif hsol.problem == 1 || hsol.problem == 12
            m3a_printf('Fitting MAMAP(2,2) F+S: program is infeasible\n');
            feas = 0;
            bobj = nan;
            bF1 = nan;
            bS11 = nan;
...
% used for non-degenerate cases: hybrid space with z variable and implicit vS11
    function [feas,bobj,bF1,bS11] = solve_nonlinear_hybrid_z(side)
        ...
        hsol = solvesdp(hcstr, hobj, yalmip_nonlinear_opt);
        if hsol.problem == 0
            m3a_printf('Fitting MAMAP(2,2) F+S: solver (hybrid-z space) objective = %f\n', double(hobj));
            feas = 1;
            bobj = double(hobj);
            bF1 = double(vF1);
            bS11 = double(S11exp);
...
% used for non-degenerate cases: characteristic space
    function [feas,bobj,bF1,bS11] = solve_nonlinear_char(side)
        is_left = 0;
        if strcmp(side,'left')
            m3a_printf('Fitting MAMAP(2,2) F+S: optimizing for F11 < M1\n');
            is_left = 1;
        else
            m3a_printf('Fitting MAMAP(2,2) F+S: optimizing for F11 > M1\n');
        end
        ...
        if use_linprog_prepass
            ...
            elseif lsol.problem == 1 || lsol.problem == 12
                m3a_printf('Fitting MAMAP(2,2) F+S: linear program is infeasible\n');
                feas = 0;
            else
                fname = tempname;
                save(fname,'map','p','F','S');
        ...
        if feas
            ...
            if csol.problem ~= 0
                ...
                bF1 = double(vF1);
                bS11 = double(vS11);
                m3a_printf('Fitting MAMAP(2,2) F+S: solver (characteristic space) objective = %e\n', double(cobj));
            end
        else
...
% used for degenerate case
    function x = solve_quadprog()
        m3a_printf('Fitting MAMAP(2,2) F+S: running quadratic programming solver...\n');
        ...
        m3a_printf('Fitting MAMAP(2,2) F+S: error = %f\n', fit_error);
    end
