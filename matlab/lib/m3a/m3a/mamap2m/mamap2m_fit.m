...
if length(P) > 2
    m3a_printf('Fitting MAMAP(2,m): fitting F+B because m > 2\n');
    MMAP = mamap2m_fit_gamma_fb(M1, M2, M3, GAMMA, P, F, B);
    return;
end

if abs(GAMMA) < gammatol
    m3a_printf('Fitting MAMAP(2,m): fitting MAPH because gamma = %e\n', GAMMA);
...
if length(MAPS) == 1 && size(MAPS{1}{1},1) == 1
    ...
    if ~isempty(MAPS2)
        m3a_printf('Fitting MAMAP(2,m): converting Poisson to second-order process\n');
        ...
        m3a_printf('Fitting MAMAP(2,m): fitting marked Poisson because the underlying process has one state\n');
        MAP = MAPS{1};
        m = length(P);
        MMAP = cell(1,2+m);
        MMAP{1} = MAP{1};
