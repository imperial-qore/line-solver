...
%m3a_printf('Fitting MAMAP(2,m) F+B: form = %d\n', form);
...
if (form == 1 && (r1 < degentol || r2 > 1-degentol || abs(h2-h1*r2) < degentol || abs(h1 - h2 + h2*r1) < degentol )) || ...
   (form == 2 && (r2 > 1-degentol || abs(h1 - h2 + h2*r1) < degentol || abs(h1 - h2 - h1*r1 + h1*r1*r2) < degentol ))
    ...
    % POISSON PROCESS
 %   m3a_printf('Fitting MAMAP(2,m) F+B: detected Poisson process\n');
...
elseif (form == 2 && r2 < degentol && abs(1-r1) < degentol)
  ...
  %  m3a_printf('Fitting MAMAP(2,m) F+B: detected degenerate phase-type form\n');
...
%    m3a_printf('Fitting MAMAP(2,m) F+B: detected canonical phase-type form\n');
    
%    m3a_printf('Fitting MAMAP(2,m) F+B: fitting backward\n');
...
elseif (form == 1 && abs(1-r1) < degentol) || ...
 ...
 %   m3a_printf('Fitting MAMAP(2,m) F+B: detected non-canonical phase-type form\n');
    ...
    for c = 1:k
  %      m3a_printf('Fitting MAMAP(2,m) F+B: F(%d) = %f -> %f\n', c, F(c), fF(c));
    end
...
elseif form == 2 && r2 < degentol
   ...
   % m3a_printf('Fitting MAMAP(2,m) F+B: detected degenerate MMAP form\n');
    
    if fbWeights(1) >= fbWeights(2)
        
    %    m3a_printf('Fitting MAMAP(2,m) F+B: fitting forward\n');
     ...
     %       m3a_printf('Fitting MAMAP(2,m) F+B: F(%d) = %f -> %f\n', c, F(c), fF(c));
      ...
      %  m3a_printf('Fitting MAMAP(2,m) F+B: fitting backward\n');
        ...
        for c = 1:k
       %     m3a_printf('Fitting MAMAP(2,m) F+B: B(%d) = %f -> %f\n', c, B(c), fB(c));
        end
...
fB = mmap_backward_moment(mmap, 1);
    ...
    function x = solve()
       % m3a_printf('Fitting MAMAP(2,m) F+B: running quadratic programming solver...\n');
        ...
        %m3a_printf('Fitting MAMAP(2,m) F+B: error = %e\n', fit_error);
    end
