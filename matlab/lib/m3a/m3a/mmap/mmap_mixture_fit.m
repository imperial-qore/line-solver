...
for i = 1:m
    for j = 1:m
        ...
        % m3a_printf('PH{%d,%d}: M1 = %.3e, M2 = %.3e (%.3e), M3 = %.3e (%.3e)\n', i, j, m1, m2, M2(i,j), m3, M3(i,j));
    end
end
...
% for i = 1:m
%     for j = 1:m
%         %m3a_printf('MMAP CROSS(%d,%d): ', i, j);
%         for h = 1:3
%           %  m3a_printf('M%d = %.3e (%.3e)', h, FM{h}(i,j), M{h}(i,j));
%             if h < 3
%                 m3a_printf(', ');
%             end
%         end
...
% for i = 1:m
%     for j = 1:m
%         %m3a_printf('MMAP p(%d,%d,:): ', i, j);
%         for h = 1:m
%          %   m3a_printf('p(%d,%d,%d) = %.3e (%.3e)', i, j, h, P2(i,j,h), FP2(i,j,h));
%             if h < m
%                 m3a_printf(', ');
%             end
%         end
