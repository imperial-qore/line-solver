...
elseif method == 2
    ...
    M2a = max(3/2*M1^2, M2);
    % find a feasible value of the third moment. The interval must be derived
    % from the ADJUSTED second moment: with the unadjusted M2 the branch test
    % and the bound formulas below refer to a normalized moment the search is
    % not constrained to, which can return an inverted interval and abort
    % nonlcon_theoretical2 with a complex constraint value.
    n2 = M2a/(M1^2);
...
elseif method == 4
    ...
    function [c, ceq] = nonlcon_theoretical(x)
        ...
        %m3a_printf('M1 = %f, M2 = %f, M3 = %f, GAMMA = %f\n', M1, xM2, xM3, xGAMMA);
        %m3a_printf('n2 = %f, n3 = %f\n', n2, n3);

        % simplifying notations for moments bounds. The formulas are only
        % defined on 3/2 <= n2 < 2, so evaluate them at a CLAMPED n2: an
        % iterate below 3/2 (which constraint 1 is there to push back) or at
        % exactly 2 would otherwise leave the third-moment rows at zero, and
        % M3 would be unconstrained while constraint 1 is satisfied to solver
        % tolerance. Observed: the search returning n2 = 3/2 - 1.3e-11 with
        % n3 = 0.795, where the only admissible n3 is 2.
        n2b = min(max(n2, 3/2), 2 - 1e-12);
        p2 = 3*(n2b-2)/(3*n2b) * (-2*sqrt(3)/sqrt(12-6*n2b) - 1);
        a2 = (n2b-2)/(p2*(1-n2b) + sqrt(p2^2 + (2*p2*(n2b-2))));
        l2 = 3*(a2+1)/(a2*p2+1) - (6*a2)/(2+a2*p2*(2*a2+2));
        u2 = 6*(n2b-1)/n2b;

        %if (M2-M1^2)/M1^2 == 1
        %    m3a_printf('Exponential\n');
        %end
        ...
        %m3a_printf('LB1 = %f, LB2 = %f\n', lb1, lb2);
        if n2 < 2
            %m3a_printf('LB1 <= gamma: %d\n', lb1 <= xGAMMA);
            %m3a_printf('gamma <= UB: %d\n', xGAMMA <= ub1);
            c(4) = lb1 - xGAMMA;
            c(5) = xGAMMA - ub1;
        elseif n2 > 2 && n3 < 9 - 12/n2
            %m3a_printf('LB1 <= gamma: %d\n', lb1 <= xGAMMA);
            %m3a_printf('gamma <= 1: %d\n', xGAMMA <= 1);
            c(4) = lb1 - xGAMMA;
            c(5) = xGAMMA - 1;
        elseif n2 > 2 && n3 >= 9 - 12/n2
            %m3a_printf('LB2 <= gamma: %d\n', lb2 <= xGAMMA);
            %m3a_printf('gamma <= 1: %d\n', xGAMMA <= 1);
            c(4) = lb2 - xGAMMA;
            c(5) = xGAMMA - 1;
        end
    ...
    function [c, ceq] = nonlcon_theoretical2(x)
        ...
        % compute normalized moments, from the ADJUSTED second moment that the
        % bound interval was built on
        n2 = M2a/(M1^2);
        n3 = xM3/(M1*M2a);
