...
if options.Method==0 % two-state AMAP compression
    ...
    m3a_printf('Init: M3A will search for a %s\n',MMAPType);
    MMAP = mamap2m_fit_gamma_fb_mmap(MMAP);
else
    m3a_printf('Unsupported option.\n');
    MMAP={};
    return
end
...
if mmap_isfeasible(MMAP)
    m3a_printf('Output: M3A found a valid %s.\n',MMAPResType);
else
    m3a_printf('Output: M3A could *not* obtain a valid MMAP.\n');
end
