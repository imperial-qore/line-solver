...
%     D1=Z1'*A*Z1;
    ...
    e1=eye(lengthD);
    e1(1:size(X1,1),(size(e1,1)-size(X1,2)+1):lengthD)=X1;

    e2=eye(size(X2,1)+size(X2,2));
    e2(1:size(X2,1),(size(e2,1)-size(X2,2)+1):size(e2,1))=X2;

    e3=eye(lengthD);
    e3((lengthD-size(e2,1)+1):lengthD,(lengthD-size(e2,1)+1):lengthD)=e2;
