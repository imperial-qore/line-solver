...
function [alpha, A, logli] = PHFromTrace (trace, orders, maxIter, stopCond, initial, result)
    ...
    while abs((ologli-logli)/logli)>stopCond && steps<=maxIter
       ...
       % lineStart or BuToolsInit runs, and `[] && x` is a hard error in MATLAB,
       % so this would abort the fit rather than merely stay quiet. `if []` is
       % simply false, so the nested form is safe either way.
       if BuToolsVerbose
           if etime(clock(),t1)>2
