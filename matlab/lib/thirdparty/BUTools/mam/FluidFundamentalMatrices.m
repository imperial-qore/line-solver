...
function varargout = FluidFundamentalMatrices (Fpp, Fpm, Fmp, Fmm, matrices, precision, maxNumIt, method)
    ...
    % Nested rather than a single && expression: BuToolsVerbose is [] until
    % lineStart or BuToolsInit runs, and `true && []` is a hard error in MATLAB.
    % The left operand is true exactly on non-convergence, so the short circuit
    % that normally hides this fails precisely when the diagnostic is wanted.
    % `if []` is simply false, so the nested form is safe either way.
    if numit == maxNumIt
