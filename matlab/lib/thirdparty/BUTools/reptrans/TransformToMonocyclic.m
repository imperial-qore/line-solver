    ...
    function febs = generateFEBs (A, evalues, repeats, maxSize, precision)
        ...
        while i<=length(evalues)
            ...
            if -evalimag < precision
                ...
                n = 3;
                size = size + 3;   
                % The feedback probability z is strictly less than 1 only if
                % the ratio stays strictly below cot(pi/n). At the exact
                % boundary (ratio == cot(pi/n)) rounding can let the loop
                % exit with z == 1, which yields a block with zero exit rate,
                % hence a degenerate generator with a zero eigenvalue. Keep a
                % margin so that the boundary is always resolved upwards.
                while evalimag / real(evalues(i)) >= cot(pi/n) - sqrt(eps)
                    n = n + 1;
                    size = size + 1;
                    if size > maxSize
