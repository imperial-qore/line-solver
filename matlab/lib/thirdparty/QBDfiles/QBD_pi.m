...
temp=(eye(m)-R)^(-1);
% Positive recurrence is sp(R)<1. For a Markovian QBD, R is nonnegative and
% sp(R)<1 is equivalent to (eye(m)-R)^(-1)>=0, the entrywise test used here
% originally; with RAP components R may have negative entries, (eye(m)-R)^(-1)
% then legitimately has negative entries too, and the entrywise test rejects
% stable RAP/RAP/1 queues. The eigenvalue test is the condition itself.
if( max(abs(eig(R))) >= 1 )
