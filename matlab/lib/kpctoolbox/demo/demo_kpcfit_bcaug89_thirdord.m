...
subplot(1,2,2)
BCest=[];
surf(log10(trace.BCGridLags)',log10(trace.BCGridLags)',reshape((trace.BC),5,5)); hold on;
for i=1:size(trace.BCLags,1)
    BCest(end+1) = map_joint(MAP,trace.BCLags(i,:),[1,1,1]);
end
surf(log10(trace.BCGridLags)',log10(trace.BCGridLags)',reshape((BCest),5,5)); hold on;
...
surf(log10(trace.BCGridLags)',log10(trace.BCGridLags)',reshape((BCest2),5,5)); hold on;
