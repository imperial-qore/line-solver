function model = ctmc_tandem(c)
% CTMC_TANDEM  Tandem queueing network of Hermanns, Meyer-Kayser and Siegle.
%
% MODEL = CTMC_TANDEM(C) builds LINE's encoding of a CTMC case study
...
% first station is blocked while the second is full: the reference encoding
% disables the synchronised route action, freezing the Coxian phase rather than
% letting the job finish and wait. That is not LINE's blocking-after-service, so
% the model is built as a Petri net, where an inhibitor arc reproduces the freeze
