%% LINE against PRISM's own CTMC case studies.
%
% The reference values are PRISM's, computed from the models distributed with
% PRISM 4.10.2-dev (prismmodelchecker/prism at commit 9486c774), through the
% solverprism driver with the explicit engine at a termination tolerance of
% 1e-12, and confirmed against the sparse engine with backwards Gauss-Seidel at
% 1e-14. They are goldens here rather than recomputed, so that this script needs
% only LINE; the PRISM invocation is recorded beside each block.
%
...
% PRISM: {"model":"tandem.sm","consts":{"c":2}} with rewards
...
t = SolverCTMC(prism_tandem(2)).getAvgTable();
...
% PRISM: {"model":"poll2.sm"} with rewards "E_s1" true : s1; "E_s2" true : s2;
...
t = SolverCTMC(prism_polling()).getAvgTable();
qlen = @(n) t.QLen(strcmp(string(t.Station), n));
...
for i = 1:numel(names)
    ...
    worst = max(worst, dev);
    fprintf('  %-14s PRISM %.12f   LINE %.12f   rel dev %.2e\n', names{i}, a, b, dev);
end
