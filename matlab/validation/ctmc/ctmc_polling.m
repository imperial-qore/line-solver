function model = ctmc_polling()
% CTMC_POLLING  Two-station cyclic server polling system of Ibe and Trivedi.
%
% MODEL = CTMC_POLLING() builds LINE's encoding of a CTMC case study
% distributed with an external probabilistic model checker, whose source is
%
...
% rate lambda = mu/N. The reference encoding carries the server position s and
% its activity a as integer variables; here each is one-hot over a pair of
% places, which is what a Petri net encoding of a finite control variable looks
% like.
%
% Arrivals are modelled as a closed net rather than with a Source. The reference
% enables each station's arrival command exactly while that station is empty, so
% a two-token pool feeding the two stations is an exact equivalent: a station
