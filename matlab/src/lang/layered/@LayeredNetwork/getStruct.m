...
lsn.hasretrieval = zeros(lsn.nhosts+lsn.ntasks,1);
...
for e=1:lsn.nentries
    ...
    % Explicit replies apply to plain entries and item entries alike -- see _kb/04-networkstruct.md
...
% Quorum count of an AND-join, indexed by the join target activity. Equals the
% number of predecessors when the join waits for all of them.
lsn.actquorum = sparse(lsn.nidx,1);
...
for t = 1:lsn.ntasks
    ...
    for ap=1:length(tasks{t}.precedences)
        ...
        % AND-join quorum recorded in actquorum, not folded into graph edge weights (which must stay 1) -- see _kb/04-networkstruct.md
        quorumCount = 0;
        if pretype == ActivityPrecedenceType.PRE_AND
            quorum = tasks{t}.precedences(ap).preParams;
            if isempty(quorum) || numel(quorum) ~= 1 || quorum < 1 || quorum > length(preacts)
                quorumCount = length(preacts);
            else
                quorumCount = round(quorum);
            end
        end
            ...
            switch posttype
                ...
                otherwise
                    for posta = 1:length(postacts)
                        ...
                        lsn.actposttype(postaidx) = sparse(tasks{t}.precedences(ap).postType);
                        if quorumCount > 0
                            lsn.actquorum(postaidx) = quorumCount;
                        end
                    end
            end
        end
...
for e = 1:length(self.entries)
    ...
    for fw = 1:length(entry.forwardingDests)
        ...
        % issynccaller NOT set for forwarding calls (forwarder does not wait; target replies to original caller) -- see _kb/04-networkstruct.md
    end
end
...
for tidx = find(lsn.sched == SchedStrategy.INF)'  % transpose to iterate over each element
    if lsn.type(tidx) == LayeredNetworkElement.TASK
        ...
        if any(callers_inf)
            % An infinite-multiplicity caller would need recursive max-multiplicity resolution; use a heuristic instead
...
% dag vs graph: entry-task edges reversed, loop edges removed -- see _kb/04-networkstruct.md
dag = lsn.graph;
n = size(dag, 1);
...
% A replying activity must have no Phase 1 successors (Phase 2 = post-reply processing is allowed) -- see _kb/04-networkstruct.md
for a = 1:lsn.nacts
...
% An entry must not be called both sync and async; FWD calls exempted (lqns interlock-forwarded pattern) -- see _kb/04-networkstruct.md
if ~isempty(lsn.callpair)
    ...
    for eidx=target_eidxs(:)'
        ...
        call_types_to_eidx(call_types_to_eidx == CallType.FWD) = [];
        if ~isempty(call_types_to_eidx) && ~all(call_types_to_eidx == call_types_to_eidx(1))
