classdef Signal < JobClass
    ...
    methods
        ...
        function concrete = resolve(self, isOpen, refstat)
            ...
            % Save properties before replacing the placeholder
            savedIndex = self.index;
            savedName = self.name;
            savedTargetJobClass = self.targetJobClass;

            % Everything a node holds per class, outputStrategy above all, is
            % stored by class index and is not renumbered when a class moves.
            % Resolving a signal at a new index therefore reassigns another
            % class's routing: with two signals the second one's routes were
            % overwritten by the first's, leaving its station unreachable and
            % its reply lost. So snapshot the routing, put the concrete signal
            % back in the placeholder's slot, and restore the snapshot: the
            % index it occupies never changes.
            savedOutputStrategy = cell(1, length(self.model.nodes));
            for n = 1:length(self.model.nodes)
                node = self.model.nodes{n};
                if ~isempty(node.output) && isprop(node.output, 'outputStrategy')
                    savedOutputStrategy{n} = node.output.outputStrategy;
                end
            end

            % The placeholder must go before the concrete signal is created,
            % since addJobClass rejects a duplicate name.
            placeholderIdx = find(cellfun(@(c) c == self, self.model.classes), 1);
            if isempty(placeholderIdx)
                placeholderIdx = savedIndex;
            else
            ...
            appendedIdx = find(cellfun(@(c) c == concrete, self.model.classes), 1);
            if ~isempty(appendedIdx)
                self.model.classes(appendedIdx) = [];
            end
            isColumn = size(self.model.classes, 2) == 1;
            cls = self.model.classes(:).';
            cls = [cls(1:placeholderIdx-1), {concrete}, cls(placeholderIdx:end)];
            if isColumn
                cls = cls.';
            end
            self.model.classes = cls;
            for c = 1:length(self.model.classes)
                self.model.classes{c}.index = c;
            end

            for n = 1:length(self.model.nodes)
                if ~isempty(savedOutputStrategy{n})
                    self.model.nodes{n}.output.outputStrategy = savedOutputStrategy{n};
                end
            end
        ...
        function self = forJobClass(self, jobClass)
            ...
            % Only a REPLY signal establishes the synchronous-call link:
            % for a NEGATIVE or CATASTROPHE signal forJobClass merely names the
            % victim class, and setting replySignalClass there made
            % sn.syncreply >= 0, so the state layer reserved a held-server
            % counter and rejected every non-FCFS station.
            if ~isempty(jobClass) && self.signalType == SignalType.REPLY
