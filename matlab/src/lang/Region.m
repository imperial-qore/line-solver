classdef Region < handle
    ...
    methods
        ...
        function self = setConstraint(self, A, b)
            % SELF = SETCONSTRAINT(A, B) — Alias of setLinearConstraints
            % matching the JAR jline.lang.Region API.
            self.setLinearConstraints(A, b);
        end

        function [A, b] = getLinearConstraints(self)
            % [A, B] = GETLINEARCONSTRAINTS() — Linear admission constraint
            % pair (A,b) with matrix A (C x K) and capacity vector b (C x 1),
            % or empties if not set.
            A = self.constraintA;
            b = self.constraintB;
        end
