...
if nargin > 2 && ~isempty(s)
    for ist=1:sn.nstations
        % A queueing place (QPN embedded queue) carries a token-marking state that
        % does not split in-service from waiting tokens; its embedded-server occupancy
        % is managed by the simulation algorithm, not the native state, so the
        % running-jobs-versus-servers constraint below does not apply to Place nodes.
        isPlaceStation = sn.nodetype(sn.stationToNode(ist)) == NodeType.Place;
        if sn.nservers(ist)>0 && ~isPlaceStation
            % if more running jobs than servers
            if sum(s(ist,:)) > sn.nservers(ist)
                switch sn.sched(ist) % don't flag invalid if ps
                    case {SchedStrategy.FCFS,SchedStrategy.SIRO,SchedStrategy.LCFS,SchedStrategy.HOL,SchedStrategy.POLLING}
                        isValid = false;
                        return
                end
            end
