...
% end
while 1
    % Cooperative wall-clock budget checkpoint (options.timeout): a partial
    % reachable space would give silently wrong results, so abort the solve.
    if lineTimeoutExceeded(options)
        line_error(mfilename,'Reachable state space generation exceeded the wall-clock time budget (options.timeout=%gs).', options.timeout);
    end
    if isempty(stack)
        %Q = ctmc_makeinfgen(Q);
        SSq = []; % initial state
    ...
    if isfield(sn,'gsync') && ~isempty(sn.gsync)
        for g=1:length(sn.gsync)
            gind = sn.gsync{g}.active{1}.node;
            isf_t = sn.nodeToStateful(gind);
            [outglspace, outrate, outprob] = State.afterGlobalEvent(sn, gind, stateCell, sn.gsync{g}, false);
            if isempty(outrate)
                continue
            end
            for io=1:length(outrate)
                ...
                if size(outglspace{isf_t},1) < io
                    continue
                end
                gStates = stateCell;
                gStates{isf_t} = outglspace{isf_t}(io,:);
                if sn.gsync{g}.active{1}.event == EventType.FIRE
                    for i=1:nstateful
                        if i ~= isf_t && ~isequal(stateCell{i}, outglspace{i})
                            gStates{i} = outglspace{i};
                        end
                    end
                end

                % see _kb/04-networkstruct.md (SPN/GSPN state generation) for rationale
                if i_hasNegativePlace(sn, gStates)
                    line_error(mfilename, sprintf(['%s at %s produced a negative place: ' ...
                        'a firing completed without its input place holding the tokens ' ...
                        'to pay for it. The FIRE handler in afterGlobalEvent should have ' ...
                        'gated this completion on the enabling degree.'], ...
                        EventType.toText(sn.gsync{g}.active{1}.event), sn.nodenames{gind}));
                end

                newstatec = cell(1,nstateful);
                for i=1:nstateful
                    maxstatesz(i) = max(maxstatesz(i),length(gStates{i}));
                    newstatec{i} = [zeros(1,maxstatesz(i)-length(gStates{i})),gStates{i}];
                end
                hashednewstate = zeros(1,nstateful);
                for i=1:nstateful
                    if length(newstatec{i}) > size(space{i},2)
                ...
                if jh == -1
                    ...
                    SSh = [SSh; hashednewstate]; %#ok<AGROW>
                    stack{end+1} = newstatec; %#ok<AGROW>
                    stack_index(end+1) = size(SSh,1); %#ok<AGROW>
                end
            end
        end
    end
    % explore fork firing synchronizations (FJ-augmented structs only):
    % each firing consumes the parent held at the Fork and emits one
    % sibling per branch atomically (multi-node successor states)
    if isfield(sn,'fjsync') && ~isempty(sn.fjsync)
        for k=1:length(sn.fjsync)
            [fjStates, ~, fjprob] = State.afterFJEvent(sn, sn.fjsync{k}, stateCell, false);
            for io=1:length(fjStates)
                if fjprob(io) > 0
                    newstatec = cell(1,nstateful);
                    for i=1:nstateful
                        maxstatesz(i) = max(maxstatesz(i),length(fjStates{io}{i}));
                        newstatec{i} = [zeros(1,maxstatesz(i)-length(fjStates{io}{i})),fjStates{io}{i}];
                    end
                    hashednewstate = zeros(1,nstateful);
                    for i=1:nstateful
                        if length(newstatec{i}) > size(space{i},2)
...
function neg = i_hasNegativePlace(sn, stateCell)
% True when any place in the successor holds a negative number of tokens, which
% means the firing that produced it consumed tokens the place did not have.
neg = false;
for ind = 1:sn.nnodes
    if ~sn.isstateful(ind) || sn.nodetype(ind) ~= NodeType.Place
        continue
    end
    isf = sn.nodeToStateful(ind);
    if isf > numel(stateCell) || isempty(stateCell{isf})
        continue
    end
    if any(stateCell{isf} < 0)
        neg = true;
        return
    end
end
