classdef MAP < MarkovModulated
    ...
    methods
        ...
        function n = getNumberOfPhases(self)
            % The method call needs its receiver: without it this reads as a
            % call to a function named D and errors out, which made every model
            % holding a MAP fail at getStruct time (refreshPetriNetNodes.m:50
            % asks a Markovian firing distribution for its phase count).
            n = size(self.D(0), 1);
        end
