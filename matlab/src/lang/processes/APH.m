    ...
    methods (Static)
        ...
        function ex = fitRawMoments(m1, m2, m3)
            ...
            if m1 <= GlobalConstants.FineTol
                % Same instantaneous-phase handling as fitMeanAndSCV above: the
                % rate is large but finite, and -Inf would void the generator.
                if m1 > 0
                    ex = Exp(1/m1);
                else
        ...
        function ex = fitMeanAndSCV(MEAN, SCV)
            ...
            % variance, skewness)
            if MEAN <= GlobalConstants.FineTol
                % see _kb/04-networkstruct.md (APH.m/PH.m sub-tolerance mean handling)
                if MEAN > 0
                    ex = Exp(1/MEAN);
                else
                    ex = Exp(1/GlobalConstants.Zero);
