    ...
    methods
        function update(self,varargin)
            ...
            if SCV<1
                ...
                if isempty(ph)
                    % the two-state search found nothing; fall back to the
                    % acyclic fitter, which searches the order itself
                    APH = aph_fit(e1,e2,e3);
                    T = APH{1};
                    alpha = map_pie(APH);
                else
                    T=ph{1,1};
...
function ex = fit(MEAN, SCV, SKEW)
        ...
        function ex = fitRawMoments(m1, m2, m3)
            ...
            if m1 <= GlobalConstants.FineTol
                ...
                % updateFromRawMoments does not exist on PH; convert to the
                % (mean, SCV, skewness) triple that update does accept
                SCV = (m2 - m1^2)/m1^2;
                SKEW = (m3 - 3*m1*m2 + 2*m1^3)/(m2 - m1^2)^(3/2);
                ex = PH(1.0, [1]);
                ex.update(m1, SCV, SKEW);
                ex.immediate = false;
            end
        end
        ...
        function ex = fitMeanAndSCV(MEAN, SCV)
            ...
            elseif SCV==1.0
                ...
                % Two moments only: take the minimum-skewness third moment of
                % the acyclic class, as APH.fitMeanAndSCV does. The SKEW that
                % used to be referenced here was never a parameter of this
                % method, so every call errored.
                ex = APH.fitMeanAndSCV(MEAN, SCV);
                ex.immediate = false;
            end
        end
