classdef ClassSwitch < Node
    ...
    methods
        ...
        function self = removeJobClass(self, jobclass)
            ...
            %
            % Slice the class-switching matrix, which is indexed by class
            % position, on top of the routing configuration handled by Node.

            remaining = self.remainingClassIndexes(jobclass);
            removeJobClass@Node(self, jobclass);
            if ~isempty(self.server) && isprop(self.server, 'csFun')
                % csFun is the handle @(r,s,...) csMatrix(r,s), so calling it
                % with the surviving index vectors returns the submatrix
                self.server.updateClassSwitch(self.server.csFun(remaining, remaining));
                if isprop(self.server, 'classes') && numel(self.server.classes) == numel(remaining) + 1
                    self.server.updateClasses(self.server.classes(remaining));
                end
            end
        end
