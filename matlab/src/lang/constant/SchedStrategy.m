classdef (Sealed) SchedStrategy
    ...
    properties (Constant)
        ...
        FSP      = 37;  % Fair Sojourn Protocol (preemptive; ranks by virtual PS finish time)
        PAS      = 38;  % Pass-and-swap (order-independent queue with class compatibility/swap graph)
        OI       = 39;  % Order-independent queue (pass-and-swap specialization with empty/zero swap graph)
    end
    ...
    methods (Static)
        ...
        function type = fromId(id)
            ...
            if ismember(id, [SchedStrategy.INF, SchedStrategy.FCFS, SchedStrategy.LCFS, ...
                            ...
                            SchedStrategy.FSP, SchedStrategy.PAS, SchedStrategy.OI])
                type = id;
            else
        ...
        function text = toFeature(type)
            ...
            switch type
                ...
                case SchedStrategy.FSP
                    text = 'SchedStrategy_FSP';
                case SchedStrategy.PAS
                    text = 'SchedStrategy_PAS';
                case SchedStrategy.OI
                    text = 'SchedStrategy_OI';
                otherwise
