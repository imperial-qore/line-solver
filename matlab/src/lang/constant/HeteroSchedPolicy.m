classdef (Sealed) HeteroSchedPolicy
    ...
    % - ALFS: Assign Least Flexible Server (prefer the type that serves fewest classes)
    ...
    methods (Static)
        ...
        function text = toJMTText(policy)
            % TOJMTTEXT Convert HeteroSchedPolicy to JMT's descriptive string
            %
            % JMT's CommonConstants.STATION_SCHEDULING_POLICY_* expects long
            % human-readable identifiers (e.g. "ALIS (Assign Longest Idle
            % Server)"). The bare names returned by toText do not match those
            % constants, so when writing JSIM XML use this method.
            switch policy
                case HeteroSchedPolicy.ORDER
                    text = 'Order (Assign according to order below)';
                case HeteroSchedPolicy.ALIS
                    text = 'ALIS (Assign Longest Idle Server)';
                case HeteroSchedPolicy.ALFS
                    text = 'ALFS (Assign Least Flexible Server)';
                case HeteroSchedPolicy.FAIRNESS
                    text = 'Fairness (Move back server type when used)';
                case HeteroSchedPolicy.FSF
                    text = 'FSF (Fastest Servers First)';
                case HeteroSchedPolicy.RAIS
                    text = 'RAIS (Random Assignment to Idle Servers)';
                otherwise
                    text = 'Order (Assign according to order below)';
            end
        end
