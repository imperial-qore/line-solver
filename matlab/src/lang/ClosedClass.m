classdef ClosedClass < JobClass
    ...
    methods
        ...
        function self = setPopulation(self, njobs)
            % SELF = SETPOPULATION(NJOBS)
            % Set the fixed circulating population of this closed class. Used
            % by the line-opt JobPopulation decision variable.
            if ~(rem(njobs,1) == 0 && njobs >= 0)
                line_error(mfilename,'Population must be a non-negative integer.\n');
            end
            self.population = njobs;
        end

        function self = setNumberOfJobs(self, njobs)
            % SELF = SETNUMBEROFJOBS(NJOBS)
            % Alias of setPopulation, mirroring the JAR ClosedClass API.
            self.setPopulation(njobs);
        end
    end
