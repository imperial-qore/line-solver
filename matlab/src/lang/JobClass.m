classdef JobClass < NetworkElement
    ...
    properties
        ...
        spawnClass; % Class of the job injected at the same station on each service completion of this class (LQN phase-2 continuation)
    ...
    methods (Access=public)
        ...
        function self = setPriority(self, priority)
            ...
            % Set the priority of this class (0 = highest). Used by the
            % line-opt ClassPriority decision variable.
            if ~(rem(priority,1) == 0 && priority >= 0)
                line_error(mfilename,'Priority must be a non-negative integer.\n');
            end
            self.priority = priority;
        end
        ...
        function self = setSpawnClass(self, spawnCls)
            % SELF = SETSPAWNCLASS(SPAWNCLS)
            %
            % On each service completion of a job of this class, a new job
            % of class SPAWNCLS is injected at the same station. Used for
            % LQN phase-2 continuations, where the reply token returns to
            % the caller while the served task continues its second phase.

            if ~isa(spawnCls, 'JobClass')
                line_error(mfilename, 'Spawn class must be a JobClass.');
            end
            self.spawnClass = spawnCls;
        end
