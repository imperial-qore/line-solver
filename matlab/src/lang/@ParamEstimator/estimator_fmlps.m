function estVal = estimator_fmlps(self, nodes)
node = nodes{1};
% ESTIMATORFMLPS Fluid Maximum Likelihood for Processor Sharing
% Estimates service demands using the FMLPS fluid likelihood method.
...
if node.schedStrategy ~= SchedStrategy.PS
    error('The FMLPS method is available only for processor sharing stations.');
end
...
for r = 1:R
    ...
    if ~arvData.isTrace()
        error('FMLPS estimator requires trace-format arrival data. Use setTrace() on the SampledMetric.');
    end
    ...
    if ~rtData.isTrace()
        error('FMLPS estimator requires trace-format response time data. Use setTrace() on the SampledMetric.');
    end
...
estVal = infer_fmlps(eqModel, eqNode, rt_sorted, class_sorted, ql, W);
estVal = estVal(:)';
