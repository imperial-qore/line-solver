classdef MNetwork < Model 
    ...
    properties (Hidden)
        ...
        csMatrix;
        hasStruct;
        % Monotonic counter bumped every time the struct is recompiled or
        % discarded. Consumers that cache something derived from the topology
        % (e.g. SolverMVA's MMT fork-join transformation) tag the cache with the
        % value seen at build time and reuse it only while the counter still
        % matches, so any structural change forces a rebuild. Python and the JAR
        % test identity of the NetworkStruct object for this; sn is a value
        % struct in MATLAB, so it needs an explicit version instead.
        structVersion = 0;
        allowReplace;
        forkStateful = false; % Fork nodes are stateful (FJ-augmented copies only, see ModelAdapter.fjtag)
        isFJAugmented = false; % true on fork-join tag-augmented copies (skips the MMT visit correction)
    end
    ...
    methods (Access=public)
        ...
        function bool = getChecks(self)
            % BOOL = GETCHECKS()
            % True if model validation is enabled on this model.
            bool = self.enableChecks;
        end
        ...
        function [stateSpace,nodeStateSpace] = stateSpace(self, varargin)
            % [STATESPACE,NODESTATESPACE] = STATESPACE(SELF, VARARGIN)
            % Alias for getStateSpace
            [stateSpace,nodeStateSpace] = self.getStateSpace(varargin{:});
        end
        ...
        function self = setUsedLangFeature(self,className)
            ...
            % An empty name means "no gated capability" (the toFeature
            % convention for internal markers such as FIRING/DISABLED routing);
            % it is skipped, as in jar Network.getUsedLangFeatures.
            if isempty(className)
                return
            end
            % see _kb/04-networkstruct.md (MNetwork.m feature-name normalization)
            if strcmp(className,'MarkedMAP') || strcmp(className,'MarkedMMPP')
                className = 'MMAP';
            end
        ...
        function list = getIndexStatefulNodes(self)
            ...
            if self.forkStateful
                % FJ tag-augmented copies treat Fork nodes as stateful
                list = union(list, find(cellisa(self.nodes, 'Fork'))');
            end
        end
    ...
    methods (Static)
        ...
        function model = cluster(lambda, D, strategy, S, dispatching)
            ...
            %
            % Open cluster: Source -> Dispatcher -> Server[1..M] -> Sink.
            %
            % lambda(r)   - arrival rate of class r
            % D(i,r)      - mean service time of class r at server i
        ...
        function model = clusterClosed(N, Z, D, strategy, S, dispatching)
            ...
            %
            % Closed cluster: Think -> Dispatcher -> Server[1..M] -> Think.
            %
            % N(r)        - population of class r
            % Z(r)        - per-class think time at the delay
            % D(i,r)      - mean service time of class r at server i
            % strategy{i} - scheduling strategy at server i
            % S(i)        - number of identical servers at queue i (default: 1)
            % dispatching - RoutingStrategy applied at the router (default: RAND)
            ...
            jobclass = cell(R,1);
            for r = 1:R
                jobclass{r} = ClosedClass(model, ['Class', num2str(r)], N(r), think, 0);
                think.setService(jobclass{r}, Exp.fitMean(Z(r)));
                for i = 1:M
                    servers{i}.setService(jobclass{r}, Exp.fitMean(D(i,r)));
                end
            end
        ...
        function P = serialRouting(varargin)
            ...
            % Auto-close the cycle (last -> first) unless the last node is a Sink
            % or the caller already closed the loop by repeating the first node as
            % the last (e.g. serialRouting(d,q,d)), which would otherwise add a
            % spurious self-loop. Mirrors the Python serial_routing guard.
            if ~isa(varargin{end},'Sink') && varargin{end} ~= varargin{1}
