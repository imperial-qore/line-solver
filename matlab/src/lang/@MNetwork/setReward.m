...
if ~isa(rewardFn, 'function_handle') && ~isa(rewardFn, 'RewardDescriptor')
    error('Reward function must be a function handle @(state, sn) -> double, or a RewardDescriptor returned by a Reward.* template');
end

% A RewardDescriptor is callable exactly like a bare handle, but additionally
% records what the reward measures ({kind, node, class}). Keep the descriptor so
% that linemodel_save can serialize the reward declaratively; a bare handle
% carries no such information and is stored as-is.
if isa(rewardFn, 'RewardDescriptor')
    descriptor = rewardFn;
else
    descriptor = [];
end
...
for i = 1:length(self.sn.reward)
    if strcmp(self.sn.reward{i}.name, name)
        ...
        self.sn.reward{i}.descriptor = descriptor;
        found = true;
        break;
    end
end
