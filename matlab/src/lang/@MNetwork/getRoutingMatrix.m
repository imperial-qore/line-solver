...
for ind=1:I % source
    ...
    elseif isa(nodes{ind}.server,'StatefulClassSwitcher')
        ...
        if isa(nodes{ind}.server,'CacheClassSwitcher')
            % A retrieval-pending class is an output class of the cache (like
            % hitClass / missClass): its routing must be taken from the routing
            % matrix directly rather than normalised/zeroed as a regular input
            % class. isReceived(r) is true when r appears as a retrieval class;
            % it is identically false for caches with no retrieval system.
            if isprop(nodes{ind}.server, 'retrievalClasses')
                rpClasses = nodes{ind}.server.retrievalClasses;
            else
                rpClasses = [];
            end
            isReceived = @(r) ~isempty(rpClasses) && any(rpClasses(:) == r);

            for r=1:K
                if (isempty(find(r == nodes{ind}.server.hitClass)) && isempty(find(r == nodes{ind}.server.missClass)) && ~isReceived(r))
            ...
            for r=1:K
                if length(nodes{ind}.server.actualHitProb)>=r && length(nodes{ind}.server.hitClass)>=r
                    ...
                    % Delayed hits (retrieval system) eventually depart as the
                    % hit class, so for flow conservation the routing split must
                    % send (true hit + delayed hit) to the hit class; otherwise
                    % ph+pm < 1 deflates the cache flow balance.
                    if isprop(nodes{ind}.server, 'actualDelayedHitProb') ...
                            && length(nodes{ind}.server.actualDelayedHitProb)>=r ...
                            && ~isnan(nodes{ind}.server.actualDelayedHitProb(r))
                        ph = ph + nodes{ind}.server.actualDelayedHitProb(r);
                    end
