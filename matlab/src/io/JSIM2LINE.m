                ...
                for j=1:ninputs
                    ...
                    for k=1:nclasses
                        ...
                        if inhibit == -1 || inhibit == 0
                            % JMT encodes "no inhibitor arc" as 0 (see the
                            % Inf->0 mapping in saveInhibitingConditions.m); a
                            % threshold of 0 would otherwise inhibit at >=0
                            % tokens, i.e. always, deadlocking the transition.
        ...
        otherwise
            for r=1:length(classes)
                switch xrouting{from}{r}.name
                    ...
                    case 'Power of k'
                        % see _kb/12-interfaces-and-docs.md (JSIM2LINE.m routing-strategy indexing)
                        k = 2;              % JMT default when <k> is absent
                        withMemory = false; % JMT default when <withMemory> is absent
                        xstrat = xsection_i{from}(3).parameter.subParameter(r);
                        if isfield(xstrat,'subParameter')
                            xpars = xstrat.subParameter;
                            for pj = 1:numel(xpars)
                                if ~isfield(xpars(pj),'ATTRIBUTE') || ~isfield(xpars(pj),'value')
                                    continue
                                end
                                switch xpars(pj).ATTRIBUTE.name
                                    case 'k'
                                        k = xpars(pj).value;
                                        if ischar(k)
                                            k = str2double(k);
                                    ...
                                    case 'withMemory'
                                        % A LINE-written file parses as logical,
                                        % a hand-written or older JMT one as the
                                        % text 'true'/'false'.
                                        wm = xpars(pj).value;
                                        if ischar(wm)
                                            withMemory = strcmpi(strtrim(wm),'true');
                                        else
                                            withMemory = logical(wm);
                                        end
                                end
                            end
                        end
                        % JMT withMemory=true selects Anselmi & Dufour
                        % SQ(d,N), which LINE does not implement. The flag is
                        % dropped and the model is imported as plain SQ(d).
                        if withMemory
                            line_warning(mfilename, 'JMT <withMemory>=true ignored: LINE supports SQ(d) only, so the imported model will not match JMT.');
                        end
                        node{from}.setRouting(jobclass{r}, RoutingStrategy.SQ, k);
                    case 'Round Robin'
