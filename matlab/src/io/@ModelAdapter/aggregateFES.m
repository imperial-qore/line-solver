...
escape = zeros(1, K);
for r = 1:K
    for a = 1:length(subsetIndices)
        j = subsetIndices(a);
        Vjr = L_iso(a, r) * sn.rates(j, r);   % visit ratio used in the demand
        isf_j = sn.stationToStateful(j);
        pexit = 0;
        for i = complementIndices
            isf_i = sn.stationToStateful(i);
            pexit = pexit + rt((isf_j-1)*K + r, (isf_i-1)*K + r);
        end
        if isfinite(Vjr)
            escape(r) = escape(r) + Vjr * pexit;
        end
    end
    if escape(r) > GlobalConstants.FineTol
        scalingTable{r} = scalingTable{r} * escape(r);
    end
end

if options.verbose
    fprintf('Throughput table computed for %d states.\n', prod(cutoffs + 1));
    fprintf('Norton escape factors per class: %s\n', mat2str(escape, 4));
end
...
% Set the class dependence on the FES from per-class throughput scaling tables
% The scalingTable{k} contains throughputs for class k at each population state
%
% For multi-class FES, we use a class-dependence handle (cdscaling) which allows
...
% Set base service rate (will be scaled by the class dependence)
% Use Exp(1.0) as base; the class-dependent scaling provides the actual throughput rate
for k = 1:K
...
% see _kb/12-interfaces-and-docs.md (@ModelAdapter: JMT export helpers) for rationale
fesBetaHandle = fes_beta_handle(scalingTable, cutoffs);
...
fesPeak = cd_peak_scaling(fesBetaHandle, cutoffs, numel(cutoffs));
fesStation.setLimitedClassDependence(fesBetaHandle, fesPeak);
...
for k = 1:K
    ...
    visitRatios = visits_iso(:, k)';  % 1 x nSub, class-k visit ratios
