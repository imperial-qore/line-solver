...
for ist = 1:sn.nstations
    ...
    hasRetrialProc = isfield(sn,'retrialProc') && ~isempty(sn.retrialProc) ...
        && size(sn.retrialProc,1) >= ist && any(~cellfun(@isempty, sn.retrialProc(ist,:)));
    isBufferless = isfinite(sn.cap(ist)) && sn.cap(ist) == sn.nservers(ist);
...
if isempty(bufferlessStations)
    retInfo.errorMsg = 'No retrial queue found (configure setOrbit, or a bufferless station with setRetrial).';
    return;
end
