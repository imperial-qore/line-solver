function result = qsys_geoxgeo1(a, beta, s, convention)
...
%
% Examples:
%   r = qsys_geoxgeo1(0.1, 0.5, 0.9);   % lambda = 0.2
%   r.arrivalRate                        % 0.2
...
result = qsys_geoxgeo1_moments(a, batchMean, batchSecondFactorial, s, convention);
end

function result = qsys_geoxgeo1_moments(a, batchMean, batchSecondFactorial, s, convention)
...
if ~ischar(convention) && ~isstring(convention)
    line_error('qsys_geoxgeo1', 'convention must be the string ''LAS_DA'' or ''EAS''');
end
convention = upper(char(convention));
if ~strcmp(convention, 'LAS_DA') && ~strcmp(convention, 'EAS')
    line_error('qsys_geoxgeo1', 'convention must be ''LAS_DA'' or ''EAS''');
end
if ~isnumeric(a) || ~isscalar(a) || ~isreal(a) || a <= 0 || a > 1
    line_error('qsys_geoxgeo1', 'a must be a real scalar in (0,1]');
end
if ~isnumeric(s) || ~isscalar(s) || ~isreal(s) || s <= 0 || s > 1
    line_error('qsys_geoxgeo1', 's must be a real scalar in (0,1]');
end
if batchMean < 1
    line_error('qsys_geoxgeo1', 'mean batch size must be at least 1: a batch that arrives carries at least one job');
end
if batchSecondFactorial < 0
    line_error('qsys_geoxgeo1', 'E[X(X-1)] must be non-negative');
end
...
if batchSecondFactorial < minSecondFactorial - 1e-9 * max(1, minSecondFactorial)
    line_error('qsys_geoxgeo1', 'E[X(X-1)] is below E[X]^2-E[X], so the batch moments describe no random variable');
end

lambda = a * batchMean;
if lambda >= s
    line_error('qsys_geoxgeo1', 'load lambda/s must be strictly less than 1');
end
