...
markovianTypes = [ProcessType.EXP, ProcessType.ERLANG, ProcessType.HYPEREXP, ...
                  ...
                  ProcessType.DMAP, ProcessType.MMAP, ...
...
if isfield(sn, 'nodeparam') && ~isempty(sn.nodeparam)
    for ind = 1:sn.nnodes
        ...
        nmodes_t = nparam.nmodes;
        for m = 1:nmodes_t
            % Markovian distributions (Exp/Erlang/HyperExp/PH/APH/...) are
            % populated with a valid (D0,D1) PH and a finite firingphases by
            % refreshPetriNetNodes. Skip them here.
            phases_m = NaN;
            if length(nparam.firingphases) >= m
                phases_m = nparam.firingphases(m);
            end
            if ~isnan(phases_m) && phases_m > 0
                continue;
            end

            % Resolve the original distribution's process-type id and target mean.
            procidT = nparam.firingprocid(m);
            if isnan(procidT)
                continue;
            end
            if any(procidT == markovianTypes)
                continue;
            end

            % Pull the user-supplied parameters from firingproc{m}.
            origProc = nparam.firingproc{m};
            if ~iscell(origProc) || isempty(origProc)
                continue;
            end

            distName = ProcessType.toText(procidT);
            line_warning(mfilename, ...
                'Firing distribution %s at Transition node %d mode %d is non-Markovian and will be converted to PH (%d phases).\n', ...
                distName, ind, m, nPhases);

            switch procidT
                case ProcessType.GAMMA
                    shape = origProc{1}; scale = origProc{2};
                    targetMean_t = shape * scale;
                    pdf_func = @(x) gampdf(x, shape, scale);
                case ProcessType.WEIBULL
                    % Stored order matches MATLAB Weibull.getProcess: {r, alpha}
                    rWb = origProc{1}; alphaWb = origProc{2};
                    targetMean_t = alphaWb * gamma(1 + 1/rWb);
                    pdf_func = @(x) wblpdf(x, alphaWb, rWb);
                case ProcessType.LOGNORMAL
                    muL = origProc{1}; sigmaL = origProc{2};
                    targetMean_t = exp(muL + sigmaL^2/2);
                    pdf_func = @(x) lognpdf(x, muL, sigmaL);
                case ProcessType.PARETO
                    alphaP = origProc{1}; kP = origProc{2};
                    if alphaP > 1
                        targetMean_t = alphaP * kP / (alphaP - 1);
                    else
                        targetMean_t = NaN;
                    end
                    pdf_func = @(x) (x >= kP) .* alphaP .* kP.^alphaP ./ x.^(alphaP + 1);
                case ProcessType.UNIFORM
                    minV = origProc{1}; maxV = origProc{2};
                    targetMean_t = (minV + maxV) / 2;
                    pdf_func = @(x) (x >= minV & x <= maxV) / (maxV - minV);
                case ProcessType.DET
                    targetMean_t = origProc{1};
                    if preserveDet
                        continue;
                    end
                    MAP = map_erlang(targetMean_t, nPhases);
                    sn = updateNodeparamForMAP(sn, ind, m, MAP);
                    continue;
                otherwise
                    continue;
            end

            if ~isfinite(targetMean_t) || targetMean_t <= 0
                MAP = map_erlang(1.0, nPhases);
            else
                try
                    MAP = map_bernstein(pdf_func, nPhases);
                    MAP = map_scale(MAP, targetMean_t);
                catch
                    MAP = map_erlang(targetMean_t, nPhases);
                end
            end
...
function sn = updateNodeparamForMAP(sn, ind, m, MAP)
% UPDATENODEPARAMFORMAP Update Transition mode firing process to a
% phase-type representation. Mirrors updateSnForMAP for stations.
nparam = sn.nodeparam{ind};
actualPhases = size(MAP{1}, 1);

nparam.firingproc{m} = MAP;
nparam.firingphases(m) = actualPhases;
nparam.firingpie{m} = map_pie(MAP);
nparam.firingprocid(m) = ProcessType.MAP;

sn.nodeparam{ind} = nparam;
end
...
sn.proc{ist}{r} = MAP;
% The conversion methods (map_bernstein, map_erlang) always produce a renewal
...
% Expand the server-phase columns of any pre-initialized state (renewal PH:
% no local-variable column is added, skip for Sources which have no local state)
...
% EXPANDSTATEFORMAP Expand state vector for renewal-PH conversion
%
% When converting a non-Markovian distribution to a renewal PH, the server
% portion (space_srv) of any pre-initialized state needs additional columns
% for the extra phases. No local-variable column is needed (renewal PH has
% no phase memory across jobs).
%
...
% Calculate state vector structure (nvars is unchanged by the conversion)
V_old = sum(sn.nvars(ind, :));
...
% Reconstruct state (space_var unchanged: renewal PH adds no local variable)
sn.state{isf} = [space_buf, space_srv_new, space_var];
...
if isfield(sn, 'space') && ~isempty(sn.space) && ~isempty(sn.space{isf})
    ...
    sn.space{isf} = [space_buf_s, space_srv_s_new, space_var_s];
end
end
