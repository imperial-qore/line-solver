...
%     E[T] = E[W] + d
%     E[A] = d*(1/2 + 1/(2*(1-rho)) + ((1-rho)/rho)*exp(rho))  (exact)
%
% Reference:
...
% Mean AoI for M/D/1 FCFS (exact, Inoue et al. 2019)
% E[A] = d * (1/2 + 1/(2*(1-rho)) + ((1-rho)/rho) * exp(rho))
% accounts for the negative correlation between interarrival and waiting
% times (validated against simulation)
meanAoI = d * (0.5 + 1 / (2 * (1 - rho)) + ((1 - rho) / rho) * exp(rho));

% Mean Peak AoI
% E[Apeak] = E[T] + E[Y]
peakAoI = E_T + E_Y;

% see _kb/03-api-layer.md (AoI family) -- variance here is an approximation, not exact
