...
% Mean Peak AoI (exact)
% Deliveries occur at rate lambda*P(S < Y) = lambda*exp(-lambda*d);
% the successful service time is d (deterministic), so
% E[Apeak] = d + exp(lambda*d)/lambda (validated by DES)
peakAoI = d + exp(lambda * d) / lambda;
