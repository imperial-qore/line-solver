...
% Mean AoI for D/M/1 FCFS (exact)
% With deterministic interarrivals Y = tau the interarrival/system-time
% correlation term vanishes, so E[A] = E[Y^2]/(2*E[Y]) + E[D]
% = tau/2 + 1/(mu*(1-sigma))  (validated against simulation)
meanAoI = tau / 2 + E_D;
