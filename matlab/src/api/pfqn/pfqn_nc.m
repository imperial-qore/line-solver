...
switch options.method
    ...
    case {'comom'}
        if R>1
            % see _kb/03-api-layer.md (pfqn_nc dispatch notes) for rationale
            if M>1
                line_error(mfilename,'The ''comom'' method supports a single queueing station, but this model has %d. Use ''default'' or ''ca'' for an exact normalizing constant, or SolverJMT with method ''jmva.comom''.', M);
            end
            try
