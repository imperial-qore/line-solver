...
%{
 % @file pfqn_lap.m
 % @brief Laplace approximation for normalizing constant.
%}
%}
...
if ~isvector(L) || numel(L)~=numel(N)
    line_error(mfilename,'pfqn_lap expects per-class vectors for a single queueing station (repairman models): L, N, Z must be 1xR.');
end
L = L(:)'; N = N(:)'; Z = Z(:)';
Ntot = sum(N);
