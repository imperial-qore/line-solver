            ...
            elseif fcfsSchmidt && type(i) == SchedStrategy.FCFS
                ...
                if numServers > 1
                    ...
                    for j = 1:(numServers-1)
                        if mp_iskey(marginalProbs, j)
                            prob_j = mp_get(marginalProbs, j);
                        else
                            prob_j = 0;
                        end
...
% Integer-keyed map stored as parallel arrays (keys/vals are numeric rows);
% keys may be negative, so a plain index array is not usable.
marginalProbs = struct('keys', [], 'vals', []);

if strcmp(marginalProbMethod, 'scat')
    floorVal = floor(avgJobs);
    ceilVal = floorVal + 1;
    marginalProbs = mp_set(marginalProbs, floorVal, ceilVal - avgJobs);
    marginalProbs = mp_set(marginalProbs, ceilVal, avgJobs - floorVal);
    return
end
...
for j = 0:maxVal
    if j <= floorVal
        ...
        marginalProbs = mp_set(marginalProbs, j, prob);
    else
        uDist = j - ceiling;
        if uDist > 25
            marginalProbs = mp_set(marginalProbs, j, 0);
        elseif j > population(classIdx) - 1 && uDist < 25
            if mp_iskey(marginalProbs, population(classIdx) - 1)
                existingProb = mp_get(marginalProbs, population(classIdx) - 1);
            else
                existingProb = 0;
            end
            if mp_iskey(marginalProbs, floorVal - uDist)
                mp_floor_udist = mp_get(marginalProbs, floorVal - uDist);
            else
                mp_floor_udist = 0;
            end
            newProb = existingProb + (w(floorVal+1, uDist+1) - mp_floor_udist);
            marginalProbs = mp_set(marginalProbs, population(classIdx) - 1, newProb);
        else
            if mp_iskey(marginalProbs, floorVal - uDist)
                mp_floor_udist = mp_get(marginalProbs, floorVal - uDist);
            else
            ...
            marginalProbs = mp_set(marginalProbs, j, newProb);
        end
    end
end
...
function tf = mp_iskey(m, k)
% True if integer key k is present in the parallel-array map m.
tf = any(m.keys == k);
end

function v = mp_get(m, k)
% Value stored at key k (assumes the key exists).
i = find(m.keys == k, 1);
v = m.vals(i);
end

function m = mp_set(m, k, v)
% Insert or overwrite the value at key k.
i = find(m.keys == k, 1);
if isempty(i)
    m.keys(end+1) = k;
    m.vals(end+1) = v;
