...
%}
%}
function [QN,UN,WN,CN,XN,totiter] = pfqn_linearizermx(lambda,L,N,Z,nservers,type,tol,maxiter,method,QN0)
...
if nargin<10 || isempty(QN0)
    QN0c = [];
elseif size(QN0,1)==M && size(QN0,2)==length(N)
    QN0c = QN0(:,closedClasses);
elseif size(QN0,1)==M && size(QN0,2)==length(closedClasses)
    QN0c = QN0;
else
    QN0c = [];
end
if max(nservers(:))==1
    switch method
        case {'default','lin'}
            [QNc,UNc,WNc,CNc,XNc,totiter] = pfqn_linearizer(Dc,N(closedClasses),Z(closedClasses),type,tol,maxiter,QN0c);
        case 'gflin'
            linAlpha = 2.0;
            [QNc,UNc,WNc,CNc,XNc,totiter] = pfqn_gflinearizer(Dc,N(closedClasses),Z(closedClasses),type,tol,maxiter,linAlpha,QN0c);
        case 'egflin'
            alphaM = zeros(1,R);
            ...
            [QNc,UNc,WNc,CNc,XNc,totiter] = pfqn_egflinearizer(Dc,N(closedClasses),Z(closedClasses),type,tol,maxiter,alphaM,QN0c);
        otherwise
            [QNc,UNc,WNc,CNc,XNc,totiter] = pfqn_linearizer(Dc,N(closedClasses),Z(closedClasses),type,tol,maxiter,QN0c);
    end
else
    [QNc,UNc,WNc,CNc,XNc,totiter] = pfqn_linearizerms(Dc,N(closedClasses),Z(closedClasses),nservers,type,tol,maxiter,QN0c);
end
