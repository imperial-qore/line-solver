...
%{
 ...
 % @return UN Utilization (M x R), per station-class, D*X/nservers.
...
% Utilization Law: a c-server station holds D*X/c of its capacity. XN has been
% replicated over the stations above, so read the per-class throughput from its
% first row -- linear indexing XN(c) into the (M x R) replica would silently
% return the WRONG class's throughput whenever M > 1.
Xc = XN(1,:);
for m = 1:M
    for c = 1:C
        if size(S(m,:)) == 1
        ...
        UN(m,closedClasses(c)) = D(m,closedClasses(c)) * Xc(closedClasses(c)) / ns;
    end
end
% Response time
