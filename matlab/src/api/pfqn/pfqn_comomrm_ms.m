%{
%{
 % @file pfqn_comomrm_ms.m
 % @brief CoMoM for multiserver repairman model.
%}
%}
...
%{
 % @brief CoMoM for multiserver repairman model.
 % @fn pfqn_comomrm_ms(L, N, Z, m, S)
 % @param L Service demand matrix.
 % @param N Population vector.
 % @param Z Think time vector.
 % @param m Replication factor (default: 1).
 % @param S Number of servers at queueing stations.
...
if nargin<5
    S=1; % single server per queueing station, as the docstring advertises
end
atol = GlobalConstants.FineTol;
[~,L,N,Z,lG0] = pfqn_nc_sanitize(zeros(1,R),L,N,Z,atol);
% R must be re-read here: sanitize DROPS zero-population and zero-demand
% classes, so the pre-sanitize R above overruns the shortened L, N and Z in
% the loop below. Reachable on ordinary input, since a zero-population class
