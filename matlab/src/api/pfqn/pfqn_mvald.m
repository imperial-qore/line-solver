...
%UN = repmat(XN,M,1) .* L;
UN = 1-pi(:,1);
% Cycle time exclusive of think time. An empty class has zero throughput, so
% N./XN would be 0/0: report no cycle time for it rather than NaN, as PFQN_DAC
% and PFQN_MVACLD do.
CN = zeros(1,R);
CN(N > 0) = N(N > 0)./XN(N > 0) - Z(N > 0);
end
