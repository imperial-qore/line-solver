...
for it=1:maxiter
    ...
    for r=1:R
        if N(r) == 0
            % Empty class: it contributes no jobs anywhere. Without this the
            % Schweitzer term below evaluates QN*(N(r)-1)/N(r) = 0*(-Inf) = NaN,
            % which then propagates to every other class through the s~=r term,
            % so a single empty class returns an all-NaN solution.
            XN(r) = 0;
            CN(:,r) = 0;
            QN(:,r) = 0;
            UN(:,r) = 0;
...
RN = QN ./ repmat(XN,M,1);
RN(:,N==0) = 0;   % 0/0 for an empty class; its residence time is 0, not NaN
end
