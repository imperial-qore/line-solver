...
[EC,E,Eprime] = pfqn_ldmx_ec(lambda,D,mu);
...
while nvec>=0
    ...
    % Accumulate the closed-conditional log normalizing constant along the
    % canonical lattice path (fill class 1, then class 2, ...). On that path
    % G(nvec)/G(nvec-e_c) = 1/x(c,nvec), so lG += log x(c,nvec). The reference
    % throughput x(c,hnvec) is the one just computed for the current nvec.
