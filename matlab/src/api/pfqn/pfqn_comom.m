...
% atol has no other default in this family; 1e-14 matches the sibling
% pfqn_procomom. Without this the 3-argument form advertised in the header
% died on an undefined atol at the Lmax rescaling below.
if nargin<4
    atol=1e-14;
end
[M,R]=size(L);
if M>1
    line_error(mfilename,'pfqn_comom supports at most one queueing station (repairman models with a delay); use pfqn_ca or pfqn_recal for M>1.');
end
% rescale demands
Lmax = L; % use L
