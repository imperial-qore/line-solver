%{
%{
 % @file pfqn_comomrm_orig.m
 % @brief Original CoMoM implementation for finite repairman model.
%}
%}
...
%{
 % @brief Original CoMoM implementation for finite repairman model.
 % @fn pfqn_comomrm_orig(L, N, Z, atol)
...
% m is read below (A(1+s,1+s) = -m*L(1,s)) on every path, but was assigned only
% under nargin<4, so supplying atol raised an undefined-variable error for R>1.
m=1;
lambda = 0*N;
...
L=L(:,rsort);
Z=Z(:,rsort);
% see _kb/03-api-layer.md (pfqn_nc_sanitize sort-order dependency) for rationale
N=N(:,rsort);
Lmax=Lmax(:,rsort);
% initialize
nvec=zeros(1,R);
