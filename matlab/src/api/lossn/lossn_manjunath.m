...
%{
 ...
 % Integer Spaces.
%}
function [QLen, Loss, lG, niter] = lossn_manjunath(nu, A, C)
nu = nu(:)';
C  = C(:);
R = numel(nu);
J = numel(C);
...
% f_r(n) = nu_r^n / n!. Each sequence is built in the LOG domain and shifted by
% its own maximum before exponentiating, so its largest entry is exactly 1; the
% scale cancels in every ratio below and is added back in the log of the
% normalization constant. Shifting before the exponential rather than after is
% what keeps a heavy route in range: nu^n/n! peaks near n = nu at roughly
% exp(nu)/sqrt(2 pi nu), so forming the terms first and dividing by their
% maximum afterwards overflows to Inf for a load above about 700 -- and Inf is
% not finite, so the rescaling step then declined to run and the Inf reached
% G, returning NaN for every metric. Measured at nu = C = 900.
nmax = zeros(1, R);
f = cell(1, R);
logscale = 0;
for r = 1:R
...
G = lossn_manjunath_series(f, A, C, nmax);
if G <= 0
...
for r = 1:R
    ...
    Gr = lossn_manjunath_series(f, A, Cr, nmax);
    ratio = Gr / G;
    QLen(r) = nu(r) * ratio;
    Loss(r) = 1 - ratio;
end
...
function G = lossn_manjunath_series(f, A, C, nmaxFull)
...
for r = 1:R
    for j = find(first == r)
        Aser = lossn_manjunath_expand(Aser, curdim, j, C(j) + 1);
        curdim(j) = C(j) + 1;
    end
    ...
    if any(s ~= 0)
        ...
        [SUB, stride] = lossn_manjunath_index(curdim);
    ...
    for j = find(last == r)
        Aser = lossn_manjunath_extract(Aser, curdim, j);
        curdim(j) = 1;
    end
end
...
function A = lossn_manjunath_expand(A, curdim, j, newdim)
...
function A = lossn_manjunath_extract(A, curdim, j)
...
function [SUB, stride] = lossn_manjunath_index(curdim)
q = numel(curdim);
P = prod(curdim);
SUB = zeros(P, q);
rep = 1;
