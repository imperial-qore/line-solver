...
% ---------------------------------------------------------------
rho = 8; Ccap = 10;
[QLen, Loss, lG] = lossn_ms(rho, 1, Ccap);
exactB = erlangB_ref(rho, Ccap);
lGref = log(sum(exp((0:Ccap)*log(rho) - gammaln((0:Ccap)+1))));
fprintf('\n[T1] M/M/C/C Erlang-B  rho=%g C=%d\n', rho, Ccap);
fprintf('  blocking analytic=%.12f  lossn_ms=%.12f\n', exactB, Loss);
fprintf('  lG analytic=%.12f  lossn_ms=%.12f\n', lGref, lG);
...
% ---------------------------------------------------------------
nu = [2.0, 1.0];
[~, L1] = lossn_ms(nu, [2 4], 9);
[~, L2] = lossn_ms(nu, [1 2], 4);      % 9/2 floors to 4
...
Cv = 6;
[QLen, Loss, lG] = lossn_ms(nu, A, Cv);
[~, L2c] = lossn_ms(nu(1:2), A(1,1:2), Cv);
...
fprintf('\n=== test_lossn_ms: %d passed, %d failed ===\n', npass, nfail);
if nfail > 0
    error('test_lossn_ms: %d assertion(s) failed', nfail);
end
end
