...
for ind=1:sn.nnodes
    if sn.isstateful(ind)
        ...
        % Skip non-station stateful nodes like Router
        if isnan(ist)
            continue
        end
        state_i = [];
        ...
        switch sn.sched(ist)
            ...
            case {SchedStrategy.FCFS, SchedStrategy.SIRO, SchedStrategy.PS, SchedStrategy.INF, SchedStrategy.DPS, SchedStrategy.GPS, SchedStrategy.HOL, SchedStrategy.LCFS, SchedStrategy.LCFSPR}
                for r=1:size(kir_i,2)
                    for k=1:length(sn.mu{ist}{r})
                        if k==1
