...
try
    ...
    % Validate result: singular Q22 produces NaN/Inf in W_red
    if any(~isfinite(W_red(:)))
        warning('LINE:Fluid:SingularElimination', ...
            'Stochastic complementation produced non-finite values. Using original system.');
        W_red = W;
        state_map = 1:size(W, 1);
        return;
    end
