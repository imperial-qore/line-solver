...
while max(abs(1-eta./eta_1)) > options.iter_tol & it < options.iter_max
    ...
    if isempty(Xchain)
        ...
        Qchain = zeros(M,C);
        for r=closedChains
            % an approximate method may report out-of-domain via an empty lG;
            % return gracefully, do not assign [] into a scalar slot. see
            % _kb/06-solver-catalog.md (NC section, unknown-method/empty-lG)
            lGr_r = pfqn_nc(lambda,Lms,oner(Nchain,r),sum(Z,1)+sum(Zms,1), options);
            if isempty(lGr_r)
                [Q,U,R,T,C,X,lG,STeff,it,method] = deal([],[],[],[],[],[],[],[],1,options.method);
                return
            end
            ...
            for ist=1:M
                if Lchain(ist,r)>0
                    if isinf(nservers(ist)) % infinite server
                        ...
                        % method here
                        [lGar_ist_r,~,~,method] = pfqn_nc([lambda,0],[Lms(setdiff(1:size(Lms,1),ist),:),zeros(size(Lms,1)-1,1); Lms(ist,:),1], [oner(Nchain,r),1], [sum(Z,1)+sum(Zms,1),0], options);
                        if isempty(lGar_ist_r)
                            [Q,U,R,T,C,X,lG,STeff,it,method] = deal([],[],[],[],[],[],[],[],1,options.method);
                            return
                        end
                        lGar(ist,r) = lGar_ist_r;
