...
for m=1:nreplicas
    ...
    serverStation{m}.setImmediateFeedback(true);
end
                        ...
                        if ishostlayer
                            if ~iscachelayer
                                for m=1:nreplicas
                                    ...
                                    if isfunctionlayer
                                        % see _kb/06-solver-catalog.md (LN section) for rationale
                                        setupDist = lqn.setuptime{lqn.parent(nextaidx)};
                                        if ~isempty(setupDist) && ~isa(setupDist, 'Immediate') ...
                                                && setupDist.getMean() > GlobalConstants.FineTol
                                            serverStation{m}.setDelayOff(aidxClass{nextaidx}, setupDist, lqn.delayofftime{lqn.parent(nextaidx)});
                                        end
                                    end
                                end
                                jobPos = atServer;
                                ...
                                if ~isempty(aidxThinkClass{nextaidx})
                                    for m=1:nreplicas
                                        P{curClass, aidxThinkClass{nextaidx}}(serverStation{m}, clientDelay) = 1.0;
                                    end
