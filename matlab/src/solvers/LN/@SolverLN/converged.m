...
%% Stochastic iteration dispatch
% see _kb/06-solver-catalog.md (LN section) for the rationale
if ~isempty(self.stochiter_mode)
    if self.stochiter_auto && strcmp(self.stochiter_mode,'off') && it >= 1 && any(self.stochlayers)
        ...
        line_debug('LN: stochastic layer method detected at runtime, switching to Robbins-Monro iteration');
    end
    if strcmp(self.stochiter_mode,'rm')
        bool = convergedStoch(self, it);
        return
    end
end
