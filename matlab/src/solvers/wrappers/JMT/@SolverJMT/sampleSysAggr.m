...
if strcmp(self.getOptions.lang,'java')
    % Log-based sampling relies on per-node CSV logs written and parsed by the
...
logData = SolverJMT.parseLogs(modelCopy, isNodeLogged, MetricType.toText(MetricType.QLen));
...
% Flatten the 2D `event` cell into a column cell of Event objects, sorted by
% time. Robust to the fact that some `event{i,j}` slots may be `[]` (numeric)
% rather than `{}` (cell) when nclasses>1, due to implicit cell-array growth.
flatEvents = {};
for ii=1:size(event,1)
    for jj=1:size(event,2)
        cellij = event{ii,jj};
        if iscell(cellij) && ~isempty(cellij)
            for kk=1:numel(cellij)
                if isa(cellij{kk}, 'Event')
                    flatEvents{end+1,1} = cellij{kk};
                end
            end
        end
    end
end
if isempty(flatEvents)
    sysStateAggr.event = cell(0,1);
else
    event_t = cellfun(@(c) c.t, flatEvents);
    [~,I] = sort(event_t);
    sysStateAggr.event = flatEvents(I);
end
