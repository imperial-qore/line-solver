...
% FCR-only metrics: weighted occupation ('FCR Capacity') and memory occupation
% ('FCR Memory'). Populated at FCR rows only; NaN elsewhere.
result.Avg.Weight = NaN(totalRows, sn.nclasses);
result.Avg.MemOcc = NaN(totalRows, sn.nclasses);
...
if sn.nregions > 0
    ...
    result.Avg.A(fcrRowStart:fcrRowEnd, :) = NaN;  % JMT reports no region ArvR
end
    ...
    if isFCRMetric && ~isempty(fcrStationName) && startsWith(fcrStationName, 'FCRegion')
        ...
        if ~isnan(fcrIndex) && fcrIndex >= 1 && fcrIndex <= sn.nregions
            ...
            switch metric.measureType
                ...
                case 'FCR Capacity'   % JMT weighted occupation (Total Weight)
                    for r = 1:sn.nclasses
                        result.Avg.Weight(fcrRowIdx, r) = metric.meanValue / sn.nclasses;
                    end
                case 'FCR Memory'     % JMT memory occupation
                    for r = 1:sn.nclasses
                        result.Avg.MemOcc(fcrRowIdx, r) = metric.meanValue / sn.nclasses;
                    end
            end
        end
...
% Region loss metrics for getAvgRegionLossTable. JMT exposes no region drop
% measure and its region Throughput is the carried (admitted) rate, so the
% offered rate is reconstructed by flow balance: the rate at which external
% stations route jobs across the region boundary (unaffected by the drop).
% DropRateNfcr = offered - carried, clamped at 0, and zeroed for non-DROP
% (blocking/WAITQ) regions. Mirrors the LDES result.FCR.* convention so the
% table is solver-agnostic.
if sn.nregions > 0
    M = sn.nstations; K = sn.nclasses;
    fcrRows = (M + 1):(M + sn.nregions);
    TNfcr = result.Avg.T(fcrRows, :);
    TNstat = result.Avg.T(1:M, :);
    rt = sn.rt;  % (M*K) x (M*K), station-major (station-1)*K + class
    offered = zeros(sn.nregions, K);
    for f = 1:sn.nregions
        mask = sn_region_members(sn, f, sn.region{f}, []);
        member = find(mask);
        for r = 1:K
            acc = 0;
            for ii = member
                for j = 1:M
                    if any(member == j), continue; end
                    for rp = 1:K
                        w = rt((j-1)*K + rp, (ii-1)*K + r);
                        if w ~= 0
                            acc = acc + TNstat(j, rp) * w;
                        end
                    end
                end
            end
    ...
    DropRateNfcr = max(0, offered - TNfcr);
    if isfield(sn, 'regionrule') && ~isempty(sn.regionrule) ...
            && isequal(size(sn.regionrule), size(DropRateNfcr))
        DropRateNfcr(sn.regionrule ~= DropStrategy.DROP) = 0;  % only DROP loses
    end
    result.FCR.TNfcr = TNfcr;
    result.FCR.DropRateNfcr = DropRateNfcr;
end
