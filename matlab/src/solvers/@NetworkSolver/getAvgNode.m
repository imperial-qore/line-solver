...
% Resync each Cache node's hit/miss/delayed-hit split from the just-run solver's
% node-server result into the local struct before reconstructing node
% throughputs. see _kb/09-ldes-and-cache.md for rationale
for ind = 1:sn.nnodes
    if sn.nodetype(ind) == NodeType.Cache
        node = self.model.nodes{ind};
        if isprop(node.server, 'actualHitProb') && ~isempty(node.server.actualHitProb)
            sn.nodeparam{ind}.actualhitprob  = full(node.server.actualHitProb);
            sn.nodeparam{ind}.actualmissprob = full(node.server.actualMissProb);
        end
        if isprop(node.server, 'actualDelayedHitProb') && ~isempty(node.server.actualDelayedHitProb)
            sn.nodeparam{ind}.actualdelayedhitprob = full(node.server.actualDelayedHitProb);
        end
    end
end
