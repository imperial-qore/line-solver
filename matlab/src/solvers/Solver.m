classdef Solver < handle
    ...
    methods
        ...
        function bool = isStochastic(self)
            % BOOL = ISSTOCHASTIC()
            % True if the solver, with its currently configured method,
            % returns stochastic estimates (i.e., results depend on the
            % random seed, as in simulation or Monte Carlo integration).
            % A solver run with method 'default' may resolve the actual
            % method only at runtime; once results are available, the
            % classification therefore uses the method recorded in
            % result.Avg.method (e.g. 'default/imci' when the NC default
            % path resolved to Monte Carlo integration).
            method = self.options.method;
            if ~isempty(self.result) && isfield(self.result,'Avg') && ...
                    isfield(self.result.Avg,'method') && ~isempty(self.result.Avg.method) && ...
                    ischar(self.result.Avg.method)
                method = self.result.Avg.method;
            end
            bool = self.isStochasticMethod(method);
        end

        function bool = isStochasticMethod(self, method) %#ok<INUSD>
            % BOOL = ISSTOCHASTICMETHOD(METHOD)
            % Classify a (possibly runtime-resolved) method name of this
            % solver as stochastic. Deterministic by default; subclasses
            % with simulation-based or sampling-based methods override this.
            bool = false;
        end
    ...
    methods (Static)
        ...
        function [optList, allOpt] = listValidOptions()
            ...
            allOpt = {'cache','cutoff','force','init_sol','iter_max','iter_tol','lang','tol', ...
                ...
                'pbh.upper','pbh.lower','pbk.upper','pbk.lower','bjbk.upper','bjbk.lower','cub.upper','cub.lower'};
        end
                        ...
                        switch varargin{1}
                            ...
                            otherwise
                                switch varargin{1}
                                    ...
                                    case 'config.warmupfrac'
                                        options.config.warmupfrac = varargin{2};
                                    case 'config.highvar'
