    ...
    if isNCS(ind)
        ...
        switch sn.nodetype(ind)
            case {NodeType.Source, NodeType.Delay, NodeType.Queue}
                ...
                if R>1
                    % Order-preserving bounded superposition: superpose the
                    % per-class departures in class order, compressing the phase
                    % dimension back to config.space_max whenever it is exceeded,
                    % so the raw Kronecker product cannot exhaust memory (cf. the
                    % FJ variant solver_mam_traffic_mmap). The arrival is compressed
                    % to config.space_max downstream (solver_mam) regardless.
                    DEP{inc} = MMAP{ist,1};
                    for rr=2:R
                        DEP{inc} = mmap_super(DEP{inc}, MMAP{ist,rr});
                        if length(DEP{inc}{1}) > config.space_max
                            DEP{inc} = mmap_compress(DEP{inc}, config);
                        end
                    end
                else
                    DEP{inc} = MMAP{ist,1};
