...
for r = 1:R
    ...
    % FJ effective service = max of K servers; exponential case uses the
    % harmonic-sum approximation E[R] = sum_k 1/(mu*k)
    ...
    if ~isnan(sn.nodeToStation(joinIdx))
        ...
        % Join contribution = total FJ RT minus mean individual queue RT
        % (FJ_codes returns fork-to-join total including sync delay)
