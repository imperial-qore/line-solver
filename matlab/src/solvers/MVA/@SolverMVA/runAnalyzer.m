...
switch options.lang
    ...
    case 'matlab'
        ...
        while (forkLoop && forkIter < options.iter_max)
            ...
            elseif sn.nclasses>1 && sn.nclosedjobs == 0 && length(sn.nodetype)==3 && all(sort(sn.nodetype)' == sort([NodeType.Source,NodeType.Queue,NodeType.Sink])) && sn.schedid(find(sn.nodetype==NodeType.Queue)) == SchedStrategy.ID_POLLING % is an open polling system
                    ...
                    switch options.config.fork_join
                        case {'mmt', 'default'}
                            ...
                            for s=forkauxclasses(:)'
                                ...
                                ri = mva_fj_find_paths(sn, nonfjmodel.getLinkedRoutingMatrix{r,r}, f, joinIdx, r, [r,s], QN, TN, 0, fjclassmap, fjforkmap, nonfjmodel);
                                lambdai = 1./ri;
                                d0 = 0;
