function [Q,U,R,T,C,X,lG,runtime,totiter,actualmethod] = solver_mva_polling_analyzer(sn, options)
    ...
    case {'default','stationtime'}
        ...
        switch max(polling_type,[], 'omitnan') % we assume polling types to be identical
            ...
            case PollingType.DECREMENTING
                W = polling_qsys_decrementing(sn.proc{source_ist},sn.proc{queue_ist},switchover);
            otherwise
