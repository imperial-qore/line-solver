...
switch options.method
    case {'lin', 'qdlin'}
        if isempty(nnzclasses) || isempty(ccl)
            % gamma is populated only for closed classes, so with none the
            % Linearizer correction is identically zero and the lld term
            % degenerates to the uncorrected one.
            lldterm = pfqn_lldfun(1 + interpTotArvlQlen, lldscaling);
        else
            lldterm = ones(M,K);
            Ncl = Nchain_in(ccl); Ncl = Ncl(:); % |ccl| x 1
            for r=nnzclasses
                % Linearizer fraction correction seen by an arriving class-r
                % job, evaluated at every station: entry k uses gamma(r,k,:).
                gcorr = reshape(gamma(r,:,ccl), M, numel(ccl)) * Ncl - reshape(gamma(r,:,r), M, 1);
        ...
        for r=nnzclasses
            if ~isempty(cdscaling)
                if isfinite(Nchain_in(r))
                    ...
                    cdterm(:,r) = pfqn_cdfun(1 + selfArvlQlenSeenByClosed + gself, cdscaling, r); % qd-amva class-dependence term beta_{i,r}
                else
                    % gamma is zero for open classes, so the correction vanishes
                    % (the (Nchain-1) factor would otherwise be Inf*0 = NaN)
                    cdterm(:,r) = pfqn_cdfun(1 + stationaryQlen, cdscaling, r); % qd-amva class-dependence term beta_{i,r}
                end
            end
        end
    otherwise
        for r=nnzclasses
            if ~isempty(cdscaling)
                if isfinite(Nchain_in(r))
                    cdterm(:,r) = pfqn_cdfun(1 + selfArvlQlenSeenByClosed, cdscaling, r); % qd-amva class-dependence term beta_{i,r}
                else
                    cdterm(:,r) = pfqn_cdfun(1 + stationaryQlen, cdscaling, r); % qd-amva class-dependence term beta_{i,r}
                end
            end
...
for r=nnzclasses
    for k=1:M
        STeff(k,r) = STchain_in(k,r) * lldterm(k,r) * msterm(k) * cdterm(k,r) * jdterm(k,r);
    end
end
