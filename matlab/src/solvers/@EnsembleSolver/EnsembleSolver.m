classdef EnsembleSolver < Solver
    ...
    methods
        ...
        function bool = isStochastic(self)
            % BOOL = ISSTOCHASTIC()
            % An ensemble solver is stochastic if any of its submodel
            % solvers is stochastic. Each submodel solver classifies
            % itself, including from the method it resolved at runtime.
            bool = false;
            for e = 1:length(self.solvers)
                if ~isempty(self.solvers{e}) && self.solvers{e}.isStochastic()
                    bool = true;
                    return
                end
            end
        ...
        % getAvgTable shorthand family (resolved on the concrete ensemble
        % solver: SolverLN/SolverENV/UQ), matching NetworkSolver.
        function varargout = avgT(self, varargin)
            % AVGT Alias for getAvgTable
            [varargout{1:nargout}] = self.getAvgTable(varargin{:});
        end
