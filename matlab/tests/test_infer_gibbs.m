...
tput_samples = ones(n,1) / mean(diff(dep_times));
...
assert(abs(estVal - trueDemand)/trueDemand < 0.10, ...
    sprintf('Gibbs: estimated %.4f, relative error %.2f%% exceeds 10%%', estVal, 100*abs(estVal - trueDemand)/trueDemand));
