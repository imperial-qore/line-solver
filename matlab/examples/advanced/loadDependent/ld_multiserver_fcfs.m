...
node{1}.setService(jobclass{1}, Exp.fitMean(1.0)); % mean = 1
node{2}.setService(jobclass{1}, Exp.fitMean(1.5)); % mean = 1.5
node{2}.setClassDependence(@(ni) min(sum(ni),c), c); % ni is a vector where ni(r) is the number of jobs in class r at station i; peak rate scaling = c (Util = T*S/c)
...
% silently solving it unscaled (see SolverJMT.getFeatureSet). The lld
