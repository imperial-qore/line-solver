...
fprintf('Time horizon: [0, %g]\n', Tmax);
fprintf('Number of time points: %d\n', numel(t));

%% Plot Transient Rewards Converging to Steady-State
figure('Name', 'Transient Rewards', 'Position', [100 100 800 600]);

nRewards = numel(names);
for r = 1:nRewards
    subplot(2, 2, r);

    % Transient expected reward E[r(X(t))] starting from the empty state
    plot(Rt{r}.t, Rt{r}.metric, 'LineWidth', 1.5);
    hold on;
    ...
    legend('Transient E[r(X(t))]', 'Steady-state', 'Location', 'best');
    grid on;
end

sgtitle('Convergence of Transient Rewards to Steady-State');
