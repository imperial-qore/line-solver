...
%% Block 11: Posterior PDF (density) of the response time
...
figure('Name', 'Response Time: Posterior PDF', 'Position', [150, 150, 700, 400]);

edges = [respValues(1); (respValues(1:end-1) + respValues(2:end)) / 2; respValues(end)];
widths = diff(edges);
density = respProbs ./ widths;

bar(respValues, density, 'histc');
hold on;
xline(expectedR, 'k--', 'LineWidth', 2);
medianIdx = find(respCdf >= 0.5, 1, 'first');
if ~isempty(medianIdx)
    medianR = respValues(medianIdx);
    xline(medianR, 'r--', 'LineWidth', 2);
    legend({'f(R)', sprintf('mean = %.3f', expectedR), sprintf('median = %.3f', medianR)}, 'Location', 'best');
else
    legend({'f(R)', sprintf('mean = %.3f', expectedR)}, 'Location', 'best');
end
hold off;
xlabel('Response Time (R)');
ylabel('Posterior PDF f(R)');
title('Posterior PDF of Response Time');
grid on;
