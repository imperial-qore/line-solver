...
node{3} = Queue(model, 'Storage1', SchedStrategy.SIRO);
node{4} = Queue(model, 'Storage2', SchedStrategy.PS);
node{5} = Queue(model, 'Storage3', SchedStrategy.HOL);
node{6} = Sink(model, 'Sink');

%% Block 2: classes
jobclass{1} = OpenClass(model, 'Class1', 0);
jobclass{2} = OpenClass(model, 'Class2', 1); % lower priority (priority 1 < priority 0)
