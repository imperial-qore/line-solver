...
% client
A1 = Activity(model, 'A1', Exp.fitMean(0.1)).on(T1).boundTo(E1).synchCall(E2,1).synchCall(E3,1);
...
% third tier
A4 = Activity(model, 'A4', Exp.fitMean(0.15)).on(T3).boundTo(E4).repliesTo(E4);
...
t_default = toc(T0);
fprintf(1, '\nLN(default) Results [%.3f s]:\n', t_default);
disp(AvgTable{1});

lnoptions2 = lnoptions;
lnoptions2.method = 'srvn.ph';  % the method name; bare 'srvnph' is unrecognised and falls back to srvn.cs
T0 = tic;
solver{2} = LN(model, @(l)MVA(l,solveroptions), lnoptions2);
AvgTable{2} = solver{2}.getAvgTable;
t_srvnph = toc(T0);
fprintf(1, '\nLN(srvn.ph) Results [%.3f s]:\n', t_srvnph);
disp(AvgTable{2});

nclasses_default = sum(cellfun(@(m) m.getNumberOfClasses(), solver{1}.ensemble));
nclasses_srvnph = sum(cellfun(@(m) m.getNumberOfClasses(), solver{2}.ensemble));
fprintf(1, '\nLayer classes: default %d, srvn.ph %d. Runtime: %.3fs vs %.3fs (%.2fx).\n', ...
    nclasses_default, nclasses_srvnph, t_default, t_srvnph, t_default/t_srvnph);
