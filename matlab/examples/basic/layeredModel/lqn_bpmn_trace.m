...
options.verbose = 0;
solverLN = LN(model, @(model) MVA(model, options), lnoptions);

disp('=== Running LN solver with tracing ===');
solverLN.init();

for it = 1:5
    disp(['=== Iteration ' num2str(it) ' ===']);
    solverLN.pre(it);
    for e = 1:solverLN.nlayers
        [results{it,e}, ~] = solverLN.analyze(it, e);
        if ~isempty(results{it,e})
            maxTN = max(results{it,e}.TN(:));
            maxQN = max(results{it,e}.QN(:));
            if maxTN > 1e-10 || maxQN > 1e-10
                disp(['  Layer ' num2str(e) ': max(TN)=' num2str(maxTN, '%.4e') ', max(QN)=' num2str(maxQN, '%.4e')]);
            end
        end
    end
    solverLN.results = results;
    solverLN.post(it);
...
disp(['=== Final check: Getting AvgTable ===']);
AvgTable = solverLN.getAvgTable();
...
for i = 1:length(taskNames)
    ...
    if ~isempty(idx)
        disp([taskNames{i} ': Tput=' num2str(AvgTable.Tput(idx), '%.6f')]);
    end
end
