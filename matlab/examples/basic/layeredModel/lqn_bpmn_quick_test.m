...
for i = 1:length(taskNames)
    ...
    if ~isempty(idx)
        tput = AvgTable.Tput(idx);
        disp([taskNames{i} ': Tput=' num2str(tput, '%.6f')]);
    end
end

disp('');
disp('=== Full AvgTable (first 10 rows) ===');
disp(AvgTable(1:min(10,height(AvgTable)),:));
