...
public class SolverSSAMixedExamplesBenchmark {
    ...
    public static void main(String[] args) {
        ...
        options1.keep = true;
        options1.cutoff = 3;
        timeSolver(MixedModel.example_mixedModel_1(), "Mixed Example 1, PS", options1);

        SolverOptions options2 = new SolverOptions();
        options2.keep = false;
        options2.cutoff = 3;
        timeSolver(MixedModel.example_mixedModel_2(), "Mixed Example 2, PS", options2);

        SolverOptions options3 = new SolverOptions();
        options3.keep = false;
        options3.cutoff = 3;
        timeSolver(MixedModel.example_mixedModel_3(), "Mixed Example 3, FCFS", options3);

        SolverOptions options5 = new SolverOptions();
        options5.keep = false;
        options5.cutoff = 3;
        options5.samples = 20000;
        timeSolver(MixedModel.example_mixedModel_5(), "Mixed Example 5, PS",options5);
