...
public class  SolverCTMC extends NetworkSolver {
    ...
    public SolverCTMC(Network model, Object... varargin) {
        this(model, new SolverOptions(SolverType.CTMC));
...
//            }
    ...
    public ArrayList<SSAStateMatrix> getStateSpace() {
        if((((SolverCTMCResult)result).stateSpace) == null) {
            solver_ctmc();
        }
//        System.out.println("\nStateSpace =");
...
//        }
        return (((SolverCTMCResult)result).stateSpace);
    }

    public Matrix getGenerator() {
        if((((SolverCTMCResult)result).infGen) == null) {
            solver_ctmc();
        }
//        System.out.println("\nInfGen =");
...
//            System.out.println();
//        }
        return (((SolverCTMCResult)result).infGen);
    }

    public void getProbabilityVector() {
        if((((SolverCTMCResult)result).piVector) == null) {
            solver_ctmc_analyzer();
        }
        System.out.println("\nProbability Vector =");
        (((SolverCTMCResult)result).piVector).print();
    }
...
//        }
    ...
    public void solver_ctmc_analyzer() {
        ...
        (((SolverCTMCResult)result).piVector) = CTMC.ctmc_solve((((SolverCTMCResult)result).infGen));
        this.stateN = this.initialNetworkState.state.length;

        int nClasses = model.getNumberOfClasses();

        double[][] ssprobabilities = (((SolverCTMCResult)result).piVector).toArray2D();
        double [][][] arrRates = new double[stateN][nClasses][(((SolverCTMCResult)result).stateSpace).size()];
        ...
        // q length
        for (int i = 0; i< (((SolverCTMCResult)result).stateSpace).size(); i++){
            if(ssprobabilities[0][i]>0){
                SSAStateMatrix currState = (((SolverCTMCResult)result).stateSpace).get(i);
        ...
        for(EventData eventData : (((SolverCTMCResult)result).eventSpace)){
            ...
            if(!eventData.getValue1().getLeft().isDummy()) {
                ...
                double prob = (((SolverCTMCResult)result).piVector).get(indexMap.get(depState));
                    ...
                    case PS:
                        if(this.model.getLimitedLoadDependence().isEmpty() && (this.model.getLimitedClassDependence() == null || this.model.getLimitedClassDependence().isEmpty())) {
                            for(int k = 0; k < nClasses; k++) {
                                ...
                                for(int j = 0; j < (((SolverCTMCResult)result).stateSpace).size(); j++) {
                                    arrEstimate += (((SolverCTMCResult)result).piVector).get(j) * arrRates[i][k][j];
                                }
    ...
    public void solver_ctmc() {
        ...
        (((SolverCTMCResult)result).stateSpace) = stateSpace;
        // Compute event space
        ...
        eventSpace.remove(0);
        (((SolverCTMCResult)result).eventSpace) = eventSpace;
        ...
        (((SolverCTMCResult)result).infGen) = infGen;
    }
