...
public class NetworkAvgTputTable extends AvgTable {
    List<String> classNames;
    List<String> stationNames;

    public NetworkAvgTputTable(List<Double> Metricval) {
    ...
    public void print(SolverOptions options) {
        if (options.verbose != VerboseLevel.SILENT) {
            ...
            for (int i = 0; i < stationNames.size(); i++) {
                if ( getTput().get(i) > GlobalConstants.Zero) {
                    System.out.format(
                            "%-10s\t%-10s\t%-10s\n",
                            stationNames.get(i),
                            classNames.get(i),
    ...
    public NetworkAvgTputTable tget(Station station, JobClass jobclass) {
        return this.tget((String)station.getName(), (String)jobclass.getName());
    }

    public NetworkAvgTputTable tget(String stationname, String classname) {
        int rowIdx = this.stationNames.indexOf(stationname);
        int colIdx = this.classNames.indexOf(classname);
        if (rowIdx < 0 || colIdx <0) {
            NetworkAvgTputTable filteredAvgTable = new NetworkAvgTputTable(new ArrayList<>());
