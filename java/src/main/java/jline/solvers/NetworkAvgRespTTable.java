...
public class NetworkAvgRespTTable extends AvgTable {
    ...
    public NetworkAvgRespTTable tget(Station station, JobClass jobclass) {
        return this.tget((String)station.getName(), (String)jobclass.getName());
    }

    public NetworkAvgRespTTable tget(String stationname, String classname) {
        int rowIdx = this.stationNames.indexOf(stationname);
        int colIdx = this.classNames.indexOf(classname);
        if (rowIdx < 0 || colIdx <0) {
            NetworkAvgRespTTable filteredAvgTable = new NetworkAvgRespTTable(new ArrayList<>());
