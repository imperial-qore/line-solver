 ...
 */
public class MMAP extends MarkovianDistribution implements Serializable {
    List<Double> totalDepartureRate;
    List<Double> totalPhaseRate;
    private final int nPhases;

    public MMAP(Map<Integer, Matrix> mmap) {
        ...
        for (int i=2;i<mmap.size();i++){
            this.setParam(i+1, "D1"+Integer.toString(i), mmap.get(i));
        }
