...
| `/api/v1/ready` | GET | Readiness probe (reports `not_ready` above 90% heap use) |
| `/api/v1/info` | GET | Server info |
| `/api/v1/solvers` | GET | List solvers |
| `/api/v1/solvers/:id` | GET | Solver detail (formats, features, notes) |
| `/api/v1/solvers/:id/methods` | POST | Methods the solver admits for the posted model |
...
| `/api/v1/models/describe` | POST | Return model as LINE JSON interchange document |
...
Calibration is a closed-form estimate (`mu = X/U`, falling back on `N/R`), not a
fitting loop; it does not use the inference or `line-opt` machinery.
...
| Solver | Type | Model | Description |
|--------|------|-------|-------------|
| `mva` | Analytical | Network, LQN | Mean Value Analysis (on an LQN it runs `SolverLN`) |
| `jmt` | Simulation | Network | Java Modelling Tools |
| `ssa` | Simulation | Network | Stochastic simulation algorithm |
| `ctmc` | Analytical | Network | CTMC (state space forced) |
| `fluid` | Analytical | Network | Fluid approximation |
| `nc` | Analytical | Network, LQN | Normalizing constant |
| `ldes` | Simulation | Network | Discrete event simulation (SSJ) |
| `ln` | Analytical | LQN | Layered networks |
| `lqns` | Analytical | LQN | LQN solver |

The identifier is `ldes`, matching `GET /api/v1/solvers`, `LineCLI` and native
Python. It is not `des`: that token belongs to `line-cli.py` alone, whose
`SOLVERS` registry uses it for the same engine.
