...
public class LineRestServer {
    ...
    public void start() {
        ...
        // Request accounting for the metrics routes
        before((request, response) -> {
            String path = request.pathInfo();
            if (path == null || path.startsWith(API_BASE + "/metrics")) {
                return;
            }
            MetricsRoutes.incrementRequests();
            if (path.startsWith(API_BASE + "/models/solve")) {
                MetricsRoutes.incrementSolveRequests();
                request.attribute("line.startedAt", System.currentTimeMillis());
            } else if (path.startsWith(API_BASE + "/analysis/")) {
                MetricsRoutes.incrementAnalysisRequests();
            }
        });

        after((request, response) -> {
            Object startedAt = request.attribute("line.startedAt");
            if (startedAt instanceof Long) {
                MetricsRoutes.addSolveTime(System.currentTimeMillis() - (Long) startedAt);
            }
            if (response.status() >= 400) {
                MetricsRoutes.incrementFailedRequests();
            }
        });
