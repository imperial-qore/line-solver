"""
Tutorial 13: Posterior analysis with uncertain parameters
...
# Block 11: Plot the posterior PDF (density) of the response time
# Difference the empirical CDF into probability masses, then normalise by the
# bin width to obtain a density estimate over the unequally spaced values.
import matplotlib.pyplot as plt

order = np.argsort(resp_values)
rv = resp_values[order]
rp = resp_probs[order]
edges = np.concatenate([[rv[0]], (rv[:-1] + rv[1:]) / 2, [rv[-1]]])
widths = np.diff(edges)
density = rp / widths

plt.figure(figsize=(7, 4))
plt.bar(rv, density, width=widths, align='center', color=(0.2, 0.6, 0.8),
        edgecolor='white', linewidth=0.4)
plt.axvline(expected_R, linestyle='--', color='k', label=f'mean = {expected_R:.3f}')
if median_R is not None:
    plt.axvline(median_R, linestyle='--', color='r', label=f'median = {median_R:.3f}')
plt.xlabel('Response Time (R)')
plt.ylabel('Posterior PDF f(R)')
plt.title('Posterior PDF of Response Time')
plt.legend()
plt.tight_layout()
