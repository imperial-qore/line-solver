...
for N in n_jobs_list:
    ...
    # Direct fluid mean response times (exact, codebase-independent parity target)
    _r = np.array(avg_respt).reshape(-1)
    print('FLD getAvgRespT N=%d: %s' % (N, ' '.join('%.6f' % v for v in _r)))
