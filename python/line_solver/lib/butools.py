...
def lib_butools_ph_from_moments(moments):
    ...
    from line_solver.lib.thirdparty.butools.ph.appie import MEFromMoments
...
def lib_butools_ph_moments(alpha, A):
    ...
    from line_solver.lib.thirdparty.butools.ph.baseph import MomentsFromPH
...
def lib_butools_ph_me_moments(D0, D1):
    ...
    from line_solver.lib.thirdparty.butools.ph.baseph import MomentsFromME
...
def lib_butools_ph2_from_moments(m1, m2, m3):
    ...
    from line_solver.lib.thirdparty.butools.ph.canonical import PH2From3Moments
...
def lib_butools_ph3_from_moments(moments):
    ...
    from line_solver.lib.thirdparty.butools.ph.canonical import PH3From5Moments
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_aph_bounds(m1, m2, m3, order=2):
    ...
    from line_solver.lib.thirdparty.butools.ph.canonical import (
...
def lib_butools_ph_cdf(alpha, A, x):
    ...
    from line_solver.lib.thirdparty.butools.ph.baseph import CdfFromPH
...
def lib_butools_ph_pdf(alpha, A, x):
    ...
    from line_solver.lib.thirdparty.butools.ph.baseph import PdfFromPH
    alpha = np.matrix(np.atleast_2d(np.asarray(alpha, dtype=float)))
    A = np.matrix(np.asarray(A, dtype=float))
    x = np.atleast_1d(np.asarray(x, dtype=float))
...
def lib_butools_ph_canonical2(alpha, A):
    ...
    from line_solver.lib.thirdparty.butools.ph.canonical import CanonicalFromPH2
    alpha = np.atleast_2d(np.asarray(alpha, dtype=float))
    A = np.asarray(A, dtype=float)
...
def lib_butools_ph_check(alpha, A):
    ...
    from line_solver.lib.thirdparty.butools.ph.check import CheckPHRepresentation
    alpha = np.atleast_2d(np.asarray(alpha, dtype=float))
    A = np.asarray(A, dtype=float)
...
def lib_butools_moment_factorial_from_raw(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.conv import FactorialMomsFromMoms
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_moment_raw_from_factorial(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.conv import MomsFromFactorialMoms
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_moment_hankel_from_raw(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.conv import HankelMomsFromMoms
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_moment_normalized_from_raw(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.conv import NormMomsFromMoms
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_moment_reduced_from_raw(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.conv import ReducedMomsFromMoms
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_moment_joint_factorial(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.conv import JFactorialMomsFromJMoms
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_map_check(D0, D1):
    ...
    from line_solver.lib.thirdparty.butools.map.check import CheckMAPRepresentation
...
def lib_butools_map_marginal_moments(D0, D1):
    ...
    from line_solver.lib.thirdparty.butools.map.basemap import MarginalMomentsFromMAP
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_map_lag_correlations(D0, D1, L=1):
    ...
    from line_solver.lib.thirdparty.butools.map.basemap import LagCorrelationsFromMAP
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_markov_check_generator(Q):
    ...
    from line_solver.lib.thirdparty.butools.mc.check import CheckGenerator
...
def lib_butools_markov_check_probability_matrix(P):
    ...
    from line_solver.lib.thirdparty.butools.mc.check import CheckProbMatrix
...
def lib_butools_markov_ctmc_solve(Q):
    ...
    from line_solver.lib.thirdparty.butools.mc.stst import CTMCSolve
    Q = np.asarray(Q, dtype=float)
    result = CTMCSolve(Q)
    return np.asarray(result)
...
def lib_butools_markov_dtmc_solve(P):
    ...
    from line_solver.lib.thirdparty.butools.mc.stst import DTMCSolve
    P = np.asarray(P, dtype=float)
    result = DTMCSolve(P)
    return np.asarray(result)
...
def lib_butools_markov_crp_solve(Q):
    ...
    from line_solver.lib.thirdparty.butools.mc.stst import CRPSolve
    Q = np.asarray(Q, dtype=float)
    result = CRPSolve(Q)
    return np.asarray(result)
...
def lib_butools_markov_drp_solve(P):
    ...
    from line_solver.lib.thirdparty.butools.mc.stst import DRPSolve
    P = np.asarray(P, dtype=float)
    result = DRPSolve(P)
    return np.asarray(result)
...
def lib_butools_check_moments(moments):
    ...
    from line_solver.lib.thirdparty.butools.moments.check import CheckMoments
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_dph2_from_moments(m1, m2, m3):
    ...
    from line_solver.lib.thirdparty.butools.dph.canonical import DPH2From3Moments
...
def lib_butools_dph3_from_moments(moments):
    ...
    from line_solver.lib.thirdparty.butools.dph.canonical import DPH3From5Moments
    moments = list(np.asarray(moments).flatten())
...
def lib_butools_dph_canonical_form(sigma, D):
    ...
    from line_solver.lib.thirdparty.butools.dph.canonical import CanonicalFromDPH2
...
def lib_butools_dph_pmf(sigma, D, n):
    ...
    from line_solver.lib.thirdparty.butools.dph.basedph import PmfFromDPH
    sigma = np.atleast_2d(np.asarray(sigma, dtype=float))
    D = np.asarray(D, dtype=float)
...
def lib_butools_dph_moments(sigma, D, num_moments=3):
    ...
    from line_solver.lib.thirdparty.butools.dph.basedph import MomentsFromDPH
    sigma = np.atleast_2d(np.asarray(sigma, dtype=float))
    D = np.asarray(D, dtype=float)
...
def lib_butools_transform_me_to_ph(alpha, A):
    ...
    from line_solver.lib.thirdparty.butools.ph.monocyclic import MonocyclicPHFromME
    alpha = np.atleast_2d(np.asarray(alpha, dtype=float))
    A = np.asarray(A, dtype=float)
...
def lib_butools_dmap_check(D0, D1, prec=1e-14):
    ...
    from line_solver.lib.thirdparty.butools.dmap.check import CheckDMAPRepresentation
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_drap_check(H0, H1, prec=1e-14):
    ...
    from line_solver.lib.thirdparty.butools.dmap.check import CheckDRAPRepresentation
...
def lib_butools_dmap_marginal_moments(D0, D1, K=0):
    ...
    from line_solver.lib.thirdparty.butools.dmap.basedmap import MarginalMomentsFromDMAP
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_drap_marginal_moments(H0, H1, K=0):
    ...
    from line_solver.lib.thirdparty.butools.dmap.basedmap import MarginalMomentsFromDRAP
    H0 = np.asarray(H0, dtype=float)
    H1 = np.asarray(H1, dtype=float)
...
def lib_butools_dmap_lag_correlations(D0, D1, L=1):
    ...
    from line_solver.lib.thirdparty.butools.dmap.basedmap import LagCorrelationsFromDMAP
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_drap_lag_correlations(H0, H1, L=1):
    ...
    from line_solver.lib.thirdparty.butools.dmap.basedmap import LagCorrelationsFromDRAP
    H0 = np.asarray(H0, dtype=float)
    H1 = np.asarray(H1, dtype=float)
...
def lib_butools_dmap_marginal_distribution(D0, D1):
    ...
    from line_solver.lib.thirdparty.butools.dmap.basedmap import MarginalDistributionFromDMAP
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_dmap_samples(D0, D1, K):
    ...
    from line_solver.lib.thirdparty.butools.dmap.misc import SamplesFromDMAP
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_dmap_from_drap(H0, H1, prec=1e-14):
    ...
    from line_solver.lib.thirdparty.butools.dmap.dmapfromdrap import DMAPFromDRAP
    H0 = np.asarray(H0, dtype=float)
    H1 = np.asarray(H1, dtype=float)
...
def lib_butools_dmap2_from_moments(moms, corr1):
    ...
    from line_solver.lib.thirdparty.butools.dmap.matching import DMAP2FromMoments
...
def lib_butools_dmap_canonical2(D0, D1, prec=1e-14):
    ...
    from line_solver.lib.thirdparty.butools.dmap.matching import CanonicalFromDMAP2
    D0 = np.asarray(D0, dtype=float)
    D1 = np.asarray(D1, dtype=float)
...
def lib_butools_random_dmap(order, mean=10.0, zero_entries=0, max_trials=1000, prec=1e-7):
    ...
    from line_solver.lib.thirdparty.butools.dmap.basedmap import RandomDMAP
