...
def _is_exponential_arrival(sn: NetworkStruct, source_station: int, class_idx: int) -> bool:
    ...
    if isinstance(arrival_proc, dict):
        return 'rate' in arrival_proc
    if isinstance(arrival_proc, (list, tuple)) and len(arrival_proc) >= 2:
        second = np.atleast_2d(np.asarray(arrival_proc[1], dtype=float))
        return second.shape == (1, 1)
