...
def _map_to_ph(D0: np.ndarray, D1: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Convert a (D0, D1) MAP to PH: entry vector = embedded stationary
    distribution at completion epochs, sub-generator = D0 (as MATLAB
    aoi_dist2ph)."""
    n = D0.shape[0]
    Q = D0 + D1
    row_sums = Q.sum(axis=1)
    if np.max(np.abs(row_sums)) > 1e-10:
        Q = Q - np.diag(row_sums)
    A = np.vstack([Q.T, np.ones((1, n))])
    b = np.zeros((n + 1, 1))
    b[n, 0] = 1.0
    pi, _, _, _ = np.linalg.lstsq(A, b, rcond=None)
    pi = np.maximum(pi.ravel(), 0.0)
    pi = pi / pi.sum()
    completion = (D1 @ np.ones((n, 1))).ravel()
    numerator = pi * completion
    alpha = numerator / numerator.sum() if numerator.sum() > 0 else pi
    return alpha, D0
...
def aoi_dist2ph(dist_proc: Any) -> Tuple[np.ndarray, np.ndarray]:
    ...
    **Supported Distributions**:
    ...
    array([[-2.]])
    """
    # Native NetworkStruct payloads (see lang/network.py getStruct proc population)
    if isinstance(dist_proc, dict):
        if 'rate' in dist_proc:
            rate = float(dist_proc['rate'])
            return np.array([1.0]), np.array([[-rate]])
        if 'k' in dist_proc and 'mu' in dist_proc:
            k = int(dist_proc['k'])
            rate = float(dist_proc['mu'])
            sigma = np.zeros(k)
            sigma[0] = 1.0
            S = -rate * np.eye(k)
            for i in range(k - 1):
                S[i, i + 1] = rate
            return sigma, S
        if 'probs' in dist_proc and 'rates' in dist_proc:
            probs = np.asarray(dist_proc['probs'], dtype=float).ravel()
            rates = np.asarray(dist_proc['rates'], dtype=float).ravel()
            return probs / probs.sum(), -np.diag(rates)

    if isinstance(dist_proc, (list, tuple)) and len(dist_proc) >= 2:
        first = np.atleast_2d(np.asarray(dist_proc[0], dtype=float))
        second = np.atleast_2d(np.asarray(dist_proc[1], dtype=float))
        if np.any(np.isnan(first)) or np.any(np.isnan(second)):
            raise ValueError("Process contains NaN - class may be disabled")
        if (first.shape == second.shape and first.shape[0] == first.shape[1]
                and np.any(np.diag(first) < 0)):
            # [D0, D1] MAP payload
            return _map_to_ph(first, second)
        # [alpha, T] PH payload
        return np.asarray(dist_proc[0], dtype=float).ravel(), second
