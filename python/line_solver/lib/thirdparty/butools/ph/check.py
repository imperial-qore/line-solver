...
def CheckMERepresentation (alpha, A, prec=None):
    ...
    # exponential of order 2n+1 puts its whole spectrum on the line Re = -mu1,
    # so the arbitrary pick returns a complex eigenvalue and the test below
    # rejects a valid ME even though the real eigenvalue -mu1 is equally
    # dominant. Among the eigenvalues attaining the dominant real part, prefer
    # a real one: "the dominant eigenvalue is real" is a statement about the
    # spectrum, not about which tied eigenvalue the sort happened to return.
    # This only widens the accepted set, and only in the tied case, which
    # BuTools itself reports as a warning rather than a rejection below.
    domre = abs(np.real(maxev))
    domtol = max(prec, 1e-8 * domre)
    tied = ev[np.abs(np.abs(np.real(ev)) - domre) <= domtol]
    realtied = tied[np.abs(np.imag(tied)) <= domtol]
    if realtied.size > 0:
        maxev = np.real(realtied[0])
