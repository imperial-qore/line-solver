...
def CheckMEPositiveDensity (alpha, A, maxSize=100, prec=None):
    ...
    if not CheckMERepresentation (alpha, A, prec):
        return False

    try:
        # The monocyclic search deliberately probes ever larger representations,
        # whose entries can overflow before the search gives up. Those overflows
        # are part of the search, not an error, so they are not reported.
        with np.errstate(over='ignore', invalid='ignore'):
            beta, B = MonocyclicPHFromME (alpha, A, maxSize, prec)
        # The transformed pair must be a valid PH representation, not merely a
        # valid ME one: every monocyclic representation trivially satisfies the
        # latter, which made the test vacuous. This matches the JAR port in
        # jline/lib/butools/ph/CheckMEPositiveDensity.java.
        r = CheckPHRepresentation (beta, B, prec)
    except Exception:
        r = False
    return r
