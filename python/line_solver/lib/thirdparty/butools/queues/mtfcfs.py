...
# checkInput is read through a property rather than snapshot at import time:
# a snapshot makes the package-level flag inert, so a caller that legitimately
# needs a check relaxed for one call (a matrix-exponential or rational arrival
# is not a MAP, yet MMAPPH1FCFS handles it) has no way to say so.
class _butools_settings:
    @property
    def checkInput(self):
        import line_solver.lib.thirdparty.butools as _pkg
        return _pkg.checkInput
