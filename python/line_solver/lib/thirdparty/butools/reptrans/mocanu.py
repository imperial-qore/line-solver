    ...
    def generateFEBs (A, evalues, repeats, maxSize):
        ...
        while i<len(evalues):
            ...
            if -evalimag < precision:
                ...
                # TransformToMonocyclic.m and the JAR port.
                while evalimag / evalues[i].real >= 1.0 / math.tan(math.pi/n) - math.sqrt(np.finfo(float).eps):
                    n += 1
                    size += 1
                    if size > maxSize:
        ...
        # MATLAB's max() over a complex vector returns the entry of largest
        # modulus and its relational operators compare real parts only, so the
        # key is the real part of the largest-modulus eigenvalue of the block.
        # numpy's max() is lexicographic instead, which orders the blocks
        # differently and yields a different (still valid) monocyclic generator;
        # reproduce the MATLAB and JAR key here so all three agree.
        def febKey (feb):
            ev = np.atleast_1d(np.asarray(feb["evals"]))
            return ev[np.argmax(np.abs(ev))].real

        N = len(febs)
        for i in range(N):
            for j in range(N-i-1):
                if febKey(febs[j]) < febKey(febs[j+1]):
