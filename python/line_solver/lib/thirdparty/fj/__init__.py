...
Key functions:
    fj_is_homogeneous: Check if network has valid Fork-Join topology for FJ_codes analysis
...
@dataclass
class FjInfo:
    """Fork-Join topology information returned by fj_is_homogeneous.

    Matches MATLAB struct returned by fj_is_homogeneous.m.
    """
...
def fj_is_homogeneous(sn) -> Tuple[bool, FjInfo]:
    """
    Check for a single fork-join pair with homogeneous parallel branches.

    This is NOT a test for the presence of Fork/Join nodes (use sn_has_fork_join
    for that): most fork-join models fail this predicate. It tests membership in
    the homogeneous class on which the FJ_codes tail approximation is defined,
    ...
    References:
        Original MATLAB: matlab/src/api/fj/fj_is_homogeneous.m
...
def fj_extract_params_detailed(sn, fjInfo):
    ...
    Args:
        ...
        fjInfo: FjInfo from fj_is_homogeneous (with forkIdx, joinIdx, queueIdx, K)
...
def fj_extract_params(sn) -> Optional[FjParams]:
    ...
    isFJ, fjInfo = fj_is_homogeneous(sn)
    if not isFJ:
        return None
