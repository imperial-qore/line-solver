...
def q_ct_map_map_1(C0: np.ndarray, C1: np.ndarray, D0: np.ndarray, D1: np.ndarray,
    ...
    # Compute G and R using QBD solver (qbd_cr returns the continuous-time R
    # satisfying A1 + R*A0 + R^2*Am1 = 0).
    qbd_result = qbd_cr(Am1, A0, A1, verbose=options.verbose > 0)
    R = qbd_result['R']

    # Stationary distribution. The boundary vector pi_0 is the left-null vector
    # of the level-0 censored generator (B0 + R*Am1), where B0 = kron(C0, I) is
    # the empty-queue local block and Am1 = kron(I, D1) is a service completion
    # returning to level 0. NB: the shared qbd_pi helper computes
    # stat(B1 + R*B0) after a CT->DT conversion, which does not yield this
    # boundary vector for continuous-time blocks (it leaves a large residual and
    # over-weights the tail), so the boundary is computed directly here.
    Mbound = B0 + R @ Am1
    ns = la.null_space(Mbound.T)
    if ns.shape[1] >= 1:
        pi0 = ns[:, 0]
        if np.sum(pi0) < 0:
            pi0 = -pi0
        temp = la.inv(np.eye(mtot) - R)
        pi0 = pi0 / float(pi0 @ temp @ np.ones(mtot))
        # Queue length distribution: pi_n = pi_0 R^n, aggregated over phases.
        ql_list = [float(np.sum(pi0))]
        pk = pi0.copy()
        mass = ql_list[0]
        while mass < 1.0 - 1e-12 and len(ql_list) < options.max_num_comp:
            pk = pk @ R
            ql_list.append(float(np.sum(pk)))
            mass += ql_list[-1]
        ql = np.array(ql_list)
    else:
        # Degenerate boundary: fall back to the generic QBD stationary solver.
