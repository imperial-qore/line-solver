...
def ctmc_randomization(Q: np.ndarray, q: float = None,
    ...
    Args:
        Q: Infinitesimal generator matrix
        q: Uniformization rate (if None, uses 1.05*max|Q|)
        seed: Random seed (optional)
    ...
    # Compute uniformization rate. Deterministic default: an unseeded rand made
    # P, and everything downstream of it, irreproducible run to run. 1.05*max|Q|
    # is the rate ctmc_courtois already derives, and every quantity taken from P
    # is invariant to the choice of q above max|q_ii|.
    if q is None:
...
def dtmc_solve(P: np.ndarray) -> np.ndarray:
    ...
    Returns:
        Equilibrium distribution vector
    """
    from line_solver.api.mc.dtmc import dtmc_solve as _dtmc_solve
    return _dtmc_solve(P)
...
def dtmc_stochcomp(P: np.ndarray, I: np.ndarray = None) -> np.ndarray:
    ...
    """
    from line_solver.api.mc.dtmc import dtmc_stochcomp as _dtmc_stochcomp
    P = np.asarray(P, dtype=np.float64)
    if I is None:
        I = np.arange((P.shape[0] + 1) // 2)
    return _dtmc_stochcomp(P, I)
...
def dtmc_timereverse(P: np.ndarray) -> np.ndarray:
    ...
    """
    from line_solver.api.mc.dtmc import dtmc_timereverse as _dtmc_timereverse
    return _dtmc_timereverse(P)
...
def ctmc_stochcomp(Q: np.ndarray, I: np.ndarray = None) -> np.ndarray:
    ...
    """
    from line_solver.api.mc.ctmc import ctmc_stochcomp as _ctmc_stochcomp
    return _ctmc_stochcomp(Q, I)['S']
...
def ctmc_courtois(Q: np.ndarray, MS: list,
    ...
    """
    from line_solver.api.mc.aggregation import ctmc_courtois as _ctmc_courtois
    r = _ctmc_courtois(Q, MS, q)
    return r.p, r.Qperm, r.Qdec, r.eps, r.epsMAX, r.P, r.B, 0, r.q
...
def ctmc_kms(Q: np.ndarray, MS: list,
    ...
    """
    # Delegate to the canonical implementation in line_solver.api.mc.aggregation.
    # This module previously carried an independent copy that had drifted: it
    # started the iteration from pcourt in the ORIGINAL ordering while indexing P
    # by permuted macro-block offsets, inverted (D-U) via a 2-block split at the
    # arbitrary midpoint (nStates-2)//2 rather than a macro-block boundary, and
    # never renormalized between sweeps.
    from line_solver.api.mc.aggregation import ctmc_kms as _ctmc_kms
    r = _ctmc_kms(Q, MS, numSteps)
    return r.p, r.p_1, r.Qperm, r.eps, r.epsMAX, r.pcourt
...
def ctmc_takahashi(Q: np.ndarray, MS: list,
    ...
    """
    # Delegate to the canonical implementation in line_solver.api.mc.aggregation.
    # This module previously carried an independent copy that randomized Q with an
    # unseeded max|diag(Q)|+rand rate.
    from line_solver.api.mc.aggregation import ctmc_takahashi as _ctmc_takahashi
    r = _ctmc_takahashi(Q, MS, numSteps)
    return r.p, r.p_1, r.pcourt, r.Qperm, r.eps, r.epsMAX
...
def ctmc_multi(Q: np.ndarray, MS: list, MSS: list) -> Tuple:
    ...
    eps_val = np.max(np.sum(B_mat, axis=1))
