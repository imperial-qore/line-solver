...
def _map_scale_to_mean(D0, D1, new_mean):
    ...
    """
    from line_solver.api.mam import map_mean, map_normalize
...
def kpcfit_sub_bcfit(E: np.ndarray, SCVj: np.ndarray, G2j: np.ndarray,
    ...
    """
    from line_solver.api.mam import map_joint, map_scale, map_normalize
...
def kpcfit_sub_compose(E1j: np.ndarray, SCVj: np.ndarray,
    ...
    """
    from line_solver.api.mam import (map_kpc, map_isfeasible, map_normalize, map_erlang,
                       map_exponential, map_feasblock, map_embedded, map2_fit)
    from . import mmpp2_fit3
...
def kpcfit_manual(NumMAPs: int, E: np.ndarray, AC: np.ndarray,
    ...
    """
    from line_solver.api.mam import (map_acf, map_scv, map_skew, map_joint,
                       map_moment)
...
def kpcfit_auto(trace_data: KpcfitTraceData,
    ...
    Returns:
        Tuple of (bestMAP, fac, fbc, kpcMAPs,
                  otherMAPs, otherFACs, otherFBCs, otherSubMAPs)
    """
    from line_solver.api.mam import map_moment, map_acf
...
def kpcfit_ph_exact(E: np.ndarray, options: KpcfitPhOptions
    ...
    """
    from line_solver.api.mam import map_isfeasible, map_erlang, map_moment, map2_fit
    from . import aph_fit
...
def kpcfit_ph_search(E: np.ndarray, J: int, options: KpcfitPhOptions,
    ...
    """
    from line_solver.api.mam import (map_isfeasible, map_moment, map_feasblock,
...
def kpcfit_ph_manual(E: np.ndarray,
    ...
    """
    from line_solver.api.mam import (map_normalize, map_moment, map_feasblock, map_kpc,
    ...
    for j in range(NumPHs):
        if j == 0:
            ...
            from line_solver.api.mam import map_normalize as mn
            PH[j] = mn(D0_r, D1_r)
        else:
...
def kpcfit_ph_summary(PH_list: List[Tuple[Tuple[np.ndarray, np.ndarray],
    ...
    Args:
        ...
        E: Target moments
    """
    from line_solver.api.mam import map_moment
