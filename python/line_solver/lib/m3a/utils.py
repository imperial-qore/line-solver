...
def compute_moments(MMAP: List[np.ndarray], n: int = 3) -> np.ndarray:
    ...
    try:
        ...
        for k in range(n):
            moments[k] = mean**(k + 1) * math.factorial(k + 1)
...
def validate_mmap(MMAP: List[np.ndarray], tolerance: float = 1e-10) -> bool:
    ...
    # An MMAP is [D0, D1, D11, ..., D1m] with D1 the AGGREGATE of the class
    # matrices, so the generator is D0 + D1: adding the class matrices on top
    # of D1 as well would double count the arrivals and reject valid MMAPs.
    # Same two checks as mmap_isfeasible.m: closed generator, and class
    # matrices summing to D1.
    row_sums = (D0 + D1) @ np.ones(n)
    if np.any(np.abs(row_sums) > tolerance):
        return False

    if len(MMAP) > 2:
        S = D1.copy()
        for k in range(2, len(MMAP)):
            if np.any(np.asarray(MMAP[k]) < -tolerance):
                return False
            S = S - MMAP[k]
