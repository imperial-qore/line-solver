...
class SolverOptions:
    ...
    def __init__(self, solver_type=None):
        ...
        self.seed = 23000  # fixed for determinism harness (MATLAB/JAR use a random draw)
        self.iter_max = 1000
        self.iter_tol = 1e-4
        self.tol = 1e-4
        self.samples = 10000
        ...
        self.stiff = True
        self.timespan = [float('inf'), float('inf')]
        self.timestep = None
        # see _kb/11-conventions-and-gotchas.md (Python long-tail low-hit gotchas) for rationale
...
class SolverMVA(NetworkSolver):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        method = kwargs.pop('method', 'default')  # 'default' auto-selects exact/amva (matches MATLAB/JAR)
    ...
    def defaultOptions():
        ...
        return OptionsDict({
            'method': 'default',  # auto-selects exact/amva (matches MATLAB/JAR)
            'verbose': VerboseLevel.STD,
            'iter_max': 1000,
            'tol': 1e-4,
            'config': OptionsDict({
...
class SolverBA(NetworkSolver):
    """Bound-analysis solver (asymptotic + hierarchical throughput/queue-length
    bounds, plus QRF LP-based bounds). Bound methods and QRF that formerly lived
    under SolverMVA/SolverCTMC are served here."""

    def __init__(self, model, *args, **kwargs):
        options = SolverOptions(SolverType.BA)
        super().__init__(model, options, *args, **kwargs)

        from .solvers.solver_ba import SolverBA as _NativeBA
        method = kwargs.pop('method', 'default')
        supported = {'verbose', 'seed', 'max_iter', 'tol', 'level'}
        ba_kwargs = {k: v for k, v in kwargs.items() if k in supported}
        self._native_solver = _NativeBA(model, method=method, **ba_kwargs)
        if 'level' in kwargs:
            setattr(self._native_solver.options, 'level', kwargs['level'])

    def list_valid_methods(self):
        return self._native_solver.list_valid_methods()

    listValidMethods = list_valid_methods

    def get_bounds(self):
        """Return {Tlower,Tupper,Qlower,Qupper} bracket for the current family."""
        return self._native_solver.get_bounds()

    getBounds = get_bounds

    def get_bounds_table(self, keepDisabled=False):
        """Return the bracket per station and class as a table, in the layout of
        getAvgTable: Station, JobClass, Qlower, Qupper, Tlower, Tupper.

        One-sided families (cub upper-only, mbjb/ldbcmp lower-only) carry NaN on
        the missing side."""
        result = self._native_solver.get_bounds_table(keepDisabled)
        if isinstance(result, pd.DataFrame):
            return IndexedTable(result)
        return result
...
class SolverLDES(NetworkSolver):
    ...
    def defaultOptions():
        ...
        return OptionsDict({
            'samples': 200000,  # effective LDESOptions default (was misadvertised as 10000)
            'events': None,
            'seed': 23000,
            'verbose': VerboseLevel.STD
        })
...
class SolverLQNS(Solver):
    ...
    def get_raw_avg_tables(self):
        ...
        if self._native_solver is not None:
            # The native LQNS solver already builds the call-level table
            # (From/To/CallRate/Wait); forward it instead of dropping it.
            if hasattr(self._native_solver, 'getRawAvgTables'):
                return self._native_solver.getRawAvgTables()
            return self._native_solver.getAvgTable(), None
        return None, None
...
class SolverUQ(EnsembleSolver):
    """UQ solver for Bayesian inference."""
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        from .solvers.solver_uq import SolverUQ
        self._native_solver = SolverUQ(model, **kwargs)
