...
def rcat_supports_processes(sn, method='inap') -> Tuple[bool, Optional[str]]:
    """Reject non-exponential processes: RCAT models each station-class by its
    mean rate only.

    build_rcat builds one scalar birth-death chain per (station,class) from
    sn.rates: the state is the queue length, with no service-phase dimension.
    Every process is therefore collapsed to its mean rate, so a non-exponential
    arrival or service process would be answered as if it were exponential.
    Measured vs SolverCTMC on an open M/PH/1, INAP returns the M/M/1 queue
    length for EVERY scv (1.000000 at rho=0.5, 4.000000 at rho=0.8): 14% error
    at scv=0.5 and 43% at scv=4.0. Reject rather than mis-answer.
    ...
    # see _kb/06-solver-catalog.md (MAM: "AG / RCAT methods") -- signal
    # classes are exempt from the EXP-only service gate
    ...
    for ist in range(sn.nstations):
        for r in range(sn.nclasses):
            if r < len(issignal) and issignal[r] and not _is_source(ist):
                continue
            try:
            ...
            if rates.size and (not np.isfinite(rates[ist, r]) or rates[ist, r] <= 0):
                continue
            return False, (
                "The %s method supports exponential processes only (RCAT models "
                "each station-class by its mean rate, with no service-phase "
                "dimension), but station %d class %d is %s. Use the dec.source "
                "method for non-exponential models." % (method, ist + 1, r + 1, pname)
            )
...
def _build_rcat(sn, max_states: int = 100) -> RCATModelData:
    ...
    for a, am in enumerate(action_map):
        ...
        # "Boundary self-loop is closed-class only")
        if Np_active > 0 and r < len(njobs) and njobs[r] < np.inf:
...
def _compute_equilibrium(x: np.ndarray, model: RCATModelData) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    ...
    for k in range(num_processes):
        ...
        # Solve for equilibrium using birth-death recursion for tridiagonal
        # generators (numerically stable), falling back to ctmc_solve otherwise
        if _is_tridiagonal(Qk):
            pi_k = _birth_death_solve(Qk)
        else:
            pi_k = ctmc_solve(Qk)
        pi_list.append(pi_k)
...
def _inap_solve(model: RCATModelData, tol: float = 1e-6, max_iter: int = 1000,
    ...
    # processes (catastrophe/batch-removal) take the INAP+ rate-conservation estimator
    not_birth_death = [False] * num_processes
    for k in range(num_processes):
        Lk = R.get((num_actions, k), np.zeros((N[k], N[k])))
        for n in range(N[k]):
            for m in range(N[k]):
                if Lk[n, m] > 0 and (m < n - 1 or m > n + 1):
                    not_birth_death[k] = True
            ...
            if method == 'inapplus' or not_birth_death[k]:
                ...
                for i in range(N[k]):
                    for j in range(N[k]):
                        if Aa[i, j] > 0:
                            if i == j:
                                # Diagonal (boundary): ratio is always 1
                                lambda_vec.append(Aa[i, j])
                            elif pi[k][j] > 0:
                                ratio = Aa[i, j] * pi[k][i] / pi[k][j]
                                if np.isfinite(ratio):
                                    lambda_vec.append(ratio)
                if len(lambda_vec) > 0:
...
def _compute_equilibrium_qbd(x: np.ndarray, model: RCATModelData,
    ...
    Open components are solved on their infinite state space by a scalar
    matrix-geometric (QBD / catastrophe) decomposition, yielding the exact
    geometric marginal pi_n = (1-rho) rho^n; closed (finite) components fall
    back to the explicit finite solve. Mirrors solver_mam_ag.m
    (compute_equilibrium_qbd).
    """
    num_processes = model.num_processes
    num_actions = model.num_actions
    N = model.N
        ...
        if is_open_proc[k] and Nk >= 5:
            ...
            if inter_down <= 1e-11 and f > 0:
                ...
                if np.isfinite(rho) and 0.0 < rho < 1.0 - 1e-12:
                    rho_proc[k] = rho
                    is_geom_proc[k] = True
                    pi_list.append((1.0 - rho) * rho ** np.arange(Nk))
                    solved_geom = True

        if not solved_geom:
            if _is_tridiagonal(Qk):
                pi_list.append(_birth_death_solve(Qk))
            else:
                pi_list.append(ctmc_solve(Qk))

    return pi_list, Q_list, rho_proc, is_geom_proc
...
def _inap_inf_solve(model: RCATModelData, is_open_proc: np.ndarray,
    ...
    if num_actions == 0:
        ...
        return (x, pi, Q, 0, np.zeros(num_processes),
                np.zeros(num_processes, dtype=bool), 0.0)
    ...
    pi, Q, rho_proc, is_geom_proc = _compute_equilibrium_qbd(x, model, is_open_proc)
    ...
    def inapinf_sweep(pi_prev, itnum):
        nonlocal pi, Q, rho_proc, is_geom_proc
    ...
    return x, pi, Q, iteration, rho_proc, is_geom_proc, rcat_res
...
def _rcat_metrics(sn, x: np.ndarray, pi: List[np.ndarray], Q: List[np.ndarray],
                  ...
                  is_geom_proc: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Convert RCAT solution to LINE performance metrics.

    When rho_proc/is_geom_proc are supplied ('inapinf'), processes flagged
    geometric use the exact closed-form moments of the infinite geometric
...
class INAPInfAlgorithm(MAMAlgorithm):
    ...
    def solve(self, sn, options=None) -> MAMResult:
        ...
        x, pi, Q, iterations, rho_proc, is_geom_proc, rcat_res = _inap_inf_solve(
            model, is_open_proc, tol, max_iter, verbose)

        QN, UN, RN, TN, CN, XN = _rcat_metrics(sn, x, pi, Q, model, rho_proc, is_geom_proc)
