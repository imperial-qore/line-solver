...
class INAPAlgorithm(MAMAlgorithm):
    ...
    def solve(self, sn, options=None) -> MAMResult:
        ...
        # Main iteration: rate fixed point, driven by the generic DA driver
        def inap_sweep(prev_lambda, itnum):
            nonlocal lambda_k

            if verbose and itnum <= 3:
                print(f"  Iteration {itnum}: lambda = {lambda_k}")
            ...
            return lambda_k.copy(), prev_lambda

        _, iteration, _ = da_fpi(inap_sweep, lambda_k.copy(), max_iter, tol, nanstop=True)
