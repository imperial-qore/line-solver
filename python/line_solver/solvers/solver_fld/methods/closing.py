    ...
    def _solve_fluid_ode(
        ...
        try:
            ...
            if odemaxstep is not None and np.isfinite(odemaxstep):
                ...
                max_step = min((t_end - t_start) / 10, ratemult_max_step(rt_tgrid))
            sol = solve_ivp(
                ode_func,
                [t_start, t_end],
                x0,
    ...
    def _build_ode_system(
        ...
        for j in range(M):
            for l in range(K):
                ...
                # see _kb/06-solver-catalog.md (Fluid: NHPP transient) -- an
                # NHPP slot holds {breakpoints,rates,cyclic}, not a [D0,D1] pair
                if hasattr(self.sn, 'proc') and self.sn.proc is not None \
                        and not is_nhpp(self.sn, j, l):
