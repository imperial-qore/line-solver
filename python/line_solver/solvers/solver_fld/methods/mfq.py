...
class MFQMethod:
    ...
    def _is_single_queue(self) -> bool:
        ...
        # see _kb/06-solver-catalog.md (Fluid: "Fallback/utility numeric
        # methods") for the mfq topology-detection fallback rationale
    ...
    def _extract_queue_parameters(self) -> Dict:
        ...
        """
        # see _kb/06-solver-catalog.md (Fluid: "Fallback/utility numeric
        # methods") for why mu is read from the Queue row, not station 0
        from ....constants import SchedStrategy
        ext_val = getattr(SchedStrategy.EXT, 'value', SchedStrategy.EXT)
        source_st = None
        queue_st = None
        sched = getattr(self.sn, 'sched', None)
        for i in range(int(self.sn.nstations)):
            if isinstance(sched, dict):
                sch_i = sched.get(i, None)
            elif sched is not None and i < len(sched):
                sch_i = sched[i]
            else:
                sch_i = None
            is_source = sch_i is not None and getattr(sch_i, 'value', sch_i) == ext_val
            if is_source and source_st is None:
                source_st = i
            elif (not is_source) and queue_st is None:
                queue_st = i

        rates = np.asarray(self.sn.rates) if getattr(self.sn, 'rates', None) is not None else None

        # Arrival rate per class: the Source station's rate, else legacy field.
        lambda_arr = [0.5]
        if rates is not None and rates.size > 0 and source_st is not None:
            lambda_arr = np.atleast_2d(rates)[source_st, :].tolist()
        elif hasattr(self.sn, 'lambda_arr') and self.sn.lambda_arr is not None:
            lambda_arr = np.asarray(self.sn.lambda_arr).flatten().tolist()

        # Service rate per class: the queue station's rate (not the Source).
        mu_arr = [1.0]
        if rates is not None and rates.size > 0:
            r2 = np.atleast_2d(rates)
            row = queue_st if queue_st is not None else (r2.shape[0] - 1)
            mu_arr = r2[row, :].tolist()

        # Number of servers: from the identified queue station (not the Source,
        # whose nservers row also reads as finite in the struct).
        nservers = 1
        if hasattr(self.sn, 'nservers') and self.sn.nservers is not None:
            nservers_arr = np.asarray(self.sn.nservers).flatten()
            if queue_st is not None and queue_st < nservers_arr.size \
                    and np.isfinite(nservers_arr[queue_st]) and nservers_arr[queue_st] >= 1:
                nservers = int(nservers_arr[queue_st])
            else:
                for i, ns in enumerate(nservers_arr):
                    if i == source_st:
                        continue
    ...
    def _solve_mm_queue(self, params: Dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        for k in range(nclasses):
            ...
            if rho >= 1.0:
                ...
                # Total queue length: L = Lq + lambda/mu (Little's law, in-service jobs)
