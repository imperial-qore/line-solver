...
This is the path that kept the NLP QRF tokens withheld from BA_METHODS. Exercising
...
def test_exact_where_polytope_is_tight(name):
    ...
    tight = [i for i, (lo, hi) in enumerate(REFERENCE[name]) if hi - lo < 1e-9]
    if not tight:
        pytest.skip('polytope is a strict relaxation for %s' % name)
    _QN, UN, _XN = _run(name)
...
def test_infinite_server_is_rejected():
    """qrf_noblo_* models every station as a single server, so a Delay would
    silently change the model rather than approximate it."""
    model = Network('qrf_delay')
    think = Delay(model, 'Think')
    q = Queue(model, 'Q1', SchedStrategy.FCFS)
    cls = ClosedClass(model, 'C1', 3, think)
    ...
    with pytest.raises(ValueError, match='infinite-server'):
        solver_ctmc_qrf_analyzer(model.getStruct(), _Opts('qrf.mmi'))
