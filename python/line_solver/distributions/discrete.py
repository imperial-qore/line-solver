...
class Replayer(DiscreteDistribution):
    ...
    def __init__(self, trace: Union[str, list, np.ndarray], loop: bool = True):
        ...
        # see _kb/09-ldes-and-cache.md (Replayer/Trace, distributions save) for rationale
    ...
    def evalLST(self, s: float) -> float:
        ...
        Exact for the trace, and what the G/M/1 sigma-root needs: without it
        sn.lst stayed None for a Replayer arrival and the MVA gm1 branch fell
        back to a two-moment approximation. Matches MATLAB Replayer.evalLST and
        the JAR Replayer.evalLST.
        """
        return float(np.mean(np.exp(-s * self._trace)))
    ...
    def fit_exp(self):
        ...
        """
        from .continuous import Exp
        return Exp.fit_mean(self.getMean())

    def fit_coxian(self):
        """
        Fit a two-phase Coxian to the first three moments of the trace
        (MATLAB Replayer.fitCoxian, which calls Cox2.fit).

        Returns:
            Cox2 distribution fitted to the trace moments.
        """
        from .markovian import Cox2
        mean = self.getMean()
        var = self.getVar()
        return Cox2.fit_central(mean, var, self.getSkewness())
