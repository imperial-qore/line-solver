...
class IndexedTable:
    ...
    def __getattr__(self, name):
        """
        Delegate any attribute not defined on IndexedTable to the underlying
        DataFrame, so IndexedTable is a drop-in for standard pandas operations
        (iterrows, to_string, values, etc.).
        """
        if name == 'data':
            # 'data' is set in __init__; if we reach here it is genuinely
            # absent (e.g. during unpickling) - do not recurse.
            raise AttributeError(name)
        return getattr(self.data, name)

    def __iter__(self):
        """Iterate like the underlying DataFrame (over column labels)."""
