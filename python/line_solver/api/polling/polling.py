...
def polling_qsys_decrementing(
    arvMAPs: List[Tuple[np.ndarray, np.ndarray]],
    svcMAPs: List[Tuple[np.ndarray, np.ndarray]],
    switchMAPs: List[Tuple[np.ndarray, np.ndarray]]
) -> np.ndarray:
    """
    Compute exact mean waiting times for a symmetric decrementing
    (semiexhaustive) polling system with open (Poisson) arrivals.

    In decrementing service, the server serves a queue until the number of
    jobs present drops to one less than the number found at the polling
    instant. The symmetric system admits the exact closed form of Pittel
    (1973) and Takagi (1984); see Takagi, ACM Computing Surveys, Vol. 20,
    No. 1, 1988, eq (28). No exact closed form for the individual E[W_i] is
    known for asymmetric decrementing systems, so this analysis is restricted
    ...
    def _reldiff(x, y):
        ...
        return 0.0 if scale < 1e-30 else abs(x - y) / scale

    tol = 1e-6
    for i in range(1, n):
        if (_reldiff(lambda_arr[i], lambda_arr[0]) > tol or
                _reldiff(b[i], b[0]) > tol or
                _reldiff(b2[i], b2[0]) > tol or
                _reldiff(r1[i], r1[0]) > tol or
                _reldiff(delta2[i], delta2[0]) > tol):
            raise ValueError(
                "MVA analysis for decrementing polling is only available for "
                "symmetric systems (identical arrival, service and switchover "
                "parameters across all queues).")
    ...
    denom = 2.0 * (1.0 - rho - lam * r * (N - rho))
    if denom <= 0.0:
        raise ValueError(
            "Decrementing polling system is unstable: "
            "rho + lambda*r*(N-rho) >= 1.")

    residual_switchover = d2 / (2.0 * r) if r > 0.0 else 0.0
    Wval = residual_switchover + (
        N * lam * b2s * (1.0 - lam * r) + (r + lam * d2) * (N - rho)) / denom

    return Wval * np.ones(n)
