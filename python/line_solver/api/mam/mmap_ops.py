...
def mmap_normalize(D0: np.ndarray, D_list: List[np.ndarray]) -> Tuple[np.ndarray, List[np.ndarray]]:
    ...
    Port of matlab/src/api/mam/mmap_normalize.m (mirrored by the JAR
    jline.api.mam.Mmap_normalize). The MMAP is [D0] + D_list with
    D_list = [D1, D21, ..., D2C], i.e. the aggregate D1 followed by the C
    per-class marking matrices, so C = len(D_list) - 1. This is the layout
    produced by _mmap_to_tuple and by the mmap_super / mmap_max callers, which
    pass result[1:].
    ...
    Args:
        ...
        D_list: [D1, D21, ..., D2C], the aggregate followed by the per-class
            marking matrices
    ...
    # Negative off-diagonal rates are infeasible (MATLAB lines 9-15)
    for i in range(n):
        for j in range(n):
            if i != j:
                D0[i, j] = max(D0[i, j], 0.0)

    # see _kb/03-api-layer.md for rationale
    per_class = D_list[1:]
    for c in range(C):
        per_class[c][per_class[c] < 0] = 0.0
        if per_class[c].size > 0 and np.all(np.isnan(per_class[c])):
            per_class[c] = np.zeros_like(per_class[c])
    D1 = np.zeros_like(D0)
    for c in range(C):
        D1 = D1 + per_class[c]

    # Close each generator row: D0[k,k] = -offdiag(D0)[k] - rowsum(D1)[k]
    # (MATLAB lines 27-30)
    for k in range(n):
        D0[k, k] = 0.0
        D0[k, k] = -(np.sum(D0[k, :]) + np.sum(D1[k, :]))

    return D0, [D1] + per_class
...
def mmap_super_safe(mmap_list: List[Tuple[np.ndarray, List[np.ndarray]]],
    ...
    for D0, D_list in mmap_list:
        ...
        # see _kb/03-api-layer.md for rationale
        if D0.shape[0] > 1 and np.abs(agg_D1).sum() < 1e-13:
            K = len(D_list)
            D0 = np.zeros((1, 1))
            D_list = [np.zeros((1, 1)) for _ in range(K)]
            agg_D1 = np.zeros((1, 1))
...
def _mmap_super(D0_1: np.ndarray, D_list_1: List[np.ndarray],
    ...
    if method == "match":
        # see _kb/03-api-layer.md for rationale
        if len(D_list_1) != len(D_list_2):
            raise ValueError(
                "class matching failed: MMAPs have different number of classes")
        D_list_super = [np.kron(D_k1, I2) + np.kron(I1, D_k2)
                        for D_k1, D_k2 in zip(D_list_1, D_list_2)]
...
def _mmap_poisson_reduce(D0: np.ndarray,
                         D_list: List[np.ndarray]
                         ) -> Tuple[np.ndarray, List[np.ndarray]]:
    """
    Reduce an MMAP to a per-class Poisson (order-1) MMAP that preserves BOTH the
    per-class arrival rates and the class count.

    Used by mmap_super_safe when a flow would push the Kronecker product past
    maxorder. Collapsing to a single aggregate exponential (one class) would
    break method='match', which requires equal class counts, and would discard
    the marking; keeping one Poisson stream per class avoids both. D_list is the
    class-only layout used inside mmap_super_safe, so the aggregate is rebuilt as
    sum(D_list) for the mmap_lambda call (which expects aggregate-first).
    """
    D0 = np.asarray(D0, dtype=np.float64)
    D_list = [np.asarray(D, dtype=np.float64) for D in D_list]
    agg = sum(D_list) if D_list else np.zeros_like(D0)
    rates = mmap_lambda([D0, agg] + D_list)
...
def mmap_hide(D0: np.ndarray, D_list: List[np.ndarray],
    ...
    for D_k in D_list:
        D1 = D1 + D_k
    D0, full_list = mmap_normalize(D0, [D1] + D_list)

    return D0, full_list[1:]
...
def mmap_compress(D0: np.ndarray, D_list: List[np.ndarray],
    ...
    The MMAP is [D0] + D_list with D_list = [D1, D11, ..., D1C], i.e. the
    aggregate D1 followed by the C per-class marking matrices, so
    C = len(D_list) - 1. This is the layout of mmap_normalize and of every
    caller (npfqn_traffic_merge, mmap_fj), which pass mmap[1:].

    Args:
        D0: Hidden transition matrix
        D_list: [D1, D11, ..., D1C], the aggregate followed by the per-class
            marking matrices
        method: Compression method. Options:
                - "default", "mixture", "mixture.order1": order-1 mixture fitting
                - "exponential": Fit to single-state Poisson
        target_order: If given, skip compression when the MMAP order is already
                at most this value. There is no implicit size guard: the
                order-1 mixture is a lossy approximation with a contract (see
                _compress_mixture_order1), and callers rely on it holding
    ...
    if target_order is not None and n <= target_order:
        return D0.copy(), [D.copy() for D in D_list]

    if method in ["default", "mixture", "mixture.order1"]:
        return _compress_mixture_order1(D0, D_list)
...
def _compress_mixture_order1(D0: np.ndarray,
                             D_list: List[np.ndarray]) -> Tuple[np.ndarray, List[np.ndarray]]:
    """
    Order-1 mixture (M3A). Builds K components, one per class, and recombines
    them with mmap_mixture using the class probabilities p_c as mixing weights.

    Component c must carry the law of the inter-arrival time CONDITIONED ON THE
    ARRIVAL THAT ENDS IT BEING OF CLASS c: mmap_mixture marks the arrival
    leaving component c with class c, so the class of an arrival and the
    interval preceding it are both governed by the component active during that
    interval. That conditional law is the class-c BACKWARD moment set
    B(c,1:3) = E[T^k | class of the ending arrival = c]. It is NOT the forward
    moment (which conditions on the class of the STARTING arrival) and it is
    NOT the class-c marginal MAP from mmap_maps (whose mean is 1/lambda_c).
    This mirrors 'mixture.order2', which conditions on the (last,next) class
    pair and fits the cross moments; order 1 drops the "last" index.

    Preserved exactly: aggregate moments 1..3 via the M3A mixture law
    M_k = sum_c B(k,c)*p_c (M1 always, since aph2_adjust never alters M1;
    M2/M3 when APH(2)-feasible); the class probabilities p_c and hence the
    per-class rates lambda_c = p_c/M1; marking consistency D1 = sum_c D1^(c);
    and MAP feasibility.

    Lost by construction: every autocorrelation. Each component is re-entered
    at its map_pie on every arrival, so the intervals are i.i.d. and the result
    is a RENEWAL process (acf -> 0, IDC -> the SCV-determined renewal value,
    class sequence i.i.d.). Retaining the class-transition correlation is
    exactly what 'mixture.order2' buys with its K^2 components.
    """
    from .aph2_fitting import aph2_fit

    # D_list is [D1, D11, ..., D1C]: the aggregate first, then C class marks.
    K = len(D_list) - 1
    mmap = [D0] + list(D_list)

    p = mmap_pc(mmap)
    B = mmap_backward_moment(mmap, np.array([1, 2, 3]), True)

    maps = []
    for c in range(K):
        if p[c] <= GlobalConstants.Zero:
            # see _kb/03-api-layer.md for rationale
            maps.append(list(map_exponential(1.0)))
        else:
            maps.append(aph2_fit(B[c, 0], B[c, 1], B[c, 2]))

    result = mmap_mixture(p, maps)
    return result[0], list(result[1:])


def _compress_exponential(D0: np.ndarray, D_list: List[np.ndarray]) -> Tuple[np.ndarray, List[np.ndarray]]:
    """
    Compress to a marked Poisson process: a single state carrying the aggregate
    rate, split across the classes by their arrival probabilities p_c.

    Preserves the aggregate rate and the per-class rates lambda_c = p_c*Lambda;
    discards everything else (SCV, autocorrelation, class correlation). Layout
    is [D1, D11, ..., D1C], as in mmap_compress.
    """
    # D_list[0] IS the aggregate D1; summing D_list would double-count it.
    mmap = [D0] + list(D_list)
    K = len(D_list) - 1
    lambda_agg = map_lambda(D0, np.asarray(D_list[0], dtype=np.float64))
    p = mmap_pc(mmap)

    D0_comp = np.array([[-lambda_agg]])
    D_list_comp = [np.array([[lambda_agg]])]
    for c in range(K):
        D_list_comp.append(np.array([[p[c] * lambda_agg]]))
...
def mmap_isfeasible(mmap: List[np.ndarray], tol: float = None) -> bool:
    ...
    # see _kb/03-api-layer.md for rationale
    D1_total = np.asarray(mmap[1], dtype=np.float64)
...
def mmap_pie(mmap: List[np.ndarray]) -> np.ndarray:
    ...
    # mmap[1] IS the aggregate D1 (= sum_c mmap[2+c]); summing mmap[1:] double-counts it.
    D1 = np.asarray(mmap[1], dtype=np.float64)
...
def mmap_cross_moment(mmap: List[np.ndarray], k: int) -> np.ndarray:
    ...
    for i in range(C):
        ...
        for j in range(C):
            Dj = np.asarray(mmap[2 + j], dtype=np.float64)
            MC[i, j] = math.factorial(k) * np.sum(start @ M_pow @ Dj)
