...
def sn_deaggregate_chain_results(
    ...
    Args:
        ...
        Cchain: (1, C) Mean system response time per chain. MATLAB
            (sn_deaggregate_chain_results.m) rejects a non-empty Cchain and always
            derives C from Little's law; callers therefore pass None/empty and C is
            computed as njobs/X, matching MATLAB. A non-empty Cchain is accepted
            here as an optional extension (disaggregated via alpha at the reference
            station) but is never supplied on the standard solver paths.
