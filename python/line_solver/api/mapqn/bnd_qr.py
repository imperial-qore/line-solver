                ...
                for h in range(1, K[j - 1] + 1):
                    if h != k:
                        model.set_variable_bounds(
                            f'p2_{j}_{nj}_{k}_{j}_{nj}_{h}', 0.0, 0.0)
            ...
            for k in range(1, K[j - 1] + 1):
                for ni in range(N + 1):
                    if ni != nj:
                        for h in range(1, K[j - 1] + 1):
                            model.set_variable_bounds(
                                f'p2_{j}_{nj}_{k}_{j}_{ni}_{h}', 0.0, 0.0)
                ...
                for i in range(1, M + 1):
                    ...
                    for ni in range(N + 1):
                        if nj + ni > N:
                            for h in range(1, K[i - 1] + 1):
                                model.set_variable_bounds(
                                    f'p2_{j}_{nj}_{k}_{i}_{ni}_{h}', 0.0, 0.0)
    ...
    for j in range(1, M + 1):
        for k in range(1, K[j - 1] + 1):
            for i in range(1, M + 1):
                ...
                for ni in range(N + 1):
                    for h in range(1, K[i - 1] + 1):
                        constraint.add_term(f'p1_{j}_{k}_{i}_{ni}_{h}',
                                            -float(ni))
                model.add_constraint(constraint.eq(0.0))
...
def _add_balance_constraints_qr(model: MapqnLpModel, params: LinearReductionParameters) -> None:
    ...
    # SRVB: Service balance (AMPL THM1)
            ...
            for nj in range(N + 1):
                for i in range(1, M + 1):
                    ...
                    for ni in range(N - nj + 1):
                        for h in range(1, K[i - 1] + 1):
                            constraint.add_term(
                                f'p2_{j}_{nj}_{k}_{i}_{ni}_{h}', -1.0)
                    model.add_constraint(constraint.eq(0.0))
            ...
            for nj in range(N + 1):
                ...
                for i in range(1, M + 1):
                    for ni in range(1, N + 1):
                        for h in range(1, K[i - 1] + 1):
                            constraint.add_term(
                                f'p2_{j}_{nj}_{k}_{i}_{ni}_{h}', float(ni))
            ...
            for j in range(1, M + 1):
                ...
                for nj in range(1, N + 1):
                    for k in range(1, K[j - 1] + 1):
                        for h in range(1, K[j - 1] + 1):
                            constraint.add_term(
                                f'p2_{j}_{nj}_{k}_{i}_0_{u}',
                                params.q(j - 1, i - 1, k - 1, h - 1))
                for nj in range(N + 1):
                    for k in range(1, K[i - 1] + 1):
                        for h in range(1, K[j - 1] + 1):
                            constraint.add_term(
                                f'p2_{j}_{nj}_{h}_{i}_1_{k}',
                ...
                for nj in range(1, N + 1):
                    for k in range(1, K[j - 1] + 1):
                        for h in range(1, K[j - 1] + 1):
                            ...
                            for u in range(1, K[i - 1] + 1):
                                constraint.add_term(
                                    f'p2_{j}_{nj}_{k}_{i}_{ni}_{u}', w)
                for nj in range(N + 1):
                    for k in range(1, K[i - 1] + 1):
                        for u in range(1, K[j - 1] + 1):
                            for h in range(1, K[i - 1] + 1):
                                constraint.add_term(
                                    f'p2_{j}_{nj}_{u}_{i}_{ni + 1}_{k}',
...
def _add_bound_constraints_qr(model: MapqnLpModel, params: LinearReductionParameters) -> None:
    ...
    # THM4: N*P(j busy in k, i nonempty) <= sum_t C[j,k,t] (AMPL THM4)
    for j in range(1, M + 1):
        for k in range(1, K[j - 1] + 1):
            for i in range(1, M + 1):
                constraint = model.constraint_builder()
                for h in range(1, K[i - 1] + 1):
                    for nj in range(N + 1):
                        for ni in range(1, N + 1):
                            constraint.add_term(
                                f'p2_{j}_{nj}_{k}_{i}_{ni}_{h}', float(N))
                for t in range(1, M + 1):
                    for h in range(1, K[t - 1] + 1):
                        for nj in range(N + 1):
                            for nt in range(N + 1):
                                constraint.add_term(
                                    f'p2_{j}_{nj}_{k}_{t}_{nt}_{h}', -float(nt))
                model.add_constraint(constraint.leq(0.0))
