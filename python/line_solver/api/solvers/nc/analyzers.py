...
def solver_nc_lossn_analyzer(
    ...
    inside a Finite Capacity Region (FCR) with DROP policy.
    ...
    Method selection (options.method):
        ...
        'mci'                - Monte Carlo importance-sampling summation
                               ...
                               constant g(C) and class blocking with
                               confidence intervals (options.samples/seed).
    ...
    """
    from ...lossn import lossn_erlangfp, lossn_mci
    ...
    use_mci = 'mci' in tokens

    lG = np.nan  # normalization constant (finite only for mci)
    if use_mci:
    ...
    for r in range(K):
        ...
        source_idx = int(sn.refstat[r]) if hasattr(sn, 'refstat') and sn.refstat is not None else 0
        T[source_idx, r] = qlen[r]
    ...
    result.lG = lG  # normalization constant (finite only for mci)
    result.it = niter
