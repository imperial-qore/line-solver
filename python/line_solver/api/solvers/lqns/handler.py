...
Calls LQNS (Layered Queueing Network Solver) via subprocess.
...
def solver_lqns(
    ...
    # Execute command. Wall-clock budget: options.timeout (seconds) when finite,
    # else the 300s safety cap to avoid an unbounded hang.
    import math as _math
    _tmo = float(getattr(options, 'timeout', float('inf')))
    _sub_timeout = _tmo if _math.isfinite(_tmo) and _tmo > 0 else 300
    try:
        result = subprocess.run(
            cmd.split(),
            capture_output=True,
        ...
        raise RuntimeError(
            "LQNS exceeded the wall-clock time budget (timeout=%gs) and was terminated." % _sub_timeout)
