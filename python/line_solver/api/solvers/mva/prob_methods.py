...
def get_prob_aggr(
    ...
    # sn.state canonical form: list of per-stateful-node state vectors
    # (buffer|phases|vars). Decode the per-class counts via toMarginal, as
    # the CTMC probability accessors do. Legacy ndarray forms kept below.
    nir = np.zeros(K)
    if isinstance(sn.state, (list, tuple)):
        ...
        isf = ind
        if getattr(sn, 'nodeToStateful', None) is not None:
        ...
        nir = np.asarray(nir_m).reshape(-1)[:K].copy()
