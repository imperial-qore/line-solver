...
def npfqn_traffic_merge(
    ...
    if config_merge in ["default", "super"]:
        ...
        D0_result, D_list_result = mmap_super_safe(mmap_tuples, method="match")
        ...
        # Interposition (MATLAB npfqn_traffic_merge.m): fit each flow to a
        # M3PP(2,m) matching its exact per-class count-variance deltas, then
        # interleave the fitted processes.
        from ...lib.m3a import m3pp2m_fitc_theoretical, m3pp2m_interleave
        FLOW = []
        feasible = True
        for mm in valid_mmaps:
            fit = m3pp2m_fitc_theoretical(mm, 'exact_delta', 1.0, 1e6)
            # interleave requires a genuine 2-phase MMPP per flow; a flow that
            # fits to a marked-Poisson (1-phase) process has no burstiness to
            # interpose, so it cannot participate in the interleaving.
            if (not fit) or np.asarray(fit[0]).shape != (2, 2):
                feasible = False
                break
            FLOW.append(fit)
        if feasible and len(FLOW) >= 2:
            # m3pp2m_interleave already returns a valid, normalized MMAP in the
            # m3a convention [D0, D1_total, D1_1, ..., D1_M]: its D0 diagonal is
            # set so the generator rows sum to zero (identical to MATLAB
            # m3pp2m_interleave). Return it directly rather than routing it
            # through the shared compress/normalize tail.
            return m3pp2m_interleave(FLOW)
        # A per-flow M3PP fit degenerated / was infeasible; superposition is the
        # correct merge for non-bursty flows.
        warnings.warn(
            "npfqn_traffic_merge: 'interpos' fit degenerate for a flow; "
            "using 'super' merge instead.",
            RuntimeWarning
        )
        D0_result, D_list_result = mmap_super_safe(mmap_tuples, method="match")
...
def npfqn_traffic_split_cs(
    ...
    # Per-class arrival matrices live at M3A indices 2..R+1 (index 1 is the
    # aggregate D1), mirroring MATLAB MMAP{2+r}
    D_arrivals = []
    for r in range(R):
        if r + 2 < len(MMAP_input):
            D_arrivals.append(np.asarray(MMAP_input[r + 2], dtype=np.float64))
        else:
    ...
    for jst in range(M):
        ...
        for s in range(R):
            ...
            # the split flow moves OUT of the hidden part D0 (initialized to
            # D0 + D1 with all arrivals folded in), mirroring MATLAB
            # npfqn_traffic_split_cs
            D0_split = D0_split - D_s

        # Normalize. mmap_normalize consumes the full tail [D1, Dc1..DcR] with
        # the aggregate first, so the aggregate must lead the per-class list;
        # passing the per-class matrices alone would make it read Dc1 as the
        # aggregate and drop DcR, zeroing a single-class flow outright.
        # Mirrors MATLAB npfqn_traffic_split_cs, which normalizes the whole
        # {D0, D1, Dc1..DcR} cell.
        try:
            D0_norm, D_list_norm = mmap_normalize(D0_split, [D1_split] + D_list_split)
