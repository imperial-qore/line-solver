...
# exp(-x) underflows to exactly 0 for x > 745.13; beyond that softmin is min.
_SOFTMIN_MAX_EXPONENT = 745.0
...
def softmin(a: float, b: float, alpha: float = 20.0) -> float:
    ...
    gap = hi - lo
    if not gap < _SOFTMIN_MAX_EXPONENT / alpha:
        return lo
    w = np.exp(-alpha * gap)
        ...
        if lldscaling is not None and smax > 0:
            if i < lldscaling.shape[0]:
                ...
                if np.ptp(row) > 0:  # ptp = max - min = range
                    ...
                    scale_val = float(np.interp(n[i], x_vals, row))
...
def pfqn_cdfun(nvec: np.ndarray, cdscaling: Optional[List] = None,
    ...
    evaluated at the per-class population vector nvec[i, :], for class
    r = class_idx.

    cdscaling[i] is a callable of the per-class population vector at station i.
    It may return either

    - a scalar, i.e. a chain-independent scaling beta_i(n) shared by every class
      (the common case, and the historical contract), or
    - an array of length R, i.e. the per-class scalings
      [beta_{i,1}(n), ..., beta_{i,R}(n)], of which element class_idx is taken.
    ...
    An empty (None) entry means station i declares no class dependence and is
    left at the neutral scaling 1.

    Args:
        nvec: Population state matrix (M x R) or vector (M,)
        cdscaling: List of class-dependence callables, one per station
        class_idx: 0-based class index selecting beta_{i,r} (default: 0)
...
def pfqn_jdfun(nvec: np.ndarray, jdscaling: Optional[List] = None,
               class_idx: int = 0) -> np.ndarray:
    """
    ...
    evaluated at the joint per-class population vector nvec[i, :], for class
    r = class_idx.

    jdscaling[i] is a callable of the joint per-class population vector at
    station i. It may return either

    - a scalar, i.e. a scaling eta_i(n) shared by every class (broadcast, as in
      the flagship min(ni[0], c)), or
    - an array of length R, i.e. per-class scalings [eta_{i,1}(n), ...,
      eta_{i,R}(n)], of which element class_idx is taken (Sauer's
      chain-dependent rate mu_{r,i}(n)).
    ...
    exactness/uniqueness guarantee. The numerical evaluation matches
    pfqn_cdfun; the distinction is semantic and carried by sn.jdscaling.

    An empty (None) entry means station i declares no joint dependence and is
    left at the neutral scaling 1.

    Args:
        nvec: Population state matrix (M x R) or vector (M,)
        jdscaling: List of joint-dependence callables, one per station
        class_idx: 0-based class index selecting eta_{i,r} (default: 0)
