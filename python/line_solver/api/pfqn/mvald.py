...
def pfqn_mvald(
    ...
    if num_states > MAX_STATES:
        raise MemoryError(
            f"State space too large for exact load-dependent MVA: {num_states} states "
    ...
    # Utilization from pi(0), per STATION. For a load-dependent station 1-P_j(0)
    # is a property of the station, not of a class, so it is not split per class.
    UN = 1 - pi[:, 0]
...
def pfqn_mvams(
    ...
    if max_S == 1:  # No multi-server nodes
        ...
        if np.any(np.isinf(N)):  # Open or mixed model
            ...
            N_int = N.astype(int)
            XN, QN, UN, CN_ld, lGN, _, _ = pfqn_mvald(L, N_int, Z, mu)
            ...
            for r in range(R):
                if N_int[r] > 0:
                    ...
                    CN[:, r] = L[:, r] * mi  # an absent class, as in pfqn_mva
