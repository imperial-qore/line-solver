...
Key functions:
    ...
    pfqn_ldmx_ec: Effective capacity computation
...
def pfqn_ldmx_ec(lam: np.ndarray, D: np.ndarray, mu: np.ndarray
...
def pfqn_mvaldmx(lam: np.ndarray, D: np.ndarray, N: np.ndarray,
    ...
    EC, E, Eprime = pfqn_ldmx_ec(lam, D, mu)[:3]
    ...
    while np.all(nvec >= 0):
        ...
        nnz = np.where(nvec > 0)[0]
        if nnz.size > 0:
            last_nnz = int(nnz[-1])
            if (np.sum(nvec[:last_nnz]) == np.sum(Nc[:last_nnz])
                    and np.sum(nvec[last_nnz + 1:]) == 0):
                xref = x[last_nnz, hnvec - 1]
                if xref > 0:
                    lGN -= np.log(xref)
    ...
    for r in openClasses:
        ...
        for ist in range(M):
            ...
            UN[ist, r] = 0.0
            for n in range(int(np.sum(Nc)) + 1):
                if n + 1 < E.shape[1] and E[ist, n + 1] > 0:
                    UN[ist, r] += lam[r] * Eprime[ist, n + 1] / E[ist, n + 1] * Pc[ist, n, hnvec - 1]
