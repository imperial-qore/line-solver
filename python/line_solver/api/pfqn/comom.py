        ...
        for Nr in range(1, int(N[r]) + 1):
            ...
            if Nr == 1:
                if r > len(zerothinktimes):
                    ...
                    hr = np.zeros(2 * rm)
                    hr[:rm - 1] = h[:rm - 1]
                    hr[rm:2 * rm - 1] = h[rm - 1:2 * (rm - 1)]
                    h = hr

                    # Update scalings
                    if nt > 0:
                        h[rm - 1] = h_1[0] / scale[nt - 1]
                        h[-1] = h_1[rm - 1] / scale[nt - 1]
                ...
                # Class-R PCs
                B2r = np.column_stack([m * L[0, r] * np.eye(rm), Z[r] * np.eye(rm)])
...
def pfqn_procomom(L: np.ndarray, N: np.ndarray, Z: np.ndarray = None,
    ...
    using the Probabilistic Class-Oriented Method of Moments. Uses matrix recursion
