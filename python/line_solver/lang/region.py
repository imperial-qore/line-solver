...
class Region:
    ...
    def __init__(self, nodes: List[Node], classes: List[JobClass]):
        ...
        self._class_size: Dict[JobClass, float] = {}
    ...
    def set_class_size(self, job_class: JobClass, size: float) -> 'Region':
        """
    ...
    def get_class_size(self, job_class: JobClass) -> float:
    ...
    def get_linear_constraints(self):
        """Get the linear admission constraints as the pair (A, b).

        Returns:
            A tuple ``(A, b)`` with the constraint matrix A (C x K) and the
            capacity vector b (C,); each entry is None if not set.
        """
        return self._constraint_A, self._constraint_b

    # MATLAB-style alias
    def getLinearConstraints(self):
        """MATLAB-compatible alias for get_linear_constraints()."""
