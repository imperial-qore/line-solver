...
class RoutingMatrix:
    ...
    def remove_job_class(self, jobclass: JobClass) -> None:
        """
        Drop every route that references a job class.

        Routes are keyed by (source class, destination class), so a class
        removed from the model must be purged from both positions, including
        the pre-ClassSwitch snapshot in _original_routes. Leaving a stale entry
        makes toMatrix() index the rebuilt (smaller) rt matrix with the old
        class index. Called by Network.remove_class.

        Args:
            jobclass: the job class being removed from the model
        """
        for routes in (self._routes, self._original_routes):
            if not routes:
                continue
            for key in [k for k in list(routes.keys())
                        if k[0] is jobclass or k[1] is jobclass]:
                del routes[key]
        self._matrix = None
        self._nodes_cache = None
        self._java_obj = None
