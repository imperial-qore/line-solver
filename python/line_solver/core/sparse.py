...
class SparseMatrix:
    ...
    def solve(self, b: np.ndarray) -> np.ndarray:
        ...
        """
        A = self._mat.tocsc()
        # see _kb/05-solvers-overview.md (LineOpt: core/sparse.py linear-solve dispatch) for rationale
        from ..api.mc.ctmc import GMRES_MIN_STATES
        if A.shape[0] == A.shape[1] and A.shape[0] > GMRES_MIN_STATES:
            from ..api.mc.gmres import ctmc_gmres
