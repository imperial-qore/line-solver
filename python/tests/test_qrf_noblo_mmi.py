...
def _erlang2(rate):
    """Erlang-2 of the given mean rate, as an ordinary (D0, D1) pair.

    The phase-advance rate sits in D0's UPPER triangle, where an Erlang's does.
    Until this fix the helper transposed D0 instead, compensating for
    extract_mu_v_from_maps writing v[i,k,h] while mu stayed untransposed; both
    are gone now. The references below did not move: they always described the
    genuine Erlang-2 that the fixed extraction builds from a genuine D0.
    """
