...
from line_solver.api.qsys import qsys_geogeo1, qsys_geoxgeo1, qsys_geoxgeo1_moments
...
def test_conventions_differ_by_exactly_one_slot(a, s):
    las = qsys_geogeo1(a, s, 'LAS_DA')
    eas = qsys_geogeo1(a, s, 'EAS')
...
def test_las_da_is_the_default():
    default = qsys_geogeo1(0.2, 0.5)
    assert default['convention'] == 'LAS_DA'
    assert abs(default['meanSojournTime']
               - qsys_geogeo1(0.2, 0.5, 'LAS_DA')['meanSojournTime']) < TOL
...
def test_vanishing_load_leaves_only_the_service_time():
    s = 0.4
    las = qsys_geogeo1(1e-9, s, 'LAS_DA')
    eas = qsys_geogeo1(1e-9, s, 'EAS')
...
def test_las_da_matches_brute_force_chain(a, s):
    ...
    _assert_chain_agrees(pi, qsys_geogeo1(a, s, 'LAS_DA'),
                         "LAS-DA a=%s s=%s" % (a, s))
...
def test_eas_matches_brute_force_chain(a, s):
    ...
    _assert_chain_agrees(pi, qsys_geogeo1(a, s, 'EAS'), "EAS a=%s s=%s" % (a, s))
...
def test_degenerate_batch_reproduces_geogeo1(a, s, convention):
    single = qsys_geogeo1(a, s, convention)
    batch = qsys_geoxgeo1(a, 1.0, s, convention)
...
def test_batch_littles_law(a, beta, s, convention):
    r = qsys_geoxgeo1(a, beta, s, convention)
...
def test_batch_conventions_differ_by_one_slot(a, beta, s):
    las = qsys_geoxgeo1(a, beta, s, 'LAS_DA')
    eas = qsys_geoxgeo1(a, beta, s, 'EAS')
...
def test_batching_increases_congestion_at_equal_load():
    ...
    for beta in (1.0, 0.8, 0.5, 0.25):
        r = qsys_geoxgeo1(lambda_val * beta, beta, s)
...
def test_invalid_arguments_raise():
    with pytest.raises(ValueError):
        qsys_geogeo1(0.5, 0.5)          # a = s is not stable
    with pytest.raises(ValueError):
        qsys_geogeo1(0.7, 0.5)          # a > s is not stable
    with pytest.raises(ValueError):
        qsys_geogeo1(0.0, 0.5)          # a must be positive
    with pytest.raises(ValueError):
        qsys_geogeo1(0.2, 1.5)          # s must not exceed one
    with pytest.raises(ValueError):
        qsys_geogeo1(0.2, 0.5, 'MIDSLOT')
    with pytest.raises(ValueError):
        qsys_geogeo1(0.2, 0.5)['pmf'](-1)

    with pytest.raises(ValueError):
        qsys_geoxgeo1(0.5, 0.5, 0.9)    # lambda = 1.0 > s
    with pytest.raises(ValueError):
        qsys_geoxgeo1(0.1, 1.5, 0.9)    # beta must not exceed one
    with pytest.raises(ValueError):
        qsys_geoxgeo1_moments(0.1, 0.5, 0.0, 0.9)   # E[X] < 1
    with pytest.raises(ValueError):
        qsys_geoxgeo1_moments(0.1, 3.0, 0.0, 0.9)   # below E[X]^2-E[X]
    with pytest.raises(ValueError):
        qsys_geoxgeo1(0.1, 0.5, 0.9)['pgf'](1.5, 1.0)
