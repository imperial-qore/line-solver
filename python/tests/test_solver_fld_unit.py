...
class SimpleNetworkStruct:
    ...
    def __init__(self, nstations=2, nclasses=1):
        ...
        # The default-method resolution runs sn_nonmarkov_toph, which reads
        # procid; NetworkStruct always carries the field, so a mock without it
        # fails with AttributeError rather than exercising the resolution.
        self.procid = None
        self.proc = None
...
class TestSolverFLDOptions(unittest.TestCase):
    ...
    def test_default_options(self):
        ...
        # verbose FOLLOWS THE GLOBAL LEVEL (constants.default_verbose): it is
        # True unless GlobalConstants is SILENT, as in MATLAB and Java. A bare
        # assertFalse here passed only when an earlier test in the session had
        # already silenced the global.
        self.assertEqual(opts.verbose,
                         GlobalConstants.getVerbose() != VerboseLevel.SILENT)
    ...
    def test_custom_options(self):
        ...
        self.assertEqual(opts.softmin_alpha, 50.0)
...
class TestSolverFLDStaticMethods(unittest.TestCase):
    ...
    def test_get_feature_set(self):
        """Test getFeatureSet returns the canonical feature names."""
        ...
        self.assertIn('SchedStrategy_PS', features)
        self.assertIn('SchedStrategy_LCFSPR', features)
        self.assertIn('Coxian', features)
...
class TestResultAccessorMethods(unittest.TestCase):
    ...
    def test_get_avg_qlen(self):
        ...
        expected = np.sum(self.result.QN, axis=1)  # total per station (sum over classes)
    ...
    def test_get_avg_util(self):
        ...
        expected = np.sum(self.result.UN, axis=1)  # total per station (sum over classes)
    ...
    def test_get_avg_respt(self):
        ...
        # getAvgRespT returns per-station, per-class response times (response
        # time is not additive across classes, so it is not summed).
        np.testing.assert_array_almost_equal(np.asarray(rt), self.result.RN)
...
class TestMethodResolution(unittest.TestCase):
    ...
    def test_resolve_default_method(self):
        """'default' resolves to 'minnormal' wherever the closure applies.

        The second-order closure is preferred over the first-order methods
        since it dominates them on every family measured against exact CTMC;
        'matrix' is what remains when minnormal is declined. MATLAB reports
        default/minnormal on the same M/M/1, so this pins the shared rule and
        not a Python-only choice."""
        sn = SimpleNetworkStruct(nstations=2, nclasses=1)
        solver = SolverFLD(sn, method='default')

        resolved = solver._resolve_method()
        self.assertEqual(resolved, 'minnormal')
