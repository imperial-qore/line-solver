...
from line_solver.solvers.solver_ag import SolverAG
...
class TestM_M_1Network(unittest.TestCase):
    ...
    def test_inap_m_m_1(self):
        # RCAT moved out of SolverMAM into SolverAG, and this case with it.
        solver = SolverAG(self.model, method='inap')
        solver.runAnalyzer()
        self.assertIsNotNone(solver.result)
...
class TestTandemNetwork(unittest.TestCase):
    ...
    def test_method_consistency_tandem(self):
        # 'inap' is SolverAG's arm since RCAT moved, so the loop crosses
        # solvers rather than methods; what it checks is unchanged.
        for method in ['dec.source', 'mna_open', 'inap']:
            cls = SolverAG if method == 'inap' else SolverMAM
            solver = cls(self.model, method=method)
            solver.runAnalyzer()
...
class TestStaticMethods(unittest.TestCase):
    ...
    def test_list_valid_methods(self):
        ...
        # SolverMAM.m verbatim; 'mna_open'/'mna_closed' are the open and closed
        # arms 'mna' resolves to internally, not names a caller selects.
        for method in ['default', 'dec.source', 'dec.mmap', 'dec.poisson',
                       'mna', 'dec.source.mmap', 'ldqbd', 'bgchain', 'retrial']:
            self.assertIn(method, methods)
        # The RCAT names moved to SolverAG and MAM must not re-advertise a
        # method it no longer dispatches: it redirects a caller by name.
        for method in ['inap', 'inapplus', 'inapinf', 'exact']:
            self.assertNotIn(method, methods)
            self.assertIn(method, SolverAG.listValidMethods())

    def test_moved_methods_redirect_to_solver_ag(self):
        """Dropping a name from listValidMethods is what makes the solver
        refuse it and also what loses the forwarding address, so the refusal has
        to carry the address. runAnalyzerChecks sits above every dispatcher and
        used to outrank the redirect silently, reporting the flat "method is
        unsupported by this solver"; it asks unsupportedMethodReason first now.
        """
        for moved in ['inap', 'inapplus', 'inapinf', 'exact']:
            with self.assertRaises(Exception) as ctx:
                SolverMAM(tandem_open(), method=moved).runAnalyzer()
            msg = str(ctx.exception)
            self.assertIn('moved to SolverAG', msg)
            self.assertIn("SolverAG(model, '%s')" % moved, msg)
...
class TestPerformanceMetrics(unittest.TestCase):
    ...
    def test_utilization_bounds(self):
        ...
        for method in ['dec.source', 'mna_open', 'inap']:
            cls = SolverAG if method == 'inap' else SolverMAM
            solver = cls(model, method=method)
            solver.runAnalyzer()
            UN = np.asarray(solver.result.UN)
