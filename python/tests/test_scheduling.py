...
# A 10-task DAG in the repository, so the GML reader is exercised on every run.
# It used to be an absolute path into a private checkout, which meant the two
# tests below skipped everywhere -- including on the cluster, where a skip is
# indistinguishable from a pass in the phase summary.
GML_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        'fixtures', 'dag10.gml')
...
class TestGMLImport(unittest.TestCase):
    ...
    def test_load_gml(self):
        ...
        self.assertEqual(len(wf.getDependencies()), 13)
        self.assertEqual(wf.getTask('T0').getComputation(), 14.0)
        self.assertTrue(wf.validate())
    ...
    def test_schedule_gml_workflow(self):
        ...
        result = HEFT(wf, machines).schedule()
        self.assertGreater(result.getMakespan(), 0)
        self.assertEqual(len(result.getScheduleTable()), wf.getNumberOfTasks())
