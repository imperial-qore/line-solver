...
def test_infer_gibbs():
    ...
    samplePath = solver_ssa.sample(node[1], 10000)
    ...
    tput_samples = np.ones(n) / np.mean(np.diff(dep_times))
    ...
    assert abs(est - trueDemand) / trueDemand < 0.10, \
        f'Gibbs: estimated {est:.4f}, relative error {100 * abs(est - trueDemand) / trueDemand:.2f}% exceeds 10%'
