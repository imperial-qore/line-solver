...
# Every helper below puts the Source at station 0 and the first Queue at
# station 1, and a fluid result spans ALL stations -- so a queueing measure is
# read off row QROW, never row 0.
QROW = 1
...
class TestMatrixMethodSolver(unittest.TestCase):
    ...
    def test_default_method_resolves_to_minnormal(self):
        # MATLAB reports default/minnormal on this same M/M/1: the second-order
        # closure is preferred wherever it applies, and 'matrix' is what remains
        # when it is declined. Mirrors fluid_resolve_default_method.
        solver = SolverFLD(fld_mm1(0.5, 1.0), method='default')
        solver.runAnalyzer()
        self.assertEqual(solver.result.method, 'default/minnormal')
...
class TestMFQMethodSolver(unittest.TestCase):
    ...
    def test_mfq_mm1_result(self):
        # MFQ IS AN M/M/c SOLVER THAT WRITES INTO THE FULL STATION TABLE. It
        # answers on the QUEUE row of an (nstations, K) result, not on a
        # single-queue (1, K) one -- the Source is row 0 and stays empty, as it
        # does for every other fluid method and in MATLAB, the JAR and C++.
        solver = SolverFLD(fld_mm1(0.5, 1.0), method='mfq')
        result = solver.runAnalyzer()
        qn_queue = np.asarray(result.result.QN)[QROW, 0]
    ...
    def test_mfq_multiclass(self):
        ...
        QN = np.asarray(result.result.QN)
        self.assertEqual(QN.shape, (2, 2))          # Source + Queue1, two classes
        np.testing.assert_allclose(QN[QROW, :], [1.0, 1.0], rtol=1e-9)

    def test_mfq_falls_back_on_multiqueue(self):
        # MFQ IS SINGLE-QUEUE AND FALLS BACK RATHER THAN REFUSING, which is what
        # solver_fluid_analyzer.m does ("MFQ not applicable: ... Falling back to
        # matrix method") and what the JAR and C++ do. Raising here made 'mfq'
        # and its aliases 'butools'/'aoi' refuse every multi-station model the
        # other three codebases answer.
        result = SolverFLD(fld_tandem(0.5, (1.0, 1.0)), method='mfq').runAnalyzer()
        self.assertEqual(result.result.method, 'matrix')
        self.assertTrue(np.all(np.isfinite(np.asarray(result.result.QN))))
