...
class TestSolverMAMConfiguration(unittest.TestCase):
    ...
    def test_list_valid_methods(self):
        ...
        # SolverMAM.m verbatim. 'mna_open', 'mna_closed' and 'fj' are INTERNAL
        # resolution targets of the topology router, not names a caller selects
        # -- the reference resolves 'mna' to the open or closed arm itself and
        # reaches the fork-join route from 'default' -- so they are dispatched
        # but not advertised, exactly as the reference does not advertise its own
        # 'qiu' and 'reneging' labels.
        self.assertIn('default', methods)
        self.assertIn('dec.source', methods)
        self.assertIn('mna', methods)
        self.assertIn('dec.source.mmap', methods)
        self.assertNotIn('mna_open', methods)
        self.assertNotIn('fj', methods)
        # The RCAT names moved to SolverAG; MAM redirects a caller by name
        # rather than re-advertising a method it no longer dispatches.
