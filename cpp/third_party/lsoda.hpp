 ...
 *
 * Source: https://github.com/dilawar/libsoda-cxx (src/LSODA.h + src/LSODA.cpp),
 * MIT License, Copyright (c) 2018 Dilawar Singh.
 *
 * Provenance, and why this is the right port for parity. LSODA is Petzold and
 * Hindmarsh's ODEPACK solver; the C translation everything here descends from
 * is Hon Wah Tam's and Yu Feng's, distributed as ccl.net `kinetics2`. The
 * three ports LINE uses are SIBLINGS off that same C code, not derivatives of
 * one another:
 *
 *   this file  ccl.net kinetics2 -> Heng Li's merged `lsoda.c` -> libsoda-cxx
 *   the JAR    ccl.net kinetics2 -> imperial-qore/lsoda-java (BSD-3), which
 *              `jline.solvers.fluid.LSODAExt` drives
 *   Python     the same C lineage -> sdwfrost/liblsoda -> `lib/lsoda.py`
 *
 * Checked, not assumed: the order-dependent stability constants `sm1`, the
 * order caps `mord = {12, 5}` and the machine epsilon are identical in all
 * three. Running one algorithm with one set of coefficients is what makes the
 * fluid results line up across codebases, rather than tuning to match.
 *
 * Adapted for this tree, mechanically and minimally:
 *   - LSODA.h and LSODA.cpp merged into this single header; the 28 out-of-line
 *     member definitions are marked `inline`.
 *   - everything is wrapped in `line::lsoda_impl`, so the upstream
 *     `using namespace std;` cannot reach the rest of the project.
 *   - four read-only counter accessors (get_nst/get_nfe/get_nje/get_mused)
 *     were added; the private members they expose are unchanged.
 *   - the upstream `ETA` macro is renamed LINE_LSODA_ETA and #undef'd at the
 *     end, so no macro escapes this header.
 * The numerical code itself is untouched.
 *
 * The LINE-facing wrapper is line/util/lsoda.h; callers use that, not this.
 */
#ifndef LINE_THIRD_PARTY_LSODA_HPP
#define LINE_THIRD_PARTY_LSODA_HPP
...
namespace line {
namespace lsoda_impl {

using namespace std;

#define LINE_LSODA_ETA 2.2204460492503131e-16

/**
 * Diagnostic sink for the messages this integrator used to write straight to
 * cerr. Every one of them accompanies a negative istate, which line/util/lsoda.h
 * already reports to the caller as `success == false`, so the raw write was a
 * duplicate that interleaved with whatever the caller was printing. It is off by
 * default; set lsoda_diagnostics(&std::cerr) to get the running commentary back.
 */
inline std::ostream*& lsoda_diagnostic_stream() {
    static std::ostream* s = nullptr;
    return s;
}
inline void lsoda_diagnostics(std::ostream* s) { lsoda_diagnostic_stream() = s; }
inline std::ostream& lsoda_diag() {
    static std::ostream discard(nullptr);
    std::ostream* s = lsoda_diagnostic_stream();
    return s ? *s : discard;
}
...
private:
    ...
    size_t meth_;
    // LINE fork: pin the BDF half, see set_force_stiff below
    bool force_stiff_ = false;
...
public:
    ...
    // ADAPTATION (not upstream): read-only accessors for the step and
    // evaluation counters and for the method last used, which are private
    // above. line/util/lsoda.h reports them; nothing here is modified.
    size_t get_nst() const { return nst; }
    size_t get_nfe() const { return nfe; }
    size_t get_nje() const { return nje; }
    size_t get_mused() const { return mused; }

    // ADAPTATION (not upstream): set the error weights before calling lsoda()
    // directly. Upstream only ever fills rtol_/atol_ from lsoda_update(), which
    // is scalar-only and fills them once; a caller that drives lsoda() itself
    // would otherwise run with both vectors empty. Per-component tolerances are
    // what the reference test vectors use (Robertson runs atol per component),
    // so both forms are exposed here. Vectors are 1-based, slot 0 unused, and
    // itol follows ODEPACK: 1 both scalar, 2 atol per component, 3 rtol per
    // component, 4 both per component.
    void set_tolerances(const vector<double>& rtol, const vector<double>& atol, int itol)
    {
        rtol_ = rtol;
        atol_ = atol;
        itol_ = itol;
    ...
    // LINE fork (not upstream): start on BDF and stay there, never switching to
    // Adams. The Adams half loses its stability bound at a fixed point -- the
    // corrector converges on the `del <= 100*pnorm*ETA` branch before `pdest` is
    // ever formed, so `scaleh`'s pdh guard never binds and h runs up to hmax,
    // and the trajectory wanders around the fixed point instead of settling on
    // it. Same pin as LSODA.setForceStiff(true) in the JAR and forceStiff in
    // lsoda_matlab.m / lib/lsoda.py. Must be set BEFORE the first lsoda() call,
    // which is where meth_, miter and maxord are initialised.
    void set_force_stiff(bool value) { force_stiff_ = value; }
    bool get_force_stiff() const { return force_stiff_; }
};
...
inline bool LSODA::abs_compare(double a, double b)
{
    return (std::abs(a) < std::abs(b));
}

/* Purpose : Find largest component of double vector dx */
inline size_t LSODA::idamax1(
 ...
 * dx = da * dx
 */
inline void LSODA::dscal1(const double da, vector<double>& dx, const size_t n,
...
inline double LSODA::ddot1(const vector<double>& a, const vector<double>& b,
...
inline void LSODA::daxpy1(const double da, const vector<double>& dx,
...
inline void LSODA::dgesl(const vector<vector<double>>& a, const size_t n,
    vector<int>& ipvt, vector<double>& b, const size_t job)
{
    size_t k, j;
    double t;
...
inline void LSODA::dgefa(vector<vector<double>>& a, const size_t n, vector<int>& ipvt,
    size_t* const info)
{
    size_t j = 0, k = 0, i = 0;
    double t = 0.0;
...
inline void LSODA::terminate(int* istate)
{
    if (illin == 5)
        lsoda_diag() << "[lsoda] repeated occurrence of illegal input. run aborted.. "
...
inline void LSODA::terminate2(vector<double>& y, double* t)
{
    for (size_t i = 1; i <= n; i++)
        y[i] = yh_[1][i];
    *t = tn_;
...
inline void LSODA::successreturn(vector<double>& y, double* t, int itask, int ihit,
    double tcrit, int* istate)
{
    for (size_t i = 1; i <= n; i++)
        y[i] = yh_[1][i];
...
c-----------------------------------------------------------------------
*/
inline void LSODA::lsoda(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double>& y,
    double* t, double tout, int itask, int* istate, int iopt, int jt,
    array<int, 7>& iworks, array<double, 4>& rworks, void* _data)
{
    // A CONTINUATION call (istate > 1) with itask = 1 may name a `tout` BEHIND
    // `*t`: block d interpolates it out of the step history with `intdy` and
    // returns, which is how `LsodaStepper::settle_at_end` lands back on the
    // requested instant after itask = 2 stepped past it. Only a call that has
    // to ADVANCE the integration needs `tout` ahead of `*t`.
    assert(tout > *t || (*istate > 1 && itask == 1));
    ...
    */
    ...
    if (*istate < 1 || *istate > 3) {
        ...
        lsoda_diag() << "[lsoda] illegal istate = " << *istate << endl;
        terminate(istate);
    ...
    */
    ...
    if (*istate == 1 || *istate == 3) {
        ntrep = 0;
        if (neq <= 0) {
            lsoda_diag() << "[lsoda] neq = " << neq << " is less than 1." << endl;
            terminate(istate);
            return;
        }
        if (*istate == 3 && neq > n) {
            lsoda_diag() << "[lsoda] istate = 3 and neq increased" << endl;
            terminate(istate);
            return;
        }
        n = neq;
        if (itol_ < 1 || itol_ > 4) {
            lsoda_diag() << "[lsoda] itol = " << itol_ << " illegal" << endl;
            terminate(istate);
            return;
        }
        if (iopt < 0 || iopt > 1) {
            lsoda_diag() << "[lsoda] iopt = " << iopt << " illegal" << endl;
            terminate(istate);
            return;
        }
        if (jt == 3 || jt < 1 || jt > 5) {
            lsoda_diag() << "[lsoda] jt = " << jt << " illegal" << endl;
            terminate(istate);
            return;
        }
        jtyp = jt;
        if (jt > 2) {
            ...
            mu = iworks[1];
            if (ml >= n) {
                lsoda_diag() << "[lsoda] ml = " << ml << " not between 1 and neq"
                     << endl;
                terminate(istate);
                return;
            }
            if (mu >= n) {
                lsoda_diag() << "[lsoda] mu = " << mu << " not between 1 and neq"
                     << endl;
                terminate(istate);
                return;
            }
        ...
        else /* if ( iopt = 1 )  */
        {
            ixpr = iworks[2];
            if (ixpr > 1) {
                lsoda_diag() << "[lsoda] ixpr = " << ixpr << " is illegal" << endl;
                terminate(istate);
                return;
            }
            ...
            if (*istate == 1) {
                ...
                if ((tout - *t) * h0 < 0.) {
                    lsoda_diag() << "[lsoda] tout = " << tout << " behind t = " << *t
    ...
    */
    if (*istate == 1) {
        ...
        */
        sqrteta = sqrt(LINE_LSODA_ETA);
        meth_ = force_stiff_ ? 2 : 1;  // LINE fork
    ...
    */
    if (*istate == 1) {
        tn_ = *t;
        tsw = *t;
        maxord = force_stiff_ ? mxords : mxordn;  // LINE fork
        if (itask == 4 || itask == 5) {
            tcrit = rworks[0];
        ...
        nqu = 0;
        mused = 0;
        miter = force_stiff_ ? 2 : 0;  // LINE fork: BDF corrects with a Jacobian
        ccmax = 0.3;
        maxcor = 3;
        msbp = 20;
        mxncf = 10;
        ...
        for (size_t i = 1; i <= n; i++) {
            if (ewt[i] <= 0.) {
                lsoda_diag() << "[lsoda] ewt[" << i << "] = " << ewt[i] << " <= 0.\n"
                     << endl;
                terminate2(y, t);
                return;
            }
        ...
        /*
           ...
           so as to be between 100*LINE_LSODA_ETA and 0.001.
        ...
        */
        if (h0 == 0.) {
            ...
            tol = max(tol, 100. * LINE_LSODA_ETA);
            tol = min(tol, 0.001);
    ...
    */
    if (*istate == 2 || *istate == 3) {
        ...
        case 2:
            break;
        case 3:
            tp = tn_ - hu * (1. + 100. * LINE_LSODA_ETA);
            if ((tp - tout) * h_ > 0.) {
                fprintf(stderr,
        ...
        case 5:
            ...
            ihit = fabs(tn_ - tcrit) <= (100. * LINE_LSODA_ETA * hmx);
            if (ihit) {
                *t = tcrit;
                successreturn(y, t, itask, ihit, tcrit, istate);
                return;
            }
            tnext = tn_ + h_ * (1. + 4. * LINE_LSODA_ETA);
            if ((tnext - tcrit) * h_ <= 0.)
                break;
            h_ = (tcrit - tn_) * (1. - 4. * LINE_LSODA_ETA);
            if (*istate == 2)
                jstart = -2;
            break;
        } /* end switch   */
    ...
    */
    while (1) {
        ...
        tolsf = LINE_LSODA_ETA * vmnorm(n, yh_[1], ewt);
        if (tolsf > 0.01) {
            tolsf = tolsf * 200.;
            if (nst == 0) {
                fprintf(stderr,
        ...
        if (kflag == 0) {
            ...
            */
            ...
            if (meth_ != mused) {
                ...
                if (ixpr) {
                    if (meth_ == 2)
                        lsoda_diag() << "[lsoda] a switch to the stiff method has "
                                "occurred "
                             << endl;
                    if (meth_ == 1)
                        lsoda_diag() << "[lsoda] a switch to the nonstiff method has "
                                "occurred"
                             << endl;
                }
            } /* end if ( meth_ != mused )   */
    ...
    */
    if (jstart == 0) {
        ...
        */
        ...
        // cm1/cm2 need both tables, and the loop above leaves them on method 1,
        // which is what upstream assumes at the start. LINE fork: under
        // force_stiff the start is meth_ = 2 and the tables must match it.
        if (meth_ == 2)
            cfode(2);
        resetcoeff();
    } /* end if ( jstart == 0 )   */
...
inline void LSODA::ewset(const vector<double>& ycur)
{
    switch (itol_) {
    case 1:
        for (size_t i = 1; i <= n; i++)
...
*/
inline void LSODA::intdy(double t, int k, vector<double>& dky, int* iflag)
{
    int ic, jp1 = 0;
    double c, r, s, tp;
    ...
    *iflag = 0;
    ...
    tp = tn_ - hu - 100. * LINE_LSODA_ETA * (tn_ + hu);
    if ((t - tp) * (t - tn_) > 0.) {
        fprintf(stderr,
...
inline void LSODA::cfode(int meth_)
{
    int i, nq, nqm1, nqp1;
...
inline void LSODA::scaleh(double* rh, double* pdh)
{
    double r;
    /*
    ...
    const size_t neq, vector<double>& y, LSODA_ODE_SYSTEM_TYPE f, void* _data)
{
    ...
    */
    ...
    if (miter == 2) {
        ...
        r0 = 1000. * fabs(h_) * LINE_LSODA_ETA * ((double)n) * fac;
        if (r0 == 0.)
            r0 = 1.;
        // LINE fork: ODEPACK's only guard on a degenerate increment is
        // "if (r0 == 0) r0 = 1", which tests for an EXACT zero. Started at a
        // fixed point, ||f|| is at roundoff rather than zero, so r0 underflows
        // (measured 1.8e-27) instead of tripping the guard, and any coordinate
        // with y_j = 0 or |y_j| ~ 1e-9 -- what an LQN layer's immediate (1e8)
        // phases hold -- is differenced with an increment of 1e-31..1e-17. The
        // column is then rounding noise divided by that increment, i.e. entries
        // of order 1e15; the Newton matrix is meaningless, the corrector
        // diverges, h collapses 0.25 per failure and the step count explodes
        // (1.67e6 steps for a window whose exact answer is its own start). The
        // floor is MATLAB NUMJAC's increment, sqrt(eps)*max(|y_j|, atol/rtol),
        // which is what keeps ode15s off this failure mode. Same fork as
        // jar/src/main/java/odesolver/LSODA.java; see _kb/06-solver-catalog.md.
        //
        // TWO CORRECTIONS TO THAT FLOOR, both measured on the C LSODA reference
        // values in cpp/tests/test_lsoda.cpp, which it broke:
        //
        //  1. IT APPLIES ONLY WHERE ODEPACK'S INCREMENT HAS ACTUALLY
        //     DEGENERATED, which is what `fac` reports: `vmnorm(f, ewt)` is the
        //     RHS measured in units of the error the integrator tolerates, so
        //     `fac` at roundoff IS the fixed-point start the fork is about, and
        //     `r0` underflows with it. Applied unconditionally it also replaced
        //     the increment of a perfectly well-scaled component: HIRES has
        //     atol/rtol = 1e-2 while y8 ~ 1.3e-4, so every column was
        //     differenced at sqrt(eps)*1e-2 = 1.5e-10 rather than the component's
        //     own 1.9e-12, and y8(200) moved 6e-4 relative -- 7.7e-8 against a
        //     reference the unforked code matches to 1e-9.
        //  2. THE THRESHOLD IS PER COMPONENT, as NUMJAC's is. Reducing a vector
        //     atol to its max (over the min rtol) lets the LARGEST component's
        //     scale set the increment of the SMALLEST: on Robertson, atol =
        //     {1e-6, 1e-10, 1e-6} with rtol 1e-4 floored the middle species --
        //     1e-8 at its peak and 5.7e-14 at the end -- at 1.5e-10, thousands
        //     of times its own value, so its Jacobian column was again noise and
        //     the integration diverged by seven orders of magnitude.
        //  3. THE PIN NEEDS IT UNCONDITIONALLY. force_stiff_ corrects with a
        //     Jacobian from the FIRST step, where the auto-switcher would still
        //     be on Adams and would need none, so a component sitting at exactly
        //     zero under a tight atol is differenced at ~1e-19 while `fac` is
        //     large and the gate above stays shut. Robertson from (1, 0, 0) with
        //     atol {1e-6, 1e-10, 1e-6} diverges that way, and flips on a last-bit
        //     change to the right-hand side. Under the pin the floor therefore
        //     applies on every step, which is what ode15s -- also a BDF method
        //     taking a Jacobian at t0 -- does through numjac. It costs nothing
        //     where the scaling is sane: HIRES under the pin moves 3.38e-5 ->
        //     3.42e-5 relative to the same reference row.
        const bool degenerate_rhs = !(fac > sqrteta) || force_stiff_;
        for (j = 1; j <= n; j++) {
            yj = y[j];
            r = max(sqrteta * fabs(yj), r0 / ewt[j]);
            if (degenerate_rhs) {
                const double at_j =
                    (itol_ == 2 || itol_ == 4) && j < atol_.size() ? atol_[j] : atol_[1];
                const double rt_j = itol_ >= 3 && j < rtol_.size() ? rtol_[j] : rtol_[1];
                double jac_threshold = (rt_j > 0.0) ? at_j / rt_j : at_j;
                if (!(jac_threshold > 0.0) || std::isinf(jac_threshold))
                    jac_threshold = 0.0; // no usable scale: keep ODEPACK's increment
                r = max(r, sqrteta * max(fabs(yj), jac_threshold));
            }
            y[j] += r;
            fac = -hl0 / r;
...
*/
inline void LSODA::correction(const size_t neq, vector<double>& y,
    LSODA_ODE_SYSTEM_TYPE f, size_t* corflag, double pnorm, double* del,
    double* delp, double* told, size_t* ncf, double* rh, size_t* m, void* _data)
{
    ...
    */
    while (1) {
        ...
        */
        if (*del <= 100. * pnorm * LINE_LSODA_ETA)
            break;
        if (*m != 0 || meth_ != 1) {
            if (*m != 0) {
                rm = 1024.0;
...
inline void LSODA::corfailure(double* told, double* rh, size_t* ncf, size_t* corflag)
{
    // Upstream bumps the POINTER here, so the counter never moves and the
    // `*ncf == mxncf` bail-out below reads one slot past the caller's frame.
    (*ncf)++;
    rmax = 2.;
    tn_ = *told;
    for (size_t j = nq; j >= 1; j--)
...
/*
   ...
   y = the right-hand side vector on input, and the solution vector
       on output.
*/
inline void LSODA::solsy(vector<double>& y)
{
    iersl = 0;
    if (miter != 2) {
...
inline void LSODA::methodswitch(double dsm, double pnorm, double* pdh, double* rh)
{
    int lm1, lm1p1, lm2, lm2p1, nqm1, nqm2;
    double rh1, rh2, rh1it, exm2, dm2, exm1, dm1, alpha, exsm;

    if (force_stiff_)  // LINE fork: the pin, see set_force_stiff
        return;
    ...
    */
    if (meth_ == 1) {
        if (nq > 5)
            return;
        if (dsm <= (100. * pnorm * LINE_LSODA_ETA) || pdest == 0.) {
            if (irflag == 0)
                return;
            rh2 = 2.;
            nqm2 = min(nq, mxords);
    ...
    *pdh = pdnorm * fabs(h_);
    ...
    alpha = max(0.001, rh1);
    dm1 *= pow(alpha, exm1);
    if (dm1 <= 1000. * LINE_LSODA_ETA * pnorm)
        return;
    /*
...
inline void LSODA::_freevectors(void)
{
 ...
 */
/* ----------------------------------------------------------------------------*/
inline void LSODA::lsoda_update(LSODA_ODE_SYSTEM_TYPE f, const size_t neq,
...
#undef LINE_LSODA_ETA

}  // namespace lsoda_impl
}  // namespace line

#endif  // LINE_THIRD_PARTY_LSODA_HPP
