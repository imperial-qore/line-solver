 ...
 * All rights reserved.
 */
#ifndef LINE_API_PFQN_PFQN_XZGSBLOW_H
#define LINE_API_PFQN_PFQN_XZGSBLOW_H

/**
 * Geometric-square-root bound, lower bound on throughput.
 *
 * Templated port of matlab/src/api/pfqn/pfqn_xzgsblow.m. Single-class model: L is the
...
T pfqn_xzgsblow(const std::vector<T>& L, const T& N, const T& Z) {
    static_assert(num_traits<T>::has_transcendental,
                  "pfqn_xzgsblow requires transcendental arithmetic (square root)");
    ...
    for (std::size_t i = 0; i < L.size(); ++i)
        if (L[i] < Lmax) R += (L[i] - Lmax) * pfqn_qzgblow(L, T(N - one), Z, i);
    T disc = R * R - num_traits<T>::from_int(4) * Z * Lmax * (N - one);
