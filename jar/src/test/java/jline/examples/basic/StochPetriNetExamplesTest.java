...
public class StochPetriNetExamplesTest {
    ...
    @Test
    public void testSpnInhibitingCTMC() {
        Network model = StochPetriNetModel.spn_inhibiting();

        final NetworkAvgTable[] avgTableHolder = new NetworkAvgTable[1];
        withSuppressedOutput(() -> {
            jline.solvers.ctmc.SolverCTMC solver =
                new jline.solvers.ctmc.SolverCTMC(model, "keep", false, "cutoff", 10);
            avgTableHolder[0] = solver.getAvgTable();
        });
        NetworkAvgTable avgTable = avgTableHolder[0];
        ...
        // Exact CTMC values (validated: native-Python CTMC == JMT@2e6 within 2.2e-3;
        // JMT@2e6 QLen sum == 4.0). Inhibitor arc T3(P2<1) active.
        double[] expectedQLen = {1.402698, 0.750000, 1.847302};
        java.util.List<Double> q = avgTable.getQLen();
        assertEquals(3, q.size(), "Expected 3 places");
        for (int i = 0; i < 3; i++) {
            assertEquals(expectedQLen[i], q.get(i), 1e-3,
                "CTMC QLen mismatch at place " + i);
        }
    }
