...
public class CacheExamplesTest {
    ...
    public void testCacheReplcRoutingCTMC() {
        ...
        // Regenerated 2026-07-21 from a direct MATLAB CTMC('cutoff',1) run: the
        // previous four values were the pre-fix JAR output, which complemented
        // timed departure rates over vanishing Router states. They missed MATLAB
        // by up to 1.2e-1 (12x the tolerance below) and contradicted the QLen
        // assertions that follow, since QLen = Tput * D holds at an INF Delay.
        // Flow is deliberately NOT conserved across the Router here: cutoff=1
        // truncates the open state space, so MATLAB loses jobs the same way.
        assertEquals(0.389554066039, tput.get(10), tol, "Delay1 HitClass Tput");
        assertEquals(0.424640621640, tput.get(11), tol, "Delay1 MissClass Tput");
        assertEquals(0.402227913991, tput.get(13), tol, "Delay2 HitClass Tput");
        assertEquals(0.583730768028, tput.get(14), tol, "Delay2 MissClass Tput");
