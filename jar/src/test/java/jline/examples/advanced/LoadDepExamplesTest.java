...
public class LoadDepExamplesTest {
    ...
    public void testLdMultiserverPsTwoclassesNCExact() {
        ...
        withSuppressedOutput(() -> {
            ...
            avgTableHolder[0] = solver.getAvgTable();
            // Since 7a143a3bb the exact LD dispatch routes this model to pfqn_gld
            // (pfqn_comomrm_ld accepts at most one queueing station). gld is
            // numerically exact here (goldens below match to machine precision);
            // only the method label changed exact/comomld -> exact/gld.
            assertEquals("exact/gld", solver.result.method, "NC solver should use exact/gld method");
        });
        NetworkAvgTable avgTable = avgTableHolder[0];
