...
public class CyclicPollingExamplesTest {
    ...
    public void testPollingExhaustiveDetJMT() {
        ...
        try {
            ...
            assertEquals(4, avgTable.getQLen().size(),
                "Expected 4 entries (2 stations × 2 classes) for polling_exhaustive_det model");
    ...
    public void testPollingGatedJMT() {
        ...
        try {
            ...
            // 2 stations (Source, Queue) × 2 classes = 4 entries (Sink has all-zero metrics and is filtered)
            assertEquals(4, avgTable.getQLen().size(),
                "Expected 4 entries (2 stations × 2 classes) for polling_gated model");
    ...
    public void testPollingKlimitedJMT() {
        ...
        try {
            ...
            // 2 stations (Source, Queue) × 2 classes = 4 entries (Sink has all-zero metrics and is filtered)
            assertEquals(4, avgTable.getQLen().size(),
                "Expected 4 entries (2 stations × 2 classes) for polling_klimited model");
