...
public class UtilityCoverageTest {
    ...
    void testCheckGenerator() {
        ...
        assertTrue(CheckGenerator.checkGenerator(validGen, false, 1e-14));
        ...
        assertFalse(CheckGenerator.checkGenerator(invalidGen, false, 1e-14));
        ...
        assertFalse(CheckGenerator.checkGenerator(invalidRowSum, false, 1e-14));

        // Non-square matrix should fail
        Matrix nonSquare = new Matrix(2, 3);
        assertFalse(CheckGenerator.checkGenerator(nonSquare, false, 1e-14));
    }
    ...
    void testCheckProbMatrix() {
        ...
        assertTrue(CheckProbMatrix.checkProbMatrix(validProb, false, 1e-14));
        ...
        assertFalse(CheckProbMatrix.checkProbMatrix(invalidProb, false, 1e-14));
        ...
        assertFalse(CheckProbMatrix.checkProbMatrix(invalidRowSum, false, 1e-14));

        // Non-square matrix should fail
        Matrix nonSquare = new Matrix(2, 3);
        assertFalse(CheckProbMatrix.checkProbMatrix(nonSquare, false, 1e-14));
    }
    ...
    void testCustomRomberg() {
        ...
        BigDecimal result = customromberg.starter(f, 1.0, 2.0);
        ...
        // Test hn method
        BigDecimal hn = customromberg.hn(0.0, 1.0, 4);
    ...
    void testFunctionWrapper() {
        ...
        UnaryOperator<Apcomplex> sum = function_wrapper.add(f1, f2);
        ...
        UnaryOperator<Apcomplex> product = function_wrapper.multiply(f1, f2);
    ...
    void testPH2From3Moments() {
        ...
        PH2Representation expResult = PH2From3Moments.ph2From3Moments(expMoments, 1e-14);

        assertNotNull(expResult);
        assertNotNull(expResult.alpha);
        assertNotNull(expResult.A);

        // For an exponential, we expect a 1x1 matrix
        assertEquals(1, expResult.alpha.getNumCols());
        assertEquals(1, expResult.A.getNumRows());
        ...
        PH2Representation hyperResult = PH2From3Moments.ph2From3Moments(hyperMoments, 1e-14);

        assertNotNull(hyperResult);
        assertEquals(2, hyperResult.alpha.getNumCols());  // 2-phase
        assertEquals(2, hyperResult.A.getNumRows());

        // Test that infeasible moments throw
        double[] invalidMoments = {1.0, 0.5, 1.0};  // m2 too small
        assertThrows(IllegalArgumentException.class,
                () -> PH2From3Moments.ph2From3Moments(invalidMoments, 1e-14));
    }
    ...
    void testAdaPartSampler_classicMode() {
        ...
        // Check tracking variables are populated
        assertFalse(sampler.sampleAccepted.isEmpty());
        assertFalse(sampler.sampleTime.isEmpty());
        assertFalse(sampler.permStep.isEmpty());
    }
