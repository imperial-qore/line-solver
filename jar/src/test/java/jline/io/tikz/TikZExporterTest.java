...
public class TikZExporterTest {
    ...
    public void testExportTutorialPNGs() throws Exception {
        // PNG export shells out to pdflatex (TeX Live / MiKTeX) and pdftoppm
        // (poppler-utils). Skip on hosts where either is missing — there's
        // nothing the code can fix when the toolchain is absent.
        Assumptions.assumeTrue(TikZExporter.isPdfLatexAvailable(),
                "pdflatex not on PATH; skipping PNG export test");
        Assumptions.assumeTrue(isPdftoppmAvailable(),
                "pdftoppm not on PATH; skipping PNG export test");
    ...
    private static boolean isPdftoppmAvailable() {
        // Some pdftoppm versions exit non-zero on -v, so we treat a successful
        // process start as proof the binary is on PATH.
        try {
