 ...
 * This replaces: DenseMatrixTest, MatrixArithmeticTest, MatrixCoreTest, 
...
public class MatrixTest {
    ...
    void setUp() {
        ...
        denseMatrix = new DenseMatrix(3, 4);
    }
    ...
    void testDenseMatrixConstructorWithEJMLMatrix() {
        ...
        DenseMatrix matrix3 = new DenseMatrix(ejmlMatrix);
    ...
    void testDenseMatrixCopyConstructor() {
        ...
        DenseMatrix copy = new DenseMatrix(denseMatrix);
    ...
    void testDenseMatrixClone() {
        ...
        assertTrue(cloned instanceof DenseMatrix);
    ...
    void testDenseMatrixEquals() {
        ...
        DenseMatrix other = new DenseMatrix(3, 4);
        other.set(0, 0, 1.0);
        other.set(1, 2, 3.5);
        ...
        DenseMatrix different = new DenseMatrix(2, 4);
    ...
    private static Matrix mat2(double a, double b, double c, double d) {
        Matrix x = new Matrix(2, 2);
        x.set(0, 0, a); x.set(0, 1, b); x.set(1, 0, c); x.set(1, 1, d);
        return x;
    }

    private static double sylvResidual(Matrix A, Matrix B, Matrix C) {
        Matrix X = Matrix.sylv(A, B, C);
        return A.mult(X).add(1.0, X.mult(B)).add(1.0, C).elementMaxAbs();
    }

    @Test
    public void testSylvSolvesForwardAndBackwardPaths() {
        // sylv(A,B,C) solves A X + X B + C = 0. The backward path (B == A',
        // i.e. a Lyapunov equation) previously skipped column 0 of X and
        // returned a wrong solution; all three cases must now be exact.
        Matrix Asym = mat2(-2, 0.5, 0.5, -3);
        assertEquals(0.0, sylvResidual(Asym, Asym.transpose(), mat2(1, 0, 0, 1)),
                1e-9, "symmetric Lyapunov (backward path)");
        Matrix Adiag = mat2(-2, 0, 0, -3);
        assertEquals(0.0, sylvResidual(Adiag, Adiag.transpose(), mat2(1, 0, 0, 1)),
                1e-9, "diagonal Lyapunov (backward path)");
        assertEquals(0.0, sylvResidual(mat2(-4.5, 0.2, 0.5, -1.2),
                        mat2(-3, 0.5, 0.2, -2), mat2(1, 0.2, 0.3, 0.4)),
                1e-9, "general Sylvester (forward path)");
    }

    @Test
    public void testSylvWithComplexEigenvalues() {
        // The real-Schur substitution is only valid column-by-column when the
        // Schur factors are truly triangular; complex eigenvalue pairs produce
        // 2x2 quasi-triangular blocks that previously gave a wrong solution.
        Matrix A = mat2(4, 1, 2, 5);
        Matrix Brot = mat2(0, 2, -2, 0); // eigenvalues +-2i
        assertEquals(0.0, sylvResidual(A, Brot, mat2(1, 1, 1, 1)),
                1e-9, "complex-eigenvalue B");
        Matrix Ac = mat2(-1, 5, -5, -1); // eigenvalues -1 +- 5i
        assertEquals(0.0, sylvResidual(Ac, Brot, mat2(1, 0.2, 0.3, 0.4)),
                1e-9, "complex eigenvalues in both A and B");
        assertEquals(0.0, sylvResidual(Ac, Ac.transpose(), mat2(1, 0, 0, 1)),
                1e-9, "complex-eigenvalue Lyapunov (backward path)");
    }

    @Test
    public void testLogsumexpWithDuplicatedMaximum() {
        // The max must be excluded from the sum only once (argmax index), not
        // for every element equal to it.
        Matrix x = new Matrix(new double[]{0.0, 0.0});
        assertEquals(Math.log(2.0), Matrix.logsumexp(x), 1e-12);
        Matrix y = new Matrix(new double[]{1.0, 1.0, 1.0});
        assertEquals(1.0 + Math.log(3.0), Matrix.logsumexp(y), 1e-12);
        Matrix z = new Matrix(new double[]{0.0, Math.log(2.0)});
        assertEquals(Math.log(3.0), Matrix.logsumexp(z), 1e-12);
    }

    @Test
    public void testKeepRowsOnNonSquareMatrix() {
        Matrix m = new Matrix(new double[][]{{1, 2}, {3, 4}, {5, 6}});
        m.keepRows(java.util.Collections.singletonList(0));
        assertEquals(1, m.getNumRows());
        assertEquals(2, m.getNumCols());
        assertEquals(1.0, m.get(0, 0), ZERO_TOL);
        assertEquals(2.0, m.get(0, 1), ZERO_TOL);
    }

    @Test
    public void testSortIncludesZeros() {
        Matrix v = new Matrix(new double[]{3.0, 0.0, 1.0});
        Matrix vs = v.sort();
        assertEquals(0.0, vs.get(0), ZERO_TOL);
        assertEquals(1.0, vs.get(1), ZERO_TOL);
        assertEquals(3.0, vs.get(2), ZERO_TOL);
        // matrices sort column by column (MATLAB semantics)
        Matrix m = new Matrix(new double[][]{{3, 0}, {1, 2}, {2, -1}});
        Matrix ms = m.sort();
        assertEquals(1.0, ms.get(0, 0), ZERO_TOL);
        assertEquals(2.0, ms.get(1, 0), ZERO_TOL);
        assertEquals(3.0, ms.get(2, 0), ZERO_TOL);
        assertEquals(-1.0, ms.get(0, 1), ZERO_TOL);
        assertEquals(0.0, ms.get(1, 1), ZERO_TOL);
        assertEquals(2.0, ms.get(2, 1), ZERO_TOL);
    }

    @Test
    public void testElementPowOnZeroEntries() {
        Matrix z = new Matrix(new double[]{0.0, 2.0});
        Matrix zp = z.elementPow(-1);
        assertTrue(Double.isInfinite(zp.get(0)), "0^-1 must be Inf");
        assertEquals(0.5, zp.get(1), ZERO_TOL);
        Matrix z0 = z.elementPow(0);
        assertEquals(1.0, z0.get(0), ZERO_TOL);
        assertEquals(1.0, z0.get(1), ZERO_TOL);
    }

    @Test
    public void testInvAndSolveAreScaleInvariant() {
        // The former |det| < 1e-14 singularity gate rejected well-conditioned
        // matrices with small determinant, e.g. det(1e-2*eye(8)) = 1e-16.
        Matrix s = Matrix.eye(8).scale(1e-2);
        Matrix si = s.inv();
        assertEquals(100.0, si.get(0, 0), 1e-9);
        Matrix x = new Matrix(8, 1);
        assertTrue(Matrix.solve(s, Matrix.ones(8, 1), x));
        assertEquals(100.0, x.get(0), 1e-9);
        // genuinely singular inputs must still be rejected
        assertThrows(RuntimeException.class, () -> mat2(1, 2, 2, 4).inv());
        assertThrows(RuntimeException.class, () -> {
            Matrix xs = new Matrix(2, 1);
            Matrix.solve(mat2(1, 2, 2, 4), Matrix.ones(2, 1), xs);
        });
    }
