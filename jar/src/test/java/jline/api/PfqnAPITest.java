...
//
    ...
    // ===== MoM regression tests for G25 (single-class, empty class, Z=0) =====

    /** Single-class, single station: previously a stub returning lG=0. */
    @Test
    public void testMoMSingleClassSingleStation() {
        Matrix L = new Matrix("[2]");
        Matrix N = new Matrix("[3]");
        Matrix Z = new Matrix("[1]");
        Ret.pfqnMom m = Pfqn_mom.pfqn_mom(L, N, Z);
        double lGca = Pfqn_ca.pfqn_ca(L, N, Z).lG;
        assertEquals(lGca, m.lG, FINE_TOL, "single-class single-station lG must match pfqn_ca");
        assertTrue(m.X.get(0, 0) > 0, "throughput must be positive");
    }

    /** Single-class, multiple stations: R==1 extraction was M=1-only. */
    @Test
    public void testMoMSingleClassMultiStation() {
        Matrix L = new Matrix("[2;1;3]");
        Matrix N = new Matrix("[5]");
        Matrix Z = new Matrix("[2]");
        Ret.pfqnMom m = Pfqn_mom.pfqn_mom(L, N, Z);
        double lGca = Pfqn_ca.pfqn_ca(L, N, Z).lG;
        assertEquals(lGca, m.lG, FINE_TOL, "single-class multi-station lG must match pfqn_ca");
        assertEquals(3, m.Q.getNumRows(), "Q has one row per station");
    }

    /** Single-class handles zero think time exactly (MVA path). */
    @Test
    public void testMoMSingleClassZeroThink() {
        Matrix L = new Matrix("[2;1;3]");
        Matrix N = new Matrix("[5]");
        Matrix Z = new Matrix(new double[1][1]); // Z = 0
        Ret.pfqnMom m = Pfqn_mom.pfqn_mom(L, N, Z);
        double lGca = Pfqn_ca.pfqn_ca(L, N, Z).lG;
        assertEquals(lGca, m.lG, FINE_TOL, "single-class Z=0 lG must match pfqn_ca");
    }

    /** Empty-population class: previously crashed with division-by-zero. */
    @Test
    public void testMoMEmptyClass() {
        Matrix L = new Matrix("[2,1,3;1,2,1]");
        Matrix N = new Matrix("[2,0,2]");
        Matrix Z = new Matrix("[1,1,1]");
        Ret.pfqnMom m = Pfqn_mom.pfqn_mom(L, N, Z);
        double lGca = Pfqn_ca.pfqn_ca(L, N, Z).lG;
        assertEquals(lGca, m.lG, FINE_TOL, "empty-class lG must match pfqn_ca");
        assertEquals(0.0, m.X.get(0, 1), ZERO_TOL, "empty class has zero throughput");
    }

    /** Multiclass R>=3 with positive think time matches pfqn_ca exactly. */
    @Test
    public void testMoMMulticlassR3() {
        Matrix L = new Matrix("[2,1,3;1,2,1]");
        Matrix N = new Matrix("[3,2,1]");
        Matrix Z = new Matrix("[1,1,1]");
        Ret.pfqnMom m = Pfqn_mom.pfqn_mom(L, N, Z);
        double lGca = Pfqn_ca.pfqn_ca(L, N, Z).lG;
        assertEquals(lGca, m.lG, FINE_TOL, "R=3 (Z>0) lG must match pfqn_ca");
    }

    /** Multiclass with a zero-think class is rejected with a clear message. */
    @Test
    public void testMoMMulticlassZeroThinkRejected() {
        Matrix L = new Matrix("[2,1;1,2]");
        Matrix N = new Matrix("[2,2]");
        Matrix Z = new Matrix(new double[1][2]); // Z = 0
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class,
                () -> Pfqn_mom.pfqn_mom(L, N, Z));
        assertTrue(ex.getMessage().contains("think time"),
                "diagnostic must name the zero think-time cause");
    }
