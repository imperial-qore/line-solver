...
import jline.api.cache.Cache_ttl_tree;
import jline.io.Ret;
...
import static jline.api.cache.Cache_gamma_lp.cache_gamma_lp;
import static jline.api.cache.Cache_mva.cache_mva;
import static jline.api.cache.Cache_prob_fpi.*;
import static jline.api.cache.Cache_t_hlru.*;
import static jline.api.cache.Cache_ttl_hlru.*;
import static jline.api.cache.Cache_ttl_lrum_map.*;
import static jline.api.cache.Cache_erec.*;
import static jline.api.cache.Cache_miss.cache_miss;
import static jline.api.cache.Cache_miss_asy.cache_miss_asy;
import static jline.api.cache.Cache_miss_spm.cache_miss_spm;
import static jline.api.cache.Cache_mva_miss.cache_mva_miss;
import static jline.api.cache.Cache_prob_erec.cache_prob_erec;
import static jline.api.cache.Cache_prob_spm.cache_prob_spm;
import static jline.api.cache.Cache_spm.cache_spm;
import static jline.api.cache.Cache_xi_fp.cache_xi_fp;
import static jline.api.cache.Cache_gamma.cache_gamma;
import static jline.api.cache.Cache_miss_fpi.cache_miss_fpi;
import static jline.api.cache.Cache_xi_iter.cache_xi_iter;
import static jline.api.cache.Cache_is.cache_is;
import static jline.api.cache.Cache_prob_is.cache_prob_is;
import static jline.api.cache.Cache_miss_is.cache_miss_is;
 ...
 *   <li>cache_t_hlru - Response time analysis</li>
...
class CacheAPITest {
    ...
    /** Default linear access graph: list l promotes to l+1, list h self-loops. */
    private static Matrix[][] linearAccessGraph(int u, int n, int h) {
    ...
    /**
     * Demonstrates cache_ttl_lrua on the default linear access graph.
     * 
     ...
     */
    @Test
    void cache_ttl_lrua_linear_test1(){
        Matrix[] lambda = new Matrix[3];
        ...
        Matrix pij = cache_ttl_lrua(lambda, linearAccessGraph(1, 10, 2), m);
        for (int i = 0; i < 10; i++){
    ...
    /**
     * Demonstrates cache_ttl_lrua with a single cache list.
     * 
     ...
     */
    @Test
    void cache_ttl_lrua_linear_test3(){
        Matrix[] lambda = new Matrix[3];
        lambda[0] = new Matrix("[0.4,0.4;0.4,0.4;0.4,0.4;0.4,0.4;0.4,0.4]");
        ...
        Matrix pij = cache_ttl_lrua(lambda, linearAccessGraph(1, 5, 1), m);
        for (int i = 0; i < 5; i++){
            assertEquals(0.6, pij.get(i,0), TestTools.MID_TOL);
        }
        for (int i = 0; i < 5; i++){
    ...
    /**
     * Demonstrates cache_ttl_lrua with variable list capacities.
     * 
     ...
     */
    @Test
    void cache_ttl_lrua_linear_test2(){
        Matrix[] lambda = new Matrix[3];
        ...
        Matrix pij = cache_ttl_lrua(lambda, linearAccessGraph(1, 10, 3), m);
        for (int i = 0; i < 10; i++){
    ...
    void cache_t_hlru_test1(){
        ...
        // LRU(m) characteristic times: exp(0.2*t1)-1 = 1/3, exp(0.2*t2)-1 = 1
    ...
    @Test
    void cache_ttl_lrum_map_test3(){
        // Per-item MAPs thinned from a common MMPP2 (bursty, correlated).
        // Reference 0.8581223066 from an independent numpy implementation of
        // Gast-Van Houdt PEVA 2017 eqs. (5)-(9) with the exact Perron
        // boundary vector; matches MATLAB/python cache_ttl_lrum_map.
        ...
        for (int k = 0; k < n; k++) {
            Matrix D1k = Matrix.scaleMult(D1, p[k]);
            Matrix D0k = D0.add(1.0 - p[k], D1);
            D0c[k] = new MatrixCell(2);
            D1c[k] = new MatrixCell(2);
            for (int l = 0; l < 2; l++) {
                D0c[k].set(l, D0k);
                D1c[k].set(l, D1k);
            }
        }
        Matrix m = new Matrix("[2.0,1.0]");
        double hit_rate = cache_ttl_lrum_map(D0c, D1c, m);
        assertEquals(0.8581223066, hit_rate, 1e-4);
    }
    ...
    void testCacheTtlLrua_singleList() {
        ...
        lambda[0] = new Matrix(n, h + 1);  // n rows x (h+1) columns
        for (int i = 0; i < n; i++) {
            lambda[0].set(i, 0, 0.4);  // Miss state arrival rate
            lambda[0].set(i, 1, 0.4);  // List 1 arrival rate
        }
    ...
    void testCacheTtlLrua_twoLists() {
        ...
        // Lambda: arrival rates [u][n x (h+1)]
        Matrix[] lambda = new Matrix[u];
        lambda[0] = new Matrix(n, h + 1);
        for (int i = 0; i < n; i++) {
            lambda[0].set(i, 0, 0.3);  // Miss state arrival rate
            lambda[0].set(i, 1, 0.3);  // List 1 arrival rate
            lambda[0].set(i, 2, 0.3);  // List 2 arrival rate
        }
    ...
    void testCacheTtlLrua_uniformAccess() {
        ...
        lambda[0] = new Matrix(n, h + 1);
        for (int i = 0; i < n; i++) {
            lambda[0].set(i, 0, rate);
            lambda[0].set(i, 1, rate);
        }
    ...
    void testCacheTtlLrua_variableRates() {
        ...
        lambda[0].set(2, 0, 0.2);
        lambda[0].set(2, 1, 0.2);
        lambda[0].set(3, 0, 0.2);
        lambda[0].set(3, 1, 0.2);
