...
public class PetriStateTest {
    ...
    public void testEnablingEvent_Mode1Exp_NotEnabled() {
        ...
        // A mode that is ALREADY fully disabled has nothing left to disable, so the
        // ENABLE event yields no outcome and hence no rate. It must NOT report Immediate:
        // that is an immediate self-loop, which a simulator scanning for enabled events
        // fires forever (SSA livelock). MATLAB returns outrate = [] here -- fdbf72cc0
        // ("m fix: SPN simulation support for Place nodes in SSA") removed exactly this
        // unconditional Immediate -- and the native Python returns its rate 0 no-op.
        // The JAR reports rate 0 via the empty-matrix padding in AfterGlobalEvent, with
        // the state unchanged and prob = 1, matching both.
        assertEquals(0.0, result.outrate.get(0, 0), GlobalConstants.FineTol);
        assertEquals(1.0, result.outprob.get(0, 0), GlobalConstants.FineTol);
    }
    ...
    public void testFiringEvent_ConsumeAndProduceTokens() {
        // Check 11 (MATLAB lines 119-130): gsync{4} Fire Mode1, consume 1 produce 1
        ...
        // Expected buffer: [1 2 1 2 1]. This event is the FIRST completion among the
        // two running Mode 1 servers, i.e. ONE firing, so it consumes the enabling
        // multiplicity (1) and fires back the firing outcome (1); the concurrency of
        // the second server is already carried by outrate below. Mode 1 has
        // enabling == firing == 1 on P1, so the marking is invariant and the token
        // count is conserved: consuming enDegree tokens while firing back only 1
        // would destroy a token per firing.
        Matrix outBuffer = result.outglspace.get(0);
        assertEquals(5, outBuffer.getNumCols(), "Buffer should have 5 tokens after firing");
        double[] expectedBuffer = {1, 2, 1, 2, 1};
        for (int i = 0; i < 5; i++) {
    ...
    @Test
    public void testEnablingEvent_Mode1Exp_StopExcessServers() {
        // running_m > en_degree_m: 2 mode1 (Exp, 1 phase) servers are already
        // running but the marking now supports an enabling degree of only 1, so
        // ENABLE must stop one server and return it to the idle pool (matches
        // matlab/src/lang/+State/afterGlobalEvent.m, ENABLE case, "running_m >
        // en_degree_m" branch).
        Network model = PetriStateTestFixtures.createTestModelMode1TwoServersPop2();
        NetworkStruct localSn = model.getStruct();
        Transition t1 = (Transition) model.getNodeByName("T1");
        int ind = t1.getNodeIndex();

        GlobalSync glevent = localSn.gsync.get(0); // ENABLE mode0 (Exp)

        List<Matrix> glspace = new ArrayList<Matrix>();
        Matrix buffer = new Matrix(1, 1); // only 1 token now available at P1
        buffer.set(0, 0, 1);
        glspace.add(buffer);

        // Transition state before the event: both mode1 servers already running
        // (buf_mode1=0, srv_mode1=2), mode2/mode3 idle with 1 server each.
        Matrix transState = new Matrix(1, 8);
        ...
        AfterGlobalEventResult result = AfterGlobalEvent.afterGlobalEvent(
                localSn, ind, glspace, glevent, false);

        assertNotNull(result.outglspace, "Should return output global space");

        Matrix outTransState = result.outglspace.get(1);
        assertEquals(1, outTransState.getNumRows(), "Only 1 server needs to stop, single outcome");

        // Expected: [1 1 1 1 0 0 0 0] (mode1 idle=1, running=1; mode2/mode3 unchanged)
        double[] expectedTrans = {1, 1, 1, 1, 0, 0, 0, 0};
        for (int i = 0; i < 8; i++) {
            assertEquals(expectedTrans[i], outTransState.get(0, i), 0.0,
                    "outglspace{2}[" + i + "] mismatch");
        ...
        assertEquals(GlobalConstants.Immediate, result.outrate.get(0, 0), GlobalConstants.FineTol);
        assertEquals(1.0, result.outprob.get(0, 0), GlobalConstants.FineTol,
                "Single valid combination (stop the only running server pool of 1 phase) has probability 1");
    }
}
