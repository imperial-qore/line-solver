...
import jline.io.Ret;
import static jline.api.mam.Map2_fit.map2_fit;
...
import static jline.solvers.ldes.handlers.Solver_ssj.computeOBMStatistics;
import static jline.solvers.ldes.handlers.Solver_ssj.computeSpectralStatistics;
import static jline.solvers.ldes.handlers.Solver_ssj.computeStandardBatchMeansStatistics;
import static jline.solvers.ldes.handlers.Solver_ssj.getTCriticalValue;
...
public class SolverLDESTest extends SolverLDESTestFixtures {
    ...
    class CoreDESTests {
        ...
        class MulticlassTests {
                ...
                @Test
                public void test_fsp_behavior() {
                    // FSP (Fair Sojourn Protocol): closed M/G/1 with HyperExp service.
                    // Verify (a) FSP and SRPT both improve mean response time over PS
                    // under high service-time variability, and (b) FSP and SRPT achieve
                    // comparable performance (single-class single-server).
                    double rFcfs = runFspClosedNetwork(SchedStrategy.FCFS);
                    double rPs   = runFspClosedNetwork(SchedStrategy.PS);
                    double rSrpt = runFspClosedNetwork(SchedStrategy.SRPT);
                    double rFsp  = runFspClosedNetwork(SchedStrategy.FSP);

                    // Size-based disciplines should beat FCFS/PS under high-SCV workload
                    assertTrue(rSrpt < rFcfs,
                            "SRPT R=" + String.format("%.4f", rSrpt)
                                    + " expected below FCFS R=" + String.format("%.4f", rFcfs));
                    assertTrue(rFsp < rFcfs,
                            "FSP R=" + String.format("%.4f", rFsp)
                                    + " expected below FCFS R=" + String.format("%.4f", rFcfs));
                    // FSP pareto-dominates PS per Friedman-Henderson; mean must not be worse
                    // (small simulation tolerance to absorb sampling noise)
                    assertTrue(rFsp <= rPs * 1.02,
                            "FSP R=" + String.format("%.4f", rFsp)
                                    + " expected at or below PS R=" + String.format("%.4f", rPs));
                    // FSP and SRPT should be close (equivalent in single-server single-class M/G/1)
                    double rel = Math.abs(rFsp - rSrpt) / Math.max(rFsp, rSrpt);
                    assertTrue(rel < 0.20,
                            "FSP R=" + String.format("%.4f", rFsp)
                                    + " expected within 20% of SRPT R=" + String.format("%.4f", rSrpt)
                                    + " (relErr=" + String.format("%.3f", rel) + ")");
                }

                private double runFspClosedNetwork(SchedStrategy disc) {
                    Network model = new Network("fsp-" + disc);
                    Delay think = new Delay(model, "Think");
                    Queue server = new Queue(model, "Server", disc);
                    ClosedClass jobs = new ClosedClass(model, "Jobs", 10, think, 0);
                    think.setService(jobs, Exp.fitMean(1.0));
                    server.setService(jobs, new HyperExp(0.95, 0.5, 5.0));
                    RoutingMatrix P = model.initRoutingMatrix();
                    P.set(jobs, jobs, think, server, 1.0);
                    P.set(jobs, jobs, server, think, 1.0);
                    model.link(P);

                    LDESOptions opts = createDefaultTestOptions();
                    opts.samples = 20000;
                    SolverLDES solver = new SolverLDES(model, opts);
                    NetworkAvgTable t = solver.getAvgTable();
                    // index 1 is the Queue (index 0 is the Delay)
                    return t.getRespT().get(1);
                }
    ...
    class SchedulingTests {
        ...
        @Nested
        class DeadlineSchedulingTests {

                @Test
                public void testEDD_MM1() {
                    // EDD (non-preemptive earliest-due-date). With a single class and a constant
                    // deadline offset, deadline order coincides with arrival order, so the
                    // work-conserving M/M/1 metrics match the reference.
                    validateDESAgainstStaticRef("EDD M/M/1", createEDDNetwork(),
                            LCFS_JMT_QLEN, LCFS_JMT_UTIL, LCFS_JMT_TPUT, LCFS_JMT_RESPT);
                }

                @Test
                public void testEDF_MM1() {
                    // EDF (preemptive-resume earliest-deadline-first). A single class never triggers
                    // preemption (deadlines are monotone in arrival time), so metrics match M/M/1.
                    validateDESAgainstStaticRef("EDF M/M/1", createEDFNetwork(),
                            LCFS_JMT_QLEN, LCFS_JMT_UTIL, LCFS_JMT_TPUT, LCFS_JMT_RESPT);
                }

                @Test
                public void testSETF_MM1() {
                    // SETF (shortest-elapsed-time-first, non-preemptive). Waiting jobs have zero
                    // attained service, so ties break by arrival => M/M/1 reference metrics.
                    validateDESAgainstStaticRef("SETF M/M/1", createSETFNetwork(),
                            LCFS_JMT_QLEN, LCFS_JMT_UTIL, LCFS_JMT_TPUT, LCFS_JMT_RESPT);
                }

                @Test
                public void testEDF_twoClass_workConserving() {
                    // Two classes with distinct deadlines exercise the EDF preemption path (an
                    // arriving urgent job can preempt a relaxed job in service). EDF is
                    // work-conserving, so per-class throughput equals the arrival rate and the
                    // aggregate server utilization equals the offered load rho1+rho2, independently
                    // of any preemptions. Both classes are symmetric in load, so these invariants
                    // hold regardless of table row order.
                    Network model = createEDFTwoClassNetwork();
                    SolverLDES solver = new SolverLDES(model, createDefaultTestOptions());
                    NetworkAvgTable t = solver.getAvgTable();
                    List<Double> util = t.getUtil();
                    List<Double> tput = t.getTput();
                    // Row order is station-major, class-minor: Queue is station index 1 => rows 2,3.
                    double qUtil = util.get(2) + util.get(3);
                    assertEquals(0.6, qUtil, 0.03,
                            "EDF two-class: aggregate utilization should equal rho1+rho2");
                    assertEquals(0.3, tput.get(2), 0.02,
                            "EDF two-class: class throughput should equal its arrival rate");
                    assertEquals(0.3, tput.get(3), 0.02,
                            "EDF two-class: class throughput should equal its arrival rate");
                }
        }
    ...
    class SpecializedTests {
        ...
        class StatisticsTests {
            	...
            	public void test_obm_basic_computation() {
            		...
            		double mean = result.getLeft();
            		double stdError = result.getMiddle();
            		int df = result.getRight();
            	...
            	public void test_obm_vs_standard_batch_means() {
            		...
            		double obmStdError = obmResult.getMiddle();
            		double stdStdError = stdResult.getMiddle();

            		// Means should be similar
            		assertTrue(Math.abs(obmResult.getLeft() - stdResult.getLeft()) < 0.5,
            				"Means should be similar");
            	}
            	...
            	public void test_spectral_vs_bm_autocorrelated() {
            		...
            		double spectralStdErr = spectralResult.getMiddle();
            		double bmStdErr = bmResult.getMiddle();

            		// Means should be similar
            		assertTrue(Math.abs(spectralResult.getLeft() - bmResult.getLeft()) < 0.5,
            				"Means should be similar");
    ...
    private static Network createEDDNetwork() {
        Network model = new Network("M/M/1 EDD");
        Source source = new Source(model, "Source");
        Queue queue = new Queue(model, "Queue", SchedStrategy.EDD);
        Sink sink = new Sink(model, "Sink");
    ...
    private static Network createEDFNetwork() {
        Network model = new Network("M/M/1 EDF");
        Source source = new Source(model, "Source");
        Queue queue = new Queue(model, "Queue", SchedStrategy.EDF);
        Sink sink = new Sink(model, "Sink");
        OpenClass jobClass = new OpenClass(model, "Class1", 0, 5.0);
        source.setArrival(jobClass, new Exp(0.5));
        queue.setService(jobClass, new Exp(1.0));
    ...
    private static Network createSETFNetwork() {
        Network model = new Network("M/M/1 SETF");
        Source source = new Source(model, "Source");
        Queue queue = new Queue(model, "Queue", SchedStrategy.SETF);
        Sink sink = new Sink(model, "Sink");
        OpenClass jobClass = new OpenClass(model, "Class1", 0);
        source.setArrival(jobClass, new Exp(0.5));
        queue.setService(jobClass, new Exp(1.0));
    ...
    private static Network createEDFTwoClassNetwork() {
        Network model = new Network("M/M/1 EDF two-class");
        Source source = new Source(model, "Source");
        Queue queue = new Queue(model, "Queue", SchedStrategy.EDF);
        Sink sink = new Sink(model, "Sink");
        OpenClass urgent = new OpenClass(model, "Urgent", 0, 1.0);    // tight deadline
        OpenClass relaxed = new OpenClass(model, "Relaxed", 0, 20.0); // loose deadline
        source.setArrival(urgent, new Exp(0.3));
        source.setArrival(relaxed, new Exp(0.3));
        queue.setService(urgent, new Exp(1.0));
        queue.setService(relaxed, new Exp(1.0));
        RoutingMatrix P = model.initRoutingMatrix();
        P.set(urgent, urgent, source, queue, 1.0);
        P.set(urgent, urgent, queue, sink, 1.0);
        P.set(relaxed, relaxed, source, queue, 1.0);
        P.set(relaxed, relaxed, queue, sink, 1.0);
        model.link(P);
        return model;
    }
