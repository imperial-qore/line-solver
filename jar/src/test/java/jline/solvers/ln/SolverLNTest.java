...
//    }
    ...
    // Convergence-guard regression: this model exposes an AMVA layer-bistability
    // (two coexisting fixed points) that made cold-init SolverLN diverge before the
    // init_sol warm-start fix. It must converge under SolverLN(MVA).
    @Test
    public void test_doesnotconverge_v3() throws Exception {
        String resourcePath = "/lqn/randomLQN/model_C2_L2_T4_P2_1_doesnotconverge_v3.lqnx";
        String fullPath = getClass().getResource(resourcePath).getPath();
        LayeredNetwork lqn = LayeredNetwork.parseXML(fullPath);

        LNOptions options = new LNOptions();
        options.verbose = VerboseLevel.SILENT;

        SolverLN solver = new SolverLN(lqn, (m) -> new SolverMVA(m), options);
        LayeredNetworkAvgTable avgTable = (LayeredNetworkAvgTable) solver.getAvgTable();

        assertTrue(solver.hasconverged, "SolverLN(MVA) should converge on doesnotconverge_v3");
        assertNotNull(avgTable, "Average table should be computed");
    }
