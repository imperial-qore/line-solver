...
public class SolverSSATest {
    ...
    public void test_cqn_repairmenps() {
        ...
        assertTrue(TestTools.compareRelErr(avgTable.getQLen(), QLen, COARSE_TOL));
        assertTrue(TestTools.compareRelErr(avgTable.getUtil(), Util, COARSE_TOL));
        assertTrue(TestTools.compareRelErr(avgTable.getRespT(), RespT, COARSE_TOL));
        assertTrue(TestTools.compareRelErr(avgTable.getResidT(), ResidT, COARSE_TOL));
        assertTrue(TestTools.compareRelErr(avgTable.getArvR(), ArvR, COARSE_TOL));
        assertTrue(TestTools.compareRelErr(avgTable.getTput(), Tput, COARSE_TOL));
    }
...
options.method = "serial";
    ...
    /**
     * Test: Try nrm with fcfs with direct metric computation on simple open network.
     */
    @Test
    public void testSolverSsaNrm_Fcfs_Open() {
        try {
            // Expected metrics computed from old SSA algorithm
            List<Double> QLen      = Arrays.asList(0.021550, 0.110593, 0.0);
            List<Double> Util      = Arrays.asList(0.021550, 0.098849, 0.0);
            List<Double> RespT     = Arrays.asList(0.218017, 1.111302, 0.0);
            List<Double> ResidT    = Arrays.asList(0.218017, 1.111302, 0.0);
            List<Double> ArvR      = Arrays.asList(0.100000, 0.098849, 0.0);
            List<Double> Tput      = Arrays.asList(0.098849, 0.099516, 0.10);

            Network model = OpenModel.oqn_basic();

            SolverSSA solver = new SolverSSA(model);
            solver.getOptions().samples(100000);
            solver.getOptions().seed(796);
            solver.getOptions().verbose = VerboseLevel.SILENT;
            // Force nrm method
            solver.getOptions().method = "nrm";
            solver.runAnalyzer();
            NetworkAvgTable result = solver.getAvgTable();

            assertNotNull(result, "NRM should handle FCFS scheduling");
            // Check within 5%
            assertTrue(TestTools.compareRelErr(result.getQLen(), QLen, LOOSE_COARSE_TOL));
            assertTrue(TestTools.compareRelErr(result.getUtil(), Util, LOOSE_COARSE_TOL));
            assertTrue(TestTools.compareRelErr(result.getRespT(), RespT, LOOSE_COARSE_TOL));
            assertTrue(TestTools.compareRelErr(result.getResidT(), ResidT, LOOSE_COARSE_TOL));
            assertTrue(TestTools.compareRelErr(result.getArvR(), ArvR, LOOSE_COARSE_TOL));
            assertTrue(TestTools.compareRelErr(result.getTput(), Tput, LOOSE_COARSE_TOL));
        } catch (Exception e) {
            fail("FCFS scheduling may have specific requirements");
        }
    }

    /**
     * Test: Try nrm with fcfs with direct metric computation on simple closed network.
     */
    @Test
    public void testSolverSsaNrm_Fcfs_Closed() {
        try {
            // Expected metrics computed from old SSA algorithm
            List<Double> QLen      = Arrays.asList(1.9391251831987049, 1.6385007257755775, 2.060874816801295, 0.3614992742244225);
            List<Double> Util      = Arrays.asList(1.9391251831987049, 1.6385007257755775, 0.6463750610662349, 0.05461669085918591);
            List<Double> RespT     = Arrays.asList(1.0, 1.0, 1.0553746392685261, 0.22124382165910875);
            List<Double> ResidT    = Arrays.asList(1.0, 1.0, 1.0553746392685261, 0.22124382165910875);
            List<Double> ArvR      = Arrays.asList(1.9527424102493833, 1.633940652053184, 1.9391251831987049, 1.6385007257755775);
            List<Double> Tput      = Arrays.asList(1.9391251831987049, 1.6385007257755775, 1.9527424102493833, 1.633940652053184);

            Network model = ClosedModel.cqn_repairmen_multi();

            SolverSSA solver = new SolverSSA(model);
            solver.getOptions().samples(100000);
            solver.getOptions().seed(798);
            solver.getOptions().verbose = VerboseLevel.SILENT;
            // Force nrm method
            solver.getOptions().method = "nrm";
            solver.runAnalyzer();
    ...
    /**
     * Test: Try nrm with fcfs with direct metric computation on mixed multiserver network.
     */
    @Test
    public void testSolverSsaNrm_Fcfs_Mixed() {
        try {
            // Expected metrics computed from old SSA algorithm
            List<Double> QLen      = Arrays.asList(2.2496233303198037, 1.4410942918426604, 0.35728574639245125, 0.22880784663076506, 0.2237880420337682, 0.1748312502591011, 0.16930288125398382, 0.13407094074075324, 0.0);
            List<Double> Util      = Arrays.asList(0.6772115250159353, 0.30000000000000143, 0.1670816957540468, 0.10740892499384483, 0.07414555823728718, 0.058297770946568274, 0.04184105368868298, 0.027028738616466617, 0.0);
            List<Double> RespT     = Arrays.asList(3.3660529362105733, 4.743588794644234, 0.5354119276947317, 0.7553306052704639, 0.3342829922778355, 0.5785465103909216, 0.25, 0.44721359549995526, 0.0);
            List<Double> ResidT    = Arrays.asList(3.3660529362105733, 4.743588794644234, 0.5354119276947317, 0.7553306052704639, 0.3342829922778355, 0.5785465103909218, 0.25, 0.4472135954999552, 0.0);
            List<Double> ArvR      = Arrays.asList(0.6772115250159353, 0.30000000000000143, 0.6683267830161872, 0.30379831689241976, 0.6673100241355846, 0.30292410374240697, 0.6694568590189277, 0.3021904844624649, 0.0);
            List<Double> Tput      = Arrays.asList(0.6683267830161872, 0.30379831689241976, 0.6673100241355846, 0.30292410374240697, 0.6694568590189277, 0.3021904844624649, 0.6772115250159353, 0.29979173730367203, 0.30000000000000143);

            Network model = MixedModel.mqn_multiserver_fcfs();

            SolverSSA solver = new SolverSSA(model);
            solver.getOptions().samples(100000);
            solver.getOptions().seed(799);
            solver.getOptions().verbose = VerboseLevel.SILENT;
            // Force nrm method
            solver.getOptions().method = "nrm";
            solver.runAnalyzer();
    ...
    class CacheTests {
        ...
        private void testCacheConfiguration(Network model, double expectedHitRatio, int ssaSamples, String testDescription) {
            ...
            try {
                ...
                // These expected hit ratios are SERIAL-SSA point estimates at
                // seed=1, samples=1e4. The serial engine shares State.afterEventStation
                // with CTMC, so at this fixed (seed, samples) it reproduces the golden
                // bit-for-bit (verified to ~1e-8). Pin method="serial" here: the M5
                // gate made the SSA DEFAULT prefer NRM, which is a DISTINCT sampler
                // (different sample path at the same seed) that converges to the same
                // exact CTMC hit ratio only asymptotically, so at 1e4 samples it drifts
                // 1-5% and breaks these deterministic golden comparisons. The goldens
                // are serial references, so the test must ask for serial. See
                // [[nrm-vs-serial-distinct-simulators]].
                SolverOptions ssaOptions = new SolverOptions();
                ssaOptions.verbose = VerboseLevel.SILENT;
                ssaOptions.samples = ssaSamples;
                ssaOptions.seed = 1;
                ssaOptions.method = "serial";
