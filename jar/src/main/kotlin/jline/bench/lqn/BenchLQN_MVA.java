...
public class BenchLQN_MVA {
     ...
     */
    public static class MVASolverFactory implements SolverFactory {
        @Override
    ...
    public static boolean runModel(String modelPath) {
        try {
            ...
            // Create solver with MVA
            SolverLN solver = new SolverLN(lqn, new MVASolverFactory(), options);
            
            // Get results
            AvgTable avgTable = solver.getAvgTable();
            
            if (avgTable != null && solver.result.iter < options.iter_max) {
                System.out.println("    SUCCESS - MVA solver converged in " + solver.result.iter + " iterations");
                return true;
            } else {
                System.out.println("    FAILED - MVA solver did not converge");
                return false;
            }
    ...
    public static void runConfiguration(int clients, int layers, int tasks, int processors, String modelsPath) {
        System.out.printf("\nMVA Solver - Configuration: C%d_L%d_T%d_P%d\n", clients, layers, tasks, processors);
        ...
        System.out.printf("  MVA Results: %d/%d successful\n", successful, total);
    }
    ...
    public static void runAll() {
        ...
        System.out.println("=== Running LQN MVA Solver Benchmarks ===");
