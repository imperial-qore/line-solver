...
public class ClosingAndStateDepMethodsODE implements FirstOrderDifferentialEquations {
    ...
    private Matrix calculateJumps(boolean[][] enabled, Matrix qIndices, Matrix Kic) {
        ...
        int K = mu.get(sn.getStations().get(0)).size(); // Number of classes
            ...
            for (int c = 0; c < K; c++) {
                if (enabled[i][c]) {
                    for (int j = 0; j < M; j++) {
                        for (int l = 0; l < K; l++) {
                            Matrix pie;
                            if (proc.get(sn.getStations().get(j)).get(sn.jobclasses.get(l)).isEmpty()) {
                                pie = new Matrix(1, 1, 1);
                                pie.set(0, 0, 1);
                            } else {
                                Matrix D0 = proc.get(sn.getStations().get(j)).get(sn.jobclasses.get(l)).get(0);
                                Matrix D1 = proc.get(sn.getStations().get(j)).get(sn.jobclasses.get(l)).get(1);
                                ...
                                for (int kicIdx = 0; kicIdx < Kic.get(i, c); kicIdx++) {
                                    for (int kjl = 0; kjl < Kic.get(j, l); kjl++) {
                                        rateBase.set(
                                                rateIdx,
                                                0,
                                                phi.get(sn.getStations().get(i)).get(sn.jobclasses.get(c)).get(kicIdx, 0)
                                                        * mu.get(sn.getStations().get(i)).get(sn.jobclasses.get(c)).get(kicIdx, 0)
                                                        * rt.get(i * K + c, j * K + l)
                                                        * pie.get(0, kjl));
    ...
    private Matrix calculatedxdtClosingMethod(
        ...
        for (int i = 0; i < M; i++) {
            switch (sn.sched.get(sn.getStations().get(i))) {
                case INF:
                    break;
                case EXT:
        ...
        for (int i = 0; i < M; i++) {
            switch (sn.sched.get(sn.getStations().get(i))) {
                ...
                default:
                    ...
                    line_error(mfilename(new Object[]{}),
                            "Softmin ODE method does not support scheduling strategy: " + sn.sched.get(sn.getStations().get(i)) + 
    ...
    public void computeDerivatives(double t, double[] x, double[] dxdt)
        ...
        for (int i = 0; i < M; i++) {
            Station station = sn.getStations().get(i);
            for (int c = 0; c < K; c++) {
        ...
        if (Objects.equals(options.method, "statedep")) {
            ...
            if (options.config != null && options.config.hide_immediate) {
                ...
                rateBase = result.rateBaseReduced;
                eventIdx = result.eventIdxReduced;
            }
