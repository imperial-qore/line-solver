...
public class NetworkAvgChainTable extends AvgTable {
    ...
    public void print(SolverOptions options) {
        if (options.verbose != VerboseLevel.SILENT) {
            ...
            maxInChainLength = Integer.max(maxInChainLength, "JobClasses".length());


            // Print header with dynamic width for Station and JobClass
            String format = String.format("%%-%ds%%-%ds%%-%ds%%-12s%%-12s%%-12s%%-12s%%-12s%%-12s",
                    maxStationLength + 2, maxChainLength + 2, maxInChainLength + 2);
            System.out.printf(format, "Station", "Chain", "JobClasses", "QLen", "Util", "RespT", "ResidT", "ArvR", "Tput");
            System.out.println();
            ...
            int tableWidth = (maxStationLength + 2) + (maxChainLength + 2) + (maxInChainLength + 2) + 6 * (12 - 5 + nDigits);
