 ...
 * @return cacheMissRayIntReturn - An object containing the miss rate metrics and probabilities.
 *//* Example:
    ...
    cacheMissRayIntReturn ret = cache_miss_rayint(gamma, m, lambda);
    new Matrix(ret.MU).print();
 */

fun cache_miss_rayint(gamma: Matrix, m: Matrix, lambda: MatrixCell): Ret.cacheMissSpm {
    val ma = m.copy()
    ma[0, 0] = ma.value() + 1

    val lE = cache_rayint(gamma, m).lZ
    val lEa = cache_rayint(gamma, ma).lZ
    ...
    for (k in 0..<n) {
        if (gamma.getRow(k).elementSum() > 0) {
            ...
            lE1[k] = cache_rayint(subGamma, m).lZ
            pi0[k] = FastMath.exp(lE1[k] - lE)

            //                if (pi0[k] > 1 || pi0[k] < 0) {
//                    result = cache_rayint(subGamma, m);
//                    lE1[k] = result.lZ;
//                    pi0[k] = FastMath.exp(lE1[k] - lE);
//                }
            for (v in 0..<u) {
