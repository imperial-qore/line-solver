 ...
 * (delay) stations to provide accurate performance bounds.
 * 
 * @since LINE 3.0
 */
...
object Mapqn_bnd_qr_delay {
    ...
    class QuadraticDelayParameters(
        ...
        val Z: Double,  // delay (queue M)
        val D1: Double,  // service demand at queue 1
        ...
        fun validate() {
            ...
            require(D1 > 0) { "D1 must be positive" }
    ...
    private fun registerVariables(model: Mapqn_lpmodel, params: QuadraticDelayParameters) {
        val M = params.M
        val N = params.N
        val K = params.K
        
        // p2 variables only (no p1 in this model)
        for (j in 1..M) {
            for (nj in 0..N) {
                for (k in 1..K[j - 1]) {
                    for (i in 1..M) {
    ...
    private fun addDefinitionConstraints(model: Mapqn_lpmodel, params: QuadraticDelayParameters) {
        val M = params.M
        val N = params.N
        val K = params.K
    ...
    private fun addLittlesLawConstraints(model: Mapqn_lpmodel, params: QuadraticDelayParameters) {
        val M = params.M
        val N = params.N
        val K = params.K
        val Z = params.Z
        ...
        // XZ: Delay constraint
        val xzConstraint = model.constraintBuilder()
        
        // Left side: queue length at delay
        for (ni in 1..N) {
            for (k in 1..K[M - 1]) {
                xzConstraint.addTerm("p2_${M}_${ni}_${k}_${M}_${ni}_$k", ni.toDouble())
            }
        }
        
        // Right side: Z * throughput at delay
        for (k in 1..K[0]) {  // Queue 1
            for (nj in 1..N) {
    ...
    private fun addBalanceConstraints(model: Mapqn_lpmodel, params: QuadraticDelayParameters) {
        val M = params.M
        val N = params.N
        val K = params.K
    ...
    private fun addBoundConstraints(model: Mapqn_lpmodel, params: QuadraticDelayParameters) {
        val M = params.M
        val N = params.N
        val K = params.K
...
// Extension functions for delay model
fun Mapqn_solution.getP2(j: Int, nj: Int, k: Int, i: Int, ni: Int, h: Int): Double {
    return getVariable("p2_${j}_${nj}_${k}_${i}_${ni}_$h")
}

fun Mapqn_solution.getMarginalP2(j: Int, nj: Int, k: Int): Double {
    return getVariable("p2_${j}_${nj}_${k}_${j}_${nj}_$k")
}