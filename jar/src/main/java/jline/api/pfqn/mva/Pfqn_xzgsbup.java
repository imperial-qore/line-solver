...
public final class Pfqn_xzgsbup {
    private Pfqn_xzgsbup() {}

    /**
     * Computes the upper Geometric Square-Root Bound (GSB) for the throughput of the given
    ...
    public static double pfqn_xzgsbup(Matrix L, double N, double Z) {
        ...
        for (int i = 0; i < M; i++) {
            if (L.get(i) < maxL) {
                R += (L.get(i) - maxL) * Pfqn_qzgbup.pfqn_qzgbup(L, N - 1, Z, i);
            }
        }
