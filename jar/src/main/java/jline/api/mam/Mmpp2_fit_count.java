 ...
 */
public final class Mmpp2_fit_count {
    private Mmpp2_fit_count() {}

    public static Matrix[] mmpp2_fit_count(final double mu, final double bt1, final double bt2,
        ...
        if (Math.abs(h) < 1e-4) {
            ...
            if (w_val < w_min || w_val > w_max) {
                ...
                double newR1 = u + (x - u) / 2;
                double newR2 = x - newR1;
                double delta = Math.sqrt(z_val / (2 * newR1 * newR2));
                l2 = mu - newR2 / x * delta;
                l1 = l2 + delta;
            } else {
