/**
 * @file Cramer-von Mises statistical distance test
 *
 * Implements the Cramer-von Mises test statistic for comparing two empirical distributions
...
public final class Ms_cramer_von_mises {
    private Ms_cramer_von_mises() {}

    /**
     * Cramer-von Mises distance between two empirical distributions.
     */
    public static double ms_cramer_von_mises(Matrix XX, Matrix YY) {
        if (XX.getNumCols() > 1 || YY.getNumCols() > 1) {
            ...
            for (int col = 0; col < XX.getNumCols(); col++) {
                ...
                double cvm = ms_cramer_von_mises_1D(xCol, yCol);
                maxCVM = Math.max(maxCVM, cvm);
            }
            return maxCVM;
        }
    ...
    private static double ms_cramer_von_mises_1D(Matrix XX, Matrix YY) {
        ...
        // Combine samples and create indicators (value, X_indicator, Y_indicator)
