...
public class M2M {
    ...
    public Network JSIM2LINE(String filename, String modelName) {
        ...
        for (int i = 0; i < node_name.size(); i++) {
            // cast Node objects into Element; equivalent to MATLAB "xsection_javaClass{i} = {xsection_i{i}.ATTRIBUTE};"
                                    ...
                                    switch (xsection_javaClass
                                        ...
                                        case "Server":
                                            ...
                                            if (strategy.get(i) == SchedStrategy.SEPT) {
                                                // SEPT scheduling params applied later, when schedparams are processed
                                                break;
                                            }
                                            break;
                                        case "PSServer":
                                            strategy_sub = getSubParameter(xsection_par.get(i).get(1));
                                            // PSServer weights/priorities applied later via schedparams; assumed identical across classes
                            ...
                            for (int j = 0; j < ninputs; j++) {
                                ...
                                for (int k = 0; k < nclasses; k++) {
                                    ...
                                    if (inhibit == -1 || inhibit == 0) {
                                        // 0/-1 -> no-inhibition sentinel: see _kb/06-solver-catalog.md (SSA/NRM, inhibitor arcs)
                        ...
                        switch (xarv_statdistribattr.get(i).get(r).get(0).getNamedItem("name").getNodeValue()) {
                            ...
                            case "Phase-Type":
                                ...
                                if (Matrix.tril(T, -1).any()) {
                                    ...
                                    ax1 = APH.fitMeanAndSCV(Map_mean.map_mean(T, D), Map_var.map_var(T, D));
                                } else {
                            ...
                            case "Phase-Type":
                                ...
                                if (Matrix.tril(T).any()) {
                                    line_warning(
                                            ...
                                            "The input model uses a general PH distribution, which is not yet supported in LINE. Fitting the first three moments into an APH distribution.");
                                    Matrix D = Matrix.negative(T).mult(Matrix.ones(T.getNumCols(), 1)).mult(alphaMatrix);
                                    ax1 = APH.fitMeanAndSCV(Map_mean.map_mean(T, D), Map_scv.map_scv(T, D));
                                } else {
                                    ax1 = new APH(alphaMatrix, T);
                                }
