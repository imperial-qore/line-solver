...
public class FCRegionExamples {
    ...
    public static void fcr_lossn() {
        ...
        for (int r = 0; r < 2; r++) {
            ...
            double tputNC = getTableValue(avgTableNC, "Delay", className, avgTableNC.getTput());
            double tputJMT = getTableValue(avgTableJMT, "Delay", className, avgTableJMT.getTput());
    ...
    /**
     * Locates the (station, class) row of an AvgTable by name and returns the
     * corresponding value from the given metric column.
     */
    private static double getTableValue(NetworkAvgTable table, String station,
                                        String jobClass, java.util.List<Double> column) {
        java.util.List<String> stations = table.getStationNames();
        java.util.List<String> classes = table.getClassNames();
        for (int i = 0; i < stations.size(); i++) {
            if (stations.get(i).equals(station) && classes.get(i).equals(jobClass)) {
                return column.get(i);
            }
        }
        throw new IllegalArgumentException("Row not found in AvgTable: " + station + "/" + jobClass);
    }
