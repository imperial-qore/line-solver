...
public class CyclicPollingModel {
     ...
     *
     * This example demonstrates a polling system with:
     * - 2 statistically identical job classes (symmetric system)
     * - Poisson arrivals and exponential service times
     * - Decrementing (semiexhaustive) polling strategy
     * - Identical exponential switchover times between classes
     *
     * The symmetric decrementing system admits the exact closed-form mean waiting
     * time of Pittel (1973) and Takagi (1984), used by the MVA polling analyzer.
     *
     * @return network model with symmetric decrementing polling
     */
    public static Network polling_decrementing() {
        Network model = new Network("M[2]/M[2]/1-Decrementing");
        ...
        // Block 2: classes (symmetric)
        OpenClass oclass1 = new OpenClass(model, "myClass1");
        source.setArrival(oclass1, new Exp(0.4));
        queue.setService(oclass1, new Exp(1.0));

        OpenClass oclass2 = new OpenClass(model, "myClass2");
        source.setArrival(oclass2, new Exp(0.4));
        queue.setService(oclass2, new Exp(1.0));

        queue.setPollingType(PollingType.DECREMENTING);
        queue.setSwitchover(oclass1, new Exp(10.0));
        queue.setSwitchover(oclass2, new Exp(10.0));
