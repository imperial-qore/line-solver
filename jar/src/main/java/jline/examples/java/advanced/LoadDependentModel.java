...
public class LoadDependentModel {
    ...
    public static Network ld_class_dependence() {
        ...
        // eta_i(n) reads the class-1 marginal only and returns a scalar shared
        // by every class. Because it depends on a foreign class marginal rather
        // than the own-class one, it is NON-product-form and must be declared via
        // setJointDependence (not setClassDependence, which is reserved for the
        // product-form beta_{i,r}(n_{i,r}); e.g. a per-class return
        // [min(ni0,c), 1] reading only each class's own count).
