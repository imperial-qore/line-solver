...
import jline.solvers.wrappers.qns.QNS;
...
public class GettingStarted {
     ...
     * <p>
     * Models an M/M/1 queue with arrival rate &lambda; = 0.5 and an uncertain
     * service rate represented by a {@link jline.lang.processes.Prior} with 30
     * Gaussian-weighted alternatives over [0.7, 2.5] centred at &mu; = 1.3.
     * The {@link UQ} solver wraps an inner solver (here MVA), runs it
     * once per alternative, and aggregates the per-alternative results into
     * prior-weighted means and per-alternative empirical posterior CDFs.
     *
     * @return the M/M/1 network with the Prior service distribution
     */
    public static Network tut12_bayes_analysis() {
        // Block 1: build the model with an uncertain service rate
        ...
        java.util.List<jline.lang.processes.Distribution> alternatives = new java.util.ArrayList<>();
        ...
        queue.setService(jobClass, new jline.lang.processes.Prior(alternatives, priorProbs));

        model.link(Network.serialRouting(source, queue, sink));

        // Block 2: solve via UQ wrapping MVA
        UQ post = new UQ(model, m -> new MVA(m));
        post.runAnalyzer();

        // Block 3: prior-weighted average and per-alternative summary
        post.getAvgTable().print();
        post.getPosteriorTable().print();

        // Block 4: extract posterior distribution of response time at the queue
        SolverUQ.EmpiricalCDF respDist = post.getPosteriorDist("R", queue, jobClass);
        System.out.printf("E[R] at Queue = %.4f%n", respDist.getMean());
     ...
     * <p>
     * Source -> Dispatcher (Router) -> Server[1..M] -> Sink, with a single
     * open class. The example contrasts two equivalent ways to build the
     * model: the one-liner {@link Network#clusterPs(Matrix, Matrix)}
     * factory, and the chainable {@link Cluster} builder, which also
     * exposes helpers to compare dispatching policies on the same cluster.
     *
     * @return the open cluster Network built via the one-liner factory
     */
    public static Network tut13_cluster() {
        ...
        // (JMT is XML-driven; LDES is the SSJ-backed subprocess simulator; SSA is
        // LINE's native next-reaction-method stochastic simulator).
        System.out.println("\n=== JMT ===");
        new JMT(cluster.build(), "seed", 23000, "samples", 20000).getAvgTable().print();
        System.out.println("=== LDES ===");
