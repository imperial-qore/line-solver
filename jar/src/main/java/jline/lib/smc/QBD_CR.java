...
public final class QBD_CR {
    private QBD_CR() {}

    /**
     * Convenience alias for callers that expect a {@code solve(...)} entry point
     * on the QBD_CR class. Delegates to {@link #QBD_CR}.
     */
    public static Map<String, Matrix> solve(Matrix A0arg, Matrix A1arg, Matrix A2arg,
        ...
        return QBD_CR(A0arg, A1arg, A2arg, MaxNumIt_, Verbose_, Mode_, RAPComp_);
    }

    public static Map<String, Matrix> QBD_CR(Matrix A0arg, Matrix A1arg, Matrix A2arg,
                                             Integer MaxNumIt_, Integer Verbose_,
                                             String Mode_, Integer RAPComp_) {
        ...
        if (!RAPComp) {
            ...
            // A QBD with RAP components has blocks whose off-diagonal entries
            // may be negative, so the Markovian validation of QBD_ParsePara
            // rejects every valid RAP instance ("the matrix A1 contains
            // negative data"). MATLAB QBD_CR.m calls QBD_RAP_ParsePara here.
            QBD_RAP_ParsePara.QBD_RAP_ParsePara(A0, A1, A2);
            A0.scaleEq(1 / lamb);
            A1.scaleEq(1 / lamb);
            A1 = A1.add(1.0, Matrix.eye(m));
            A2.scaleEq(1 / lamb);
        ...
        while (check > FastMath.pow(10.0, -14.0) && numit < MaxNumIt) {
            Matrix Atemp = Matrix.eye(m).add(-1.0, A).inv();
