...
/**
 * Optimize the replication (fan-out replicas) of an LQN Task (integer). Mirrors
 * native-Python {@code TaskReplication}.
 */
public class TaskReplication extends LqnDecisionVariable {
    ...
    public TaskReplication(Task task, int minValue, int maxValue) {
        this(task.getName(), minValue, maxValue, task.getName() + "_replication");
    }

    public TaskReplication(String taskName, int minValue, int maxValue) {
        this(taskName, minValue, maxValue, taskName + "_replication");
    }
    ...
    public void applyLqn(LayeredNetwork model, Object value) {
        ...
        if (t != null) {
            t.setReplication(((Number) value).intValue());
        }
    }
    ...
    public Object currentValue(Object model) {
        ...
        return (t != null) ? Integer.valueOf(t.getReplication()) : null;
    }
}
