...
public final class INAPResult {
    ...
    // The matrix-geometric tail of each phase-expanded open component; null
    // entries are the single-phase components, whose tail is the scalar rhoProc.
    public final QbdTail[] geomData;
    public final double rcatRes;

    public INAPResult(Matrix x, List<Matrix> pi, List<Matrix> Q, int iter) {
        this(x, pi, Q, iter, null, null, null, 0.0);
    }

    public INAPResult(Matrix x, List<Matrix> pi, List<Matrix> Q, int iter,
                      double[] rhoProc, boolean[] isGeomProc, QbdTail[] geomData, double rcatRes) {
        this.x = x;
        this.pi = pi;
        this.Q = Q;
        this.iter = iter;
