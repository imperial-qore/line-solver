...
public class NetworkGenerator {
     ...
     * Note: the {@code randGraph} topology uses a separate static random source; use a
     * deterministic topology (e.g. {@code cyclicGraph}) for fully reproducible output.
     *
     * @param seed the seed value
     * @return this generator (for chaining)
     */
    public NetworkGenerator setSeed(long seed) {
    ...
    public Network generate(int numQueues, Integer numDelays, int numOClass, int numCClass) {
        ...
        // Rejection sampling: random routing plus random class switching can
        // occasionally strand a closed chain (a station self-loops with
        // probability 1), which is not a valid closed queueing network -- its
        // visit ratios are undefined and sn_refresh_visits rejects it. Such a
        // draw is discarded and resampled rather than returned. A bounded number
        // of attempts guards against a configuration that can never yield a
        // recurrent closed chain (e.g. a topology that is not strongly connected).
        RuntimeException lastReject = null;
        for (int attempt = 0; attempt < MAX_GENERATE_ATTEMPTS; attempt++) {
            ...
            // Initialize default states if requested. This is the first consumer
            // that materializes sn and hence validates chain recurrence.
            if (initializeStates) {
                try {
                    initDefaultStates(model);
                } catch (RuntimeException e) {
                    if (isReducibleRoutingReject(e)) {
                        lastReject = e;
                        continue; // resample: absorbing closed chain, invalid model
                    }
                    throw e;
                }
            }
        ...
        throw new RuntimeException("NetworkGenerator: could not generate a model with"
                + " recurrent closed-chain routing in " + MAX_GENERATE_ATTEMPTS
                + " attempts. The requested topology may not admit a strongly connected"
                + " closed chain.", lastReject);
    }

    /** Maximum number of resampling attempts before giving up on a valid draw. */
    private static final int MAX_GENERATE_ATTEMPTS = 100;

    /**
     * True when the exception is the sn_refresh_visits rejection of a genuinely
     * absorbing (reducible) closed chain, i.e. a random draw that is not a valid
     * closed network and should be resampled -- not an unrelated failure.
     */
    private static boolean isReducibleRoutingReject(RuntimeException e) {
        String msg = e.getMessage();
        return msg != null && msg.contains("sn_refresh_visits")
                && msg.contains("no recurrent flow");
    }
