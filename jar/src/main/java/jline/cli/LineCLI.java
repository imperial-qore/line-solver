...
public class LineCLI {
    ...
    private static void printDetailedHelp() {
        ...
        System.out.println("Type solver.libraries() for third-party dependencies, solver.citations() for references.");
        System.out.println("====================================================================");
        System.out.println();
        ...
        System.out.println("--cutoff <number>");
        System.out.println("    Scalar state-space cutoff for CTMC/SSA on open/mixed models.");
        System.out.println("    Example: --cutoff 1");
        System.out.println();
    ...
    private static boolean validateInputFormat(String format) {
        ...
        System.err.println("Valid formats: jsim, jsimg, jsimw, lqnx, xml, json");
        return false;
    }
    ...
    static {
        ...
        ANALYSIS_SOLVER_COMPAT.put("statevec", new HashSet<>(Arrays.asList("fluid")));
    }
    ...
    /**
     * Maps a CLI solver token onto the {@link SolverType} whose per-solver
     ...
     * on to 1e-15.
     *
     * A token with no matching type maps to null, which SolverOptions treats as
     * "generic defaults only" -- the previous behaviour, kept as the fallback.
     *
     * @param solver solver token as given to -s
     * @return the matching SolverType, or null when the token has none
     */
    private static SolverType solverTypeOf(String solver) {
        if (solver == null) {
            return null;
        }
        // Layered tokens (ln, ln.mva, ln.comom) select the layer solver whose
        // options the LN solver forwards, so they take that solver's defaults.
        if (solver.equals("ln") || solver.equals("ln.comom")) {
            return SolverType.NC;
        }
        if (solver.equals("ln.mva")) {
     ...
     *
     * @param solver        solver token (mva, nc, ctmc, fluid, mam, ssa, jmt, ldes, auto)
     ...
     * @return the constructed solver, or null if the token is unknown
     */
     ...
     *
     * @param solver        solver token (ln, ln.mva, ln.comom, lqns, mva, nc, auto)
    ...
    private static Solver buildLayeredSolver(String solver, LayeredNetwork model,
        ...
        switch (solver) {
            ...
            case "ln.comom":
            case "auto":
                // Default layered solver uses the normalizing-constant layer
                // solver (this is what 'auto' resolves to for LQN inputs).
                return new SolverLN(model, (net) -> new SolverNC(net, solverOptions), solverOptions);
            case "mva":
            case "ln.mva":
                return new SolverLN(model, solverOptions);
            default:
                line_error(mfilename(new Object(){}), "Unknown solver type: " + solver);
                return null;
        }
    ...
    private static boolean validateSolverCompatibility(String inputFormat, String solver) {
        ...
        if (inputFormat.equals("lqnx") || inputFormat.equals("xml")) {
            ...
            String[] validJsonSolvers = {"ctmc", "fluid", "jmt", "ldes", "mam", "mva",
                "nc", "ssa", "ln", "ln.mva", "ln.comom", "lqns"};
            ...
            System.err.println("Valid solvers for json: ctmc, fluid, jmt, ldes, mam, mva, nc, ssa, ln, ln.mva, ln.comom, lqns");
            return false;
        } else {
    ...
    public static String parseArgs(String[] varargin) throws IOException {
        ...
        // see _kb/12-interfaces-and-docs.md for LineCLI's per-solver-defaults + option-forwarding rationale
        if (!Double.isNaN(cutoffVal)) {
            solverOptions.cutoff(cutoffVal);
        }
        // see _kb/12-interfaces-and-docs.md for LineCLI's per-solver-defaults + option-forwarding rationale
        if (samplesVal > 0) {
            solverOptions.samples(samplesVal);
        }
        ...
        if (!Double.isNaN(warmupfracVal) && warmupfracVal > 0) {
            solverOptions.config.warmupfrac = warmupfracVal;
        }
        // see _kb/12-interfaces-and-docs.md for LineCLI's per-solver-defaults + option-forwarding rationale
        if (methodVal != null) {
            solverOptions.method(methodVal);
        }
        if (!Double.isNaN(tolVal)) {
            solverOptions.tol = tolVal;
        }
        if (!Double.isNaN(iterTolVal)) {
            solverOptions.iter_tol = iterTolVal;
        }
        if (iterMaxVal > 0) {
            solverOptions.iter_max = iterMaxVal;
        }
        ...
        switch (fileext) {
            ...
            case "jsim":
                ...
                solverObj = buildNetworkSolver(solver, (Network) model, solverOptions, randomSeed, verbosity);
                break;
            case "json":
                // see _kb/12-interfaces-and-docs.md for LineCLI's model-type/dispatch rationale
                ...
                if (loadedModel instanceof LayeredNetwork) {
                    ...
                    solverObj = buildLayeredSolver(solver, (LayeredNetwork) model, solverOptions);
                } else if (loadedModel instanceof Network) {
                    model = (Model) loadedModel;
                    solverObj = buildNetworkSolver(solver, (Network) model, solverOptions, randomSeed, verbosity);
                } else {
                    line_error(mfilename(new Object(){}),
                        "JSON input supports Network and LayeredNetwork models only.");
                }
                break;
            case "lqnx":
            case "xml":
        ...
        // see _kb/12-interfaces-and-docs.md (SolverSSA post-dispatch method exposed for lang="java" callers)
        if (solverObj instanceof jline.solvers.ssa.SolverSSA && !analysisResults.isEmpty()) {
            jline.solvers.SolverResult ssaRes = ((jline.solvers.ssa.SolverSSA) solverObj).getResults();
            if (ssaRes != null && ssaRes.method != null) {
                analysisResults.put("method", ssaRes.method);
            }
        }
     ...
     * Non-finite values are emitted as null to keep the output valid JSON.
     */
