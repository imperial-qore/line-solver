                ...
                if (spaceSrv.get(jobClass) > 0) {
                    ...
                    switch (sn.routing.get(sn.nodes.get(ind)).get(sn.jobclasses.get(jobClass))) {
                        ...
                        case WRROBIN: {
                            ...
                            // weighted-outlink cycle (destinations repeated by weight).
                            int nvar_sum_w = 0;
                            for (int c = 0; c <= sn.nclasses + jobClass; c++) {
                                nvar_sum_w += (int) sn.nvars.get(ind, c);
                            }
                            int spaceVarIdxW = nvar_sum_w - 1;
                            Matrix wolR = sn.nodeparam.get(sn.nodes.get(ind)).weightedOutlinks.get(sn.jobclasses.get(jobClass));
                            int cycR = (wolR == null) ? 0 : (int) wolR.length();
                            if (cycR > 0 && spaceVarIdxW >= 0 && spaceVarIdxW < spaceVar.length()) {
                                int posR = (int) spaceVar.get(spaceVarIdxW);
                                int nxtR = (posR >= 1 && posR < cycR) ? (posR + 1) : 1;
                                spaceVar.set(spaceVarIdxW, nxtR);
                            }
                            break;
                        }
                    }
