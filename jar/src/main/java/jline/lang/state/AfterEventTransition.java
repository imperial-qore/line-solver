    ...
    static Ret.EventResult afterEventTransition(NetworkStruct sn, int ind, EventType event, int jobClass, boolean isSimulation,
        ...
        switch (event) {
            ...
            case PHASE:
                ...
                // A Transition state is [idle(nmodes), phases(sum(K)), fired(nmodes), vars].
                // The fired columns must be carried through a phase move: the state space
                // rows are built with them, so emitting a row without them can never be
                // matched by matchrow and the phase advance is silently dropped (the mode
                // then never progresses and its sub-chain becomes absorbing).
                // Mirrors matlab/src/lang/+State/afterEventTransition.m, which always
                // emits [space_buf, space_srv, space_fired, space_var].
                TransitionNodeParam firedParam = (TransitionNodeParam) sn.nodeparam.get(sn.nodes.get(ind));
                int nmodesFired = firedParam.nmodes;
                int srvEndColFired = (int) (nmodesFired + K.elementSum());
                int firedEndColFired = srvEndColFired + nmodesFired;
                Matrix spaceFired;
                if (inspace.getNumCols() >= firedEndColFired) {
                    spaceFired = Matrix.extractColumns(inspace, srvEndColFired, firedEndColFired, null);
                } else {
                    spaceFired = new Matrix(inspace.getNumRows(), nmodesFired);
                    spaceFired.zero();
                }
                        ...
                        if (anyEn) {
                            for (int kdest = 0; kdest < K.get(mode); kdest++) {
                                if (kdest != k) { // setdiff(1:K(mode),k) equivalent
                                    ...
                                    Matrix spaceFiredK = new Matrix(0, 0);
                                    ...
                                    for (int row = 0; row < en.getNumRows(); row++) {
                                        if (en.get(row, 0) == 1) {
                                            ...
                                            if (spaceSrvK.isEmpty()) {
                                                ...
                                                spaceFiredK = Matrix.concatRows(spaceFiredK, spaceFiredRow, null);
                                            }
                                        }
                                    }
                                    ...
                                    leftBottomTrans = Matrix.concatColumns(leftBottomTrans, spaceFiredK, null);
