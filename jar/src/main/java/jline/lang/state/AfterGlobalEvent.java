...
public class AfterGlobalEvent implements Serializable {
    ...
    public static AfterGlobalEventResult afterGlobalEvent(NetworkStruct sn, int ind, List<Matrix> glspace, GlobalSync glevent, boolean isSimulation) {
        ...
        if (sn.nodetype.get(ind) == NodeType.Transition && glevent.getActive().size() > 0) {
            ...
            if (sn.nodetype.get(ind) == NodeType.Transition) {
                ...
                if (fK.hasNaN()) {
                    ...
                    List<Mode> modeObjs = ((Transition) sn.nodes.get(ind)).getModes();
                    for (int m = 0; m < nmodes; m++) {
                        // firingproc is keyed by Mode, not by mode index: an int
                        // autoboxes to an Integer that never matches, so
                        // containsKey was unconditionally false and every mode
                        // silently fell through to the single-phase fallback.
                        Mode modeObjK = m < modeObjs.size() ? modeObjs.get(m) : null;
                        if (transParam.firingproc != null && modeObjK != null
                            && transParam.firingproc.containsKey(modeObjK)
                            && transParam.firingproc.get(modeObjK) != null && !transParam.firingproc.get(modeObjK).isEmpty()) {
                            Matrix D0 = transParam.firingproc.get(modeObjK).get(0);
                            if (D0 != null) {
                ...
                switch (glevent.getActive().get(0).getEvent()) {
                    ...
                    case FIRE:
                        result = State.handleFireEvent(sn, ind, glevent, glspace, outglspace, inspace, spaceBuf, spaceSrv,
                                spaceVar, fK, fKs, mode, transParam, R, isSimulation);
                        break;
                }
                if (result != null) {
        ...
        if (isSimulation && outspace.getNumRows() > 1) {
            ...
            if (outcomp.getNumRows() > firingCtr) {
                outcomp = Matrix.extractRows(outcomp, firingCtr, firingCtr + 1, null);
            }
        ...
        if (outcomp.getNumRows() != outrate.getNumRows()) {
            Matrix padded = new Matrix(outrate.getNumRows(), 1);
            padded.zero();
            for (int i = 0; i < Math.min(outcomp.getNumRows(), outrate.getNumRows()); i++) {
                padded.set(i, 0, outcomp.get(i, 0));
            }
            outcomp = padded;
        }

        return new AfterGlobalEventResult(outglspace, outrate, outprob, outcomp);
    }
    ...
    public static class AfterGlobalEventResult {
        ...
        /**
         * Column vector aligned with outrate: 1.0 when that outcome is a firing
         * completion (a D1 firing of the active mode, which applies the PRE/POST
         * place updates), 0.0 otherwise (e.g. every ENABLE outcome). Callers must
         * not re-derive this from the place markings: a transition whose firing
         * outcome returns exactly what its enabling condition consumed leaves every
         * marking invariant, yet still completes at a nonzero rate.
         */
         ...
         * @param outcomp Completion flag for each resulting state transition
         */
