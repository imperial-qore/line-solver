...
public enum HeteroSchedPolicy {
    ...
    /**
     * Converts this policy to JMT's descriptive label (e.g. "ALIS (Assign
     * Longest Idle Server)"). JMT's engine compares schedulingPolicy XML
     * values against these long forms, so the JMT writer must use this method
     * rather than {@link #toText()}.
     *
     * @return the JMT-compatible descriptive label
     */
    public String toJMTText() {
        switch (this) {
            case ORDER:    return "Order (Assign according to order below)";
            case ALIS:     return "ALIS (Assign Longest Idle Server)";
            case ALFS:     return "ALFS (Assign Least Flexible Server)";
            case FAIRNESS: return "Fairness (Move back server type when used)";
            case FSF:      return "FSF (Fastest Servers First)";
            case RAIS:     return "RAIS (Random Assignment to Idle Servers)";
            default:       return "Order (Assign according to order below)";
        }
    }

    public static String toJMTText(HeteroSchedPolicy policy) {
        return policy.toJMTText();
    }
}
