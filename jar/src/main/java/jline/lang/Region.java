...
public class Region implements Serializable {
    ...
    public Map<JobClass, Double> classSize;  // per-class memory footprint; double so fractional sizes survive the MATLAB->Java transfer
    ...
    public Region(List<Node> nodes, List<JobClass> classes) {
        ...
        for (JobClass jobClass : classes) {
            ...
            this.classSize.put(jobClass, 1.0);
    ...
    public double getClassSize(JobClass jobClass) {
        return classSize.getOrDefault(jobClass, 1.0);
    }
     ...
     *
     * @return a two-element array {A, b} holding the constraint matrix A
     *         (C x K) and the capacity vector b (C x 1); each entry is null
     *         if the corresponding constraint has not been set
     */
    public Matrix[] getLinearConstraints() {
        return new Matrix[]{constraintA, constraintB};
    }
