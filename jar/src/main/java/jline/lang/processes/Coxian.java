...
public class Coxian extends Markovian {
    ...
    public Coxian(List<Double> mu0, List<Double> phi0) {
        ...
        if (this.getNumberOfPhases() == 2) {
            ...
            matrix2.set(1, 0, mu2);
            MatrixCell D = Map_normalize.map_normalize(matrix1, matrix2);
            rep.set(0, D.get(0));
            rep.set(1, D.get(1));
        } else {
            Matrix mu = getMu();
            ...
            rep = Map_normalize.map_normalize(expression3, expression6);
        }
        setProcess(rep);
    }
    ...
    public static Coxian fitCentral(double mean, double var, double skew) {
        ...
        if (Math.abs(1 - Map_scv.map_scv(cx.D(0), cx.D(1)) / scv) > 0.01) {
    ...
    public double getMean() {
        if (this.getNumberOfPhases() == 2) {
            ...
            return Map_mean.map_mean(rep.get(0), rep.get(1));
        }
    }
