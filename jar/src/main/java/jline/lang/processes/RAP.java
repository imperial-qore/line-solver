...
import static jline.lib.butools.ph.CheckRAPRepresentation.checkRAPRepresentation;
...
public class RAP extends Markovian {
     ...
     *
     * <p>Sampling propagates the conditional phase vector across events, so the
     * returned inter-event times reproduce the autocorrelation of the process
     * and not only its ME marginal. The CTMC walk inherited from
     * {@link Markovian} (map_sample) is not used because it presumes the
     * non-negativity of the MAP matrices, which a general RAP does not satisfy.</p>
     *
     * @param n      the number of samples to generate
     * @param random the random number generator to use
     * @return array of n correlated inter-event times
     */
    @Override
    public double[] sample(int n, java.util.Random random) {
        ...
        return jline.api.mam.Rap_sample.rap_sample(getH0(), getH1(), n, rng);
    }
