...
public class BMAP extends MarkedMAP implements Serializable {
    ...
    /**
     * Generates a random BMAP with the given number of phases and maximum
     * batch size. A random order-{@code order} MAP is drawn and its arrival
     * matrix is split across batch sizes {@code 1..maxBatchSize} using a random
     * batch-size probability mass function.
     *
     * @param order        the number of phases of the underlying MAP
     * @param maxBatchSize the largest batch size K (batches range over 1..K)
     * @return a random BMAP
     */
    public static BMAP rand(int order, int maxBatchSize) {
        MatrixCell map = jline.api.mam.Map_rand.map_rand(order);
        Matrix D0 = map.get(0);
        Matrix D1 = map.get(1);
        int[] batchSizes = new int[maxBatchSize];
        double[] pmf = new double[maxBatchSize];
        for (int k = 0; k < maxBatchSize; k++) {
            batchSizes[k] = k + 1;
            pmf[k] = Math.random();
        }
        return fromMAPWithBatchPMF(D0, D1, batchSizes, pmf);
    }
