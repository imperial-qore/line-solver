...
public class Cox2 extends Coxian {
    ...
    public static Cox2 fitCentral(double mean, double var, double skew) {
        ...
        double phi1 = 1 - mu21 * e1 + mu21 / mu11;
        double phi2 = 1 - mu22 * e1 + mu22 / mu12;

        Cox2 cx = null;
        if (phi1 >= 0 && phi1 <= 1 && mu11 >= 0 && mu21 >= 0) {
            // if the first solution is feasible
            cx = new Cox2(mu11, mu21, phi1);
        } else if (phi2 >= 0 && phi2 <= 1 && mu12 >= 0 && mu22 >= 0) {
            // if the second solution is feasible
            cx = new Cox2(mu12, mu22, phi2);
        } else {
