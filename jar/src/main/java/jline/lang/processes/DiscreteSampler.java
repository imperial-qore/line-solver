...
public class DiscreteSampler extends DiscreteDistribution {
    ...
    /**
     * Constructs a discrete distribution from a finite probability vector p at
     * the points specified in vector x, given as plain arrays.
     *
     * @param p - the probability (weight) of each item
     * @param x - the value of each item
     */
    public DiscreteSampler(double[] p, double[] x) {
        this(new Matrix(p), new Matrix(x));
    }
