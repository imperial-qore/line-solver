...
import jline.api.mam.Map_timereverse;
...
public class MAP extends MarkovModulated implements Serializable {
    ...
    public static MAP randn(int order, double mu, double sigma) {
        MatrixCell map = jline.api.mam.Map_randn.map_randn(order, mu, sigma);
        return new MAP(map);
    }
    ...
    public double evalCDF(double t) {
        ...
        try {
            ...
            Matrix pie = jline.api.mam.Map_pie.map_pie(D(0), D(1));
            Matrix T = D(0);
    ...
    public double getMean() {
        double E1 = Map_moment.map_moment(D(0), D(1), 1);
        return E1;
    }
    ...
    public double getSkewness() {
        double E1 = Map_moment.map_moment(D(0), D(1), 1);
        double E2 = Map_moment.map_moment(D(0), D(1), 2);
        double E3 = Map_moment.map_moment(D(0), D(1), 3);
    ...
    public MAP toTimeReversed() {
        MatrixCell reversed = Map_timereverse.map_timereverse(this.process);
    ...
    public Matrix evalACFT(int[] lags, double timescale) {
        return new Matrix(jline.api.mam.Map_acfc.map_acfc(D(0), D(1), lags, timescale));
    }
