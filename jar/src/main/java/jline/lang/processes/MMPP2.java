...
public class MMPP2 extends MarkovModulated implements Serializable {
    ...
    public static MMPP2 fitRawMomentsAndACFDecay(double m1, double m2, double m3, double gamma2) {
        ...
        MatrixCell m = Mmpp2_fit.mmpp2_fit(m1, m2, m3, gamma2);
        // see _kb/01-model-classes.md (Java process-construction notes) for rationale
        return new MMPP2(m.get(1).get(0, 0), m.get(1).get(1, 1), m.get(0).get(0, 1), m.get(0).get(1, 0));
    }
    ...
    public Matrix evalACFT(int[] lags, double timescale) {
        return new Matrix(Map_acfc.map_acfc(D(0), D(1), lags, timescale));
    }
    ...
    public double[] sample(int n, Random random) {
        if (Map_isfeasible.map_isfeasible(getProcess())) {
