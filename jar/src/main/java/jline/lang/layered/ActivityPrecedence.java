...
public class ActivityPrecedence {
    ...
    public static ActivityPrecedence AndJoin(List<Activity> preActs, Activity postAct) {
        Matrix quorum = new Matrix(0, 0); // Empty matrix denotes "no quorum" (wait for all)
    ...
    /**
     * Returns the effective quorum count of an AND-join, i.e. the number k of predecessors
     * that must complete before the join fires.
     *
     * <p>The quorum is carried in preParams. A null, empty or non-singleton matrix means
     * "no quorum", in which case all nPreActs predecessors are required and k = nPreActs.
     * Values outside [1, nPreActs] are clamped, since a quorum larger than the number of
     * branches can never be met and a quorum below one is meaningless.</p>
     *
     * @param preParams the preParams matrix of the precedence
     * @param nPreActs  the number of predecessor activities
     * @return the quorum count k, in [1, nPreActs]
     */
    public static int getQuorumCount(Matrix preParams, int nPreActs) {
        if (nPreActs <= 0) {
            return 0;
        }
        if (preParams == null || preParams.isEmpty() || preParams.length() != 1) {
            return nPreActs;
        }
        int quorum = (int) Math.round(preParams.get(0));
        if (quorum < 1 || quorum > nPreActs) {
            return nPreActs;
        }
        return quorum;
    }
