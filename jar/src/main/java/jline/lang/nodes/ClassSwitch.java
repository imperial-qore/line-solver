...
public class ClassSwitch extends Node implements Serializable {
    ...
    public void removeJobClass(JobClass jobClass) {
        ...
        StatelessClassSwitcher scs = (StatelessClassSwitcher) this.server;
        List<JobClass> oldClasses = this.model.getClasses();
        int r = oldClasses.indexOf(jobClass);
        if (r < 0) {
            return;
        }
        int K = oldClasses.size();
        ...
        int Kn = remaining.size();
        Matrix newCs = new Matrix(Kn, Kn);
        for (int i = 0; i < Kn; i++) {
            for (int j = 0; j < Kn; j++) {
                newCs.set(i, j, scs.applyCsFun(remaining.get(i), remaining.get(j)));
            }
        }
        java.util.List<JobClass> newClasses = new java.util.ArrayList<JobClass>();
        for (int idx : remaining) {
            newClasses.add(oldClasses.get(idx));
        }
        scs.updateClasses(newClasses);
        scs.updateClassSwitch(newCs);
    }
