...
public class Transition extends ServiceNode {
    ...
    // Marking-dependent firing-rate multiplier g_mode(marking); absent == unit.
    public Map<Mode, SerializableFunction<Matrix, Double>> firingRateDependence;
    ...
    public Transition(Network model, String nodeName) {
        ...
        this.firingRateDependence = new HashMap<>();
    ...
    public void addMode(Mode mode) {
        ...
        // Inhibitor thresholds default to +Inf (no inhibition), mirroring MATLAB
        // Transition.addMode (inhibitingConditions{end+1} = Inf*ones(nnodes,nclasses)).
        Matrix inhInit = new Matrix(nnodes, nclasses);
        inhInit.fill(Double.POSITIVE_INFINITY);
        this.inhibitingConditions.put(mode, inhInit);
        for (Mode m : modes) {
            Matrix I = this.inhibitingConditions.get(mode);
            if (I.getNumRows() != nnodes) {
                Matrix pad = new Matrix(nnodes - I.getNumRows(), I.getNumCols());
                pad.fill(Double.POSITIVE_INFINITY);
                Matrix.concatRows(I, pad, I);
            }
            if (I.getNumCols() != nclasses) {
                Matrix pad = new Matrix(I.getNumRows(), nclasses - I.getNumCols());
                pad.fill(Double.POSITIVE_INFINITY);
                Matrix.concatColumns(I, pad, I);
            }
        }
    ...
    /**
     * Marking-dependent firing-rate multiplier for a timed mode. g(m) takes the
     * node-indexed input-place marking matrix and returns a positive scalar; the
     * effective firing rate of an enabled binding becomes rate_base(mode)*g(m).
     * Exact only for memoryless firing, so the mode must be TIMED and
     * exponentially distributed (mirrors the PS/FCFS-only station dependence).
     */
    public void setFiringRateDependence(Mode mode, SerializableFunction<Matrix, Double> g) {
        if (g != null) {
            if (this.timingStrategies.get(mode) == TimingStrategy.IMMEDIATE) {
                throw new RuntimeException("Firing-rate dependence is not supported for IMMEDIATE modes; use setFiringWeights for immediate transitions.");
            }
            if (!(this.distributions.get(mode) instanceof Exp)) {
                throw new RuntimeException("Firing-rate dependence is supported only for exponentially distributed (memoryless) firing; set an Exp distribution on the mode first.");
            }
        }
        this.firingRateDependence.put(mode, g);
    }

    public SerializableFunction<Matrix, Double> getFiringRateDependence(Mode mode) {
        return this.firingRateDependence.get(mode);
    }
