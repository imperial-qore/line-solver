...
public class RandomManager {
    ...
    /**
     * Returns a {@link Random} adapter wrapping the parallel {@link MersenneTwister}
     * for the given solver/thread.
     *
     * @param solverId logical solver identifier
     * @param threadOffset offset to differentiate parallel workers
     * @return java.util.Random instance backed by MersenneTwister
     */
    public static Random getParallelRandomAsRandom(String solverId, int threadOffset) {
        final MersenneTwister mt = getParallelRandom(solverId, threadOffset);
        return new Random() {
            @Override
            protected int next(int bits) {
                return mt.nextInt() >>> (32 - bits);
            ...
            @Override
            public double nextDouble() { return mt.nextDouble(); }
            @Override
            public int nextInt() { return mt.nextInt(); }
            @Override
            public int nextInt(int bound) { return mt.nextInt(bound); }
            @Override
            public long nextLong() { return mt.nextLong(); }
            @Override
            public boolean nextBoolean() { return mt.nextBoolean(); }
            @Override
            public float nextFloat() { return mt.nextFloat(); }
            @Override
            public double nextGaussian() { return mt.nextGaussian(); }
        };
    }
