...
public class Maths {
    ...
    // RANDOM NUMBER GENERATION: now always via MersenneTwister through RandomManager (reproducible across all solvers)
    ...
    public static Matrix matrixExp(Matrix matrix) {
        ...
        Matrix array = new Matrix(result.getNumRows(), result.getNumCols());
        for (int i = 0; i < result.getNumRows(); i++) {
            for (int i1 = 0; i1 < result.getNumCols(); i1++) {
    ...
    public static laplaceApproxReturn laplaceapprox_h(Matrix x0, SerializableFunction<Matrix, Matrix> h) {
        ...
        // logI carries HALF the log-determinant: see _kb/03-api-layer.md ("jline.util.Maths.laplaceapprox_h")
        double I = infrad.get(0) * FastMath.sqrt(Math.pow(2 * FastMath.PI, d) / detnH);
        double logI = FastMath.log(infrad.get(0)) + ((double) d / 2 * FastMath.log(2 * FastMath.PI)) - 0.5 * FastMath.log(detnH);
    ...
    public static laplaceApproxComplexReturn laplaceapprox_h_complex(Matrix x0, SerializableFunction<Matrix, ComplexMatrix> h) {
        ...
        // See laplaceapprox_h: logI carries HALF the log-determinant.
        Complex logI = infrad.get(0).log().add((new Complex((double) d / 2 * FastMath.log(2 * FastMath.PI)))).subtract(detnH.log().multiply(0.5));
    ...
    public static Matrix multichoose(double n, double k) {
        // cooperative wall-clock budget checkpoint (LineTimeout): enforced inside the recursion, not just at the top-level call
        LineTimeout.checkpoint("Enumeration (multichoose)");
        ...
        if (n == 1) {
            ...
            for (int i = 0; i <= k; i++) {
                ...
                for (int j = 0; j < w.getNumRows(); j++) {
                    // Row-copy loops dominate a single recursion level on
                    // explosive spaces, so the budget checkpoint must sit here
                    LineTimeout.checkpoint("Enumeration (multichoose)");
                    tmpSSRow.set(j, 0, i);
    ...
    // RANDOM NUMBER GENERATION FUNCTIONS: core random number generation methods
    ...
    /**
     * Log-domain form of {@link #simplex_fun}: returns the exponent, i.e.
     * log(simplex_fun), without exponentiating. Kept in the log domain so the
     * integrand does not overflow for models with large normalizing constants.
     */
    public static double simplex_logfun(double[] x, Matrix L, Matrix N) {
        ...
        return N.mult(tmp).elementSum() + x_sum
                - (N.elementSum() + M) * FastMath.log(v.elementSum());
    }
     ...
     *
     * The literal weighted-average form
     * (x*exp(-alpha*x) + y*exp(-alpha*y)) / (exp(-alpha*x) + exp(-alpha*y))
     * overflows once alpha*|x| leaves the double range and then evaluates
     * Inf*0 = NaN. With lo = min(x,y), gap = |x-y| and w = exp(-alpha*gap) the
     * algebraically identical form (lo + hi*w)/(1 + w) = lo + gap*w/(1 + w)
     * keeps the exponent argument non-positive, so w stays in (0,1] and the
    ...
    public static double softmin(double x, double y, double alpha) {
        double lo = FastMath.min(x, y);
        double hi = FastMath.max(x, y);
        double gap = hi - lo;
        // exp(-t) underflows to exactly 0 for t > 745.13; beyond that softmin is min.
        if (!(gap < 745.0 / alpha)) {
            return lo;
        }
        double w = FastMath.exp(-alpha * gap);
    ...
    // SORTING AND UNIQUENESS FUNCTIONS: sorting and finding unique elements
    ...
    public static List<int[]> multichooseList(int n, int k) {
        // Cooperative wall-clock budget checkpoint (see LineTimeout)
        LineTimeout.checkpoint("Enumeration (multichooseList)");
        if (n == 1) {
