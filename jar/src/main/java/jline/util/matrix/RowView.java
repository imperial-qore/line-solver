...
public class RowView {
    ...
    RowView(DMatrixSparseCSC matrix, int rowIndex) {
        if (rowIndex < 0 || rowIndex >= matrix.getNumRows()) {
            throw new IndexOutOfBoundsException("Row index " + rowIndex + " out of bounds [0, " + matrix.getNumRows() + ")");
        }
        ...
        int[] tempCols = new int[matrix.getNumCols()]; // Worst case: all columns have this row
        double[] tempValues = new double[matrix.getNumCols()];
        int count = 0;
