...
public abstract class BaseMatrix implements Serializable {
     ...
     *
     * @return true for dense storage
     */
    public abstract boolean isDenseFormat();

    /**
     * Returns a CSC representation of this matrix: the live backing structure
     * when sparse, a conversion copy (explicit zeros dropped) when dense.
     *
     * @return CSC representation
     */
    public abstract DMatrixSparseCSC toCSC();

    /**
     * Returns a row-major representation of this matrix: the live backing
     * structure when dense, a conversion copy when sparse.
     *
     * @return row-major representation
     */
    public abstract DMatrixRMaj toRMaj();

    /**
     * Iterates the non-zero entries of this matrix in column-major order.
     * The returned {@link MatrixEntry} instance is reused across calls.
     *
     * @return iterator over non-zero entries
     */
    public abstract Iterator<MatrixEntry> nonZeroIterator();
