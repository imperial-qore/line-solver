...
public final class Matrix implements Serializable {
     ...
     * Not final: toDense()/toSparse() swap the storage representation in place.
     */
    private BaseMatrix delegate;

    /**
     * When true, {@link #autoFormat()} re-evaluates the storage format of
     * operation results by fill ratio (densify above {@link #DENSIFY_THRESHOLD},
     * sparsify below {@link #SPARSIFY_THRESHOLD}). On by default; disable via
     * {@link #setAutoFormatSwitch(boolean)} for format-deterministic results.
     */
    private static boolean autoFormatSwitch = true;

    /** Fill ratio above which autoFormat() densifies a sparse-backed result. */
    public static final double DENSIFY_THRESHOLD = 0.5;

    /** Fill ratio below which autoFormat() sparsifies a dense-backed result. */
    public static final double SPARSIFY_THRESHOLD = 0.1;

    /**
     * Enables or disables automatic storage-format re-evaluation of operation
     * results (hysteresis between the densify and sparsify thresholds).
     *
     * @param enabled true to enable auto-switching
     */
    public static void setAutoFormatSwitch(boolean enabled) {
        autoFormatSwitch = enabled;
    }
     ...
     */
    public static boolean isAutoFormatSwitch() {
        return autoFormatSwitch;
    }

    /**
     * Re-evaluates the storage format of this matrix by fill ratio when
     * {@link #setAutoFormatSwitch(boolean)} is enabled; no-op otherwise.
     * The hysteresis band between the two thresholds avoids oscillation.
     *
     * @return this matrix, possibly with switched storage
     */
    public Matrix autoFormat() {
        if (!autoFormatSwitch) {
            return this;
        }
        long total = (long) getNumRows() * getNumCols();
        if (total == 0) {
            return this;
        }
        double fill = (double) getNonZeros() / total;
        if (!isDense() && fill > DENSIFY_THRESHOLD) {
            return toDense();
        }
        if (isDense() && fill < SPARSIFY_THRESHOLD) {
            return toSparse();
        }
        return this;
    }
    ...
    public Matrix(int numRows, int numCols, int arrayLength) {
        this.delegate = new SparseMatrix(numRows, numCols, arrayLength);
    }
    ...
    public Matrix(int numRows, int numCols) {
        this.delegate = new SparseMatrix(numRows, numCols);
    }
    ...
    public Matrix(Matrix matrix) {
        this.delegate = matrix.delegate.copy();
    }
    ...
    public Matrix(DMatrix matrix) {
        if (matrix instanceof DMatrixRMaj) {
            this.delegate = new DenseMatrix((DMatrixRMaj) matrix);
        } else {
            this.delegate = new SparseMatrix((DMatrixSparseCSC) matrix);
        }
    }
    ...
    public Matrix(DMatrixRMaj matrix) {
        this.delegate = new DenseMatrix(matrix);
    }
    ...
    public Matrix(double[][] arrays) {
        ...
        if (numRows == 0) {
            this.delegate = new SparseMatrix(new DMatrixSparseCSC(0, 0, 0));
            return;
        }
        ...
        this.delegate = new SparseMatrix(csc);
    }
    ...
    public Matrix(int[] array) {
        this.delegate = new SparseMatrix(array.length, 1, array.length);
    ...
    public Matrix(List<Double> array) {
        this.delegate = new SparseMatrix(array.size(), 1, array.size());
    ...
    public Matrix(ArrayList<List<Double>> arrays) {
        ...
        this.delegate = new SparseMatrix(maxSize, arrays.size(), maxSize * arrays.size());
     ...
     * Static factory because constructor would clash with Matrix(List&lt;Double&gt;) under erasure.
     */
    public static Matrix fromRows(List<List<Double>> rows) {
        ...
        Matrix out = new Matrix(numRows, numCols);
        for (int i = 0; i < numRows; i++) {
    ...
    public Matrix(SimpleMatrix matrix) {
        this.delegate = new SparseMatrix(matrix.numRows(), matrix.numCols());
    ...
    public Matrix(String matrixString) {
        if (matrixString.isEmpty()) {
            this.delegate = new SparseMatrix(0, 0, 0);
            return;
        }
    ...
    public static Matrix cell2mat(Map<Integer, Matrix> cellArray) {
        ...
        for (Matrix matrix : cellArray.values()) {
            ...
            Iterator<MatrixEntry> it = matrix.nonZeroIterator();
            while (it.hasNext()) {
                MatrixEntry e = it.next();
                newMatrix.set(0, currentColIndex + e.col * numRows + e.row, e.value);
            }
            currentColIndex += numRows * matrix.getNumCols();
        }
    ...
    public static double[] columnMatrixToDoubleArray(Matrix columnMatrix) {
        ...
        double[] resultArray = new double[columnMatrix.getNumRows()];

        Iterator<MatrixEntry> it = columnMatrix.nonZeroIterator();
        while (it.hasNext()) {
            MatrixEntry e = it.next();
            resultArray[e.row] = e.value;
        }
    ...
    public static Matrix concatColumns(Matrix left, Matrix right, Matrix out) {
        ...
        if (left.isDense() || right.isDense() || (out != null && out.isDense())) {
            DMatrixRMaj res = CommonOps_DDRM.concatColumns(left.asRMaj(), right.asRMaj(), null);
            if (out != null) {
                out.setTo(new Matrix(res));
                return out;
            }
            return new Matrix(res);
        }
        if (out == null) {
            return new Matrix(SparseMatrix.concatColumnsStatic(left.asCSC(), right.asCSC(), null));
        } else {
            SparseMatrix.concatColumnsInPlaceStatic(left.asCSC(), right.asCSC(), out.asCSC());
            return out;
        }
    }
    ...
    public static Matrix concatRows(Matrix top, Matrix bottom, Matrix out) {
        ...
        if (top.isDense() || bottom.isDense() || (out != null && out.isDense())) {
            DMatrixRMaj res = new DMatrixRMaj(top.getNumRows() + bottom.getNumRows(), top.getNumCols());
            CommonOps_DDRM.concatRows(top.asRMaj(), bottom.asRMaj(), res);
            if (out != null) {
                out.setTo(new Matrix(res));
                return out;
            }
        ...
        if (out == null) {
            return new Matrix(SparseMatrix.concatRowsStatic(top.asCSC(), bottom.asCSC(), null));
        } else {
            SparseMatrix.concatRowsInPlaceStatic(top.asCSC(), bottom.asCSC(), out.asCSC());
            return out;
        }
    }
    ...
    public static Matrix diagMatrix(Matrix A, double[] values, int offset, int length) {
        if (A == null) {
            ...
            return new Matrix(SparseMatrix.diagWithMatrixStatic(A.asCSC(), values, offset, length));
        }
    }
    ...
    public static void extract(
        ...
        // dispatch on the write target's format; src converts read-only
        if (dst.isDense()) {
            DenseMatrix.extractMatrixStatic(src.asRMaj(), srcX0, srcX1, srcY0, srcY1, dst.asRMaj(), dstY0, dstX0);
        } else {
            SparseMatrix.extractMatrixStatic(src.asCSC(), srcX0, srcX1, srcY0, srcY1, dst.asCSC(), dstY0, dstX0);
        }
    }
    ...
    public static Matrix extractColumn(Matrix A, int column, Matrix out) {
        if (A.isDense() || (out != null && out.isDense())) {
        ...
        if (out == null) {
            return new Matrix(SparseMatrix.extractColumnStatic(A.asCSC(), column, null));
        } else {
            SparseMatrix.extractColumnInPlaceStatic(A.asCSC(), column, out.asCSC());
            return out;
        }
    }
    ...
    public static void extractDiag(Matrix A, Matrix outputB) {
        ...
        SparseMatrix.extractDiagStatic(A.asCSC(), outputB.asCSC());
    }
    ...
    public static Matrix extractRows(Matrix A, int row0, int row1, Matrix out) {
        if (A.isDense() || (out != null && out.isDense())) {
            ...
            Matrix res = new Matrix(denseOut);
            if (out != null) {
                out.setTo(res);
                return out;
            }
        ...
        if (out == null) {
            return new Matrix(SparseMatrix.extractRowsStatic(A.asCSC(), row0, row1, null));
        } else {
            SparseMatrix.extractRowsInPlaceStatic(A.asCSC(), row0, row1, out.asCSC());
            return out;
        }
    }
    ...
    public static double logsumexp(Matrix x) {
        ...
        // Exclude only the argmax index from the sum (not every element equal
        // to the maximum), otherwise inputs with duplicated maxima undercount.
    ...
    /**
     * Solves the Sylvester equation A·X + X·B + C = 0, matching the
     * three-argument MATLAB lyap(A,B,C) convention; the Lyapunov equation
     * A·X + X·Aᵀ + Q = 0 corresponds to B = Aᵀ, C = Q. Redirects to
     * {@link #sylv}.
     *
     * @param A Left coefficient matrix
     * @param B Right coefficient matrix
     * @param C Constant matrix
     * @param D Ignored (generalized form not supported)
    ...
    public static boolean solve(Matrix a, Matrix b, Matrix x) {
        ...
        // singularity from the factorization, not det(): see _kb/11-conventions-and-gotchas.md
        boolean ok = solveDispatch(a, b, x);
        if (!ok || (x.hasNaN() || x.hasInfinite()) && a.isFinite() && b.isFinite()) {
            throw new RuntimeException("Matrix is singular and system cannot be solved");
        }
        return true;
    }

    /**
     * Runs the linear solve on the storage-appropriate EJML backend, writing
     * the solution into x while preserving x's storage format.
     */
    private static boolean solveDispatch(Matrix a, Matrix b, Matrix x) {
        if (a.isDense() || b.isDense() || x.isDense()) {
            DMatrixRMaj sol = new DMatrixRMaj(a.getNumCols(), b.getNumCols());
            boolean ok = CommonOps_DDRM.solve(a.asRMaj(), b.asRMaj(), sol);
            if (ok) {
                x.setTo(new Matrix(sol));
            }
            return ok;
        }
        return SparseMatrix.solveStatic(a.asCSC(), b.asCSC(), x.asCSC());
    }
    ...
    public static boolean solveSafe(Matrix a, Matrix b, Matrix x) {
        ...
        // rank via SVD, not a det() threshold: see _kb/11-conventions-and-gotchas.md
        int n = a.getNumRows();
        try {
            if (a.rank() < n) {
        ...
        boolean success = solveDispatch(a, b, x);
    ...
    public static Matrix robustLeftDivide(Matrix A, Matrix B) {
        try {
            ...
            if (e.getMessage() != null && e.getMessage().contains("singular")) {
                ...
                double tol = Math.max(m, n) * Math.ulp(S.get(0, 0));  // MATLAB pinv tolerance
    ...
    /**
     * Solves the Sylvester equation A·X + X·B = -C via Bartels-Stewart on the
     * complex Schur form, mirroring MATLAB lyap(A,B,C). The complex Schur form
     * is required so that the column-wise substitution remains valid when A or
     * B have complex eigenvalue pairs (the real Schur form is only
     * quasi-triangular, and its 2x2 diagonal blocks couple adjacent columns).
     * The case B = A^H is handled with a specialized backward solve.
     *
     * @param A Left coefficient matrix
     * @param B Right coefficient matrix
     * @param C Constant matrix
    ...
    public static Matrix sylv(Matrix A, Matrix B, Matrix C) {
        int m = C.getNumRows();
        int n = C.getNumCols();
        Map<String, ComplexMatrix> schurA = A.schurComplex();
        ComplexMatrix ZA = schurA.get("U");
        ComplexMatrix TA = schurA.get("T");
        ComplexMatrix ZB;
        ComplexMatrix TB;
        boolean backward;
        if (A.transpose().isEqualTo(B)) {
            ZB = ZA;
            TB = TA.conjugateTranspose();
            backward = true;
        } else {
            Map<String, ComplexMatrix> schurB = B.schurComplex();
            ZB = schurB.get("U");
            TB = schurB.get("T");
            backward = false;
        }

        ComplexMatrix F = ZA.conjugateTranspose().mult(new ComplexMatrix(C)).mult(ZB);

        org.apache.commons.math3.complex.Complex[] p =
                new org.apache.commons.math3.complex.Complex[m];
        for (int i = 0; i < m; i++) {
            p[i] = TA.get(i, i);
        }

        // Bartels-Stewart column-wise back substitution: see _kb/03-api-layer.md ("Matrix.sylv")
        ComplexMatrix Y = new ComplexMatrix(m, n);
        org.apache.commons.math3.complex.Complex[] rhs =
                new org.apache.commons.math3.complex.Complex[m];
        org.apache.commons.math3.complex.Complex[] y =
                new org.apache.commons.math3.complex.Complex[m];
        for (int step = 0; step < n; step++) {
            int k = backward ? n - 1 - step : step;
            for (int i = 0; i < m; i++) {
                rhs[i] = F.get(i, k);
            }
            for (int j = 0; j < n; j++) {
                org.apache.commons.math3.complex.Complex t = TB.get(j, k);
                if (t.getReal() != 0.0 || t.getImaginary() != 0.0) {
                    for (int i = 0; i < m; i++) {
                        rhs[i] = rhs[i].add(Y.get(i, j).multiply(t));
                    }
                }
            }
            org.apache.commons.math3.complex.Complex shift = TB.get(k, k);
            for (int i = m - 1; i >= 0; i--) {
                org.apache.commons.math3.complex.Complex s = rhs[i].negate();
                for (int j = i + 1; j < m; j++) {
                    s = s.subtract(TA.get(i, j).multiply(y[j]));
                }
                y[i] = s.divide(p[i].add(shift));
            }
            for (int i = 0; i < m; i++) {
                Y.set(i, k, y[i]);
            }
        ...
        ComplexMatrix X = ZA.mult(Y).mult(ZB.conjugateTranspose());
        return X.real.copy();
    }
    ...
    public Matrix add(double alpha) {
        if (isDense()) {
            DMatrixRMaj out = this.asRMaj().copy();
            CommonOps_DDRM.add(out, alpha);
        ...
        return new Matrix(
                addMatrices(
                        ...
                        this.asCSC(),
                        alpha,
                        Matrix.ones(this.getNumRows(), this.getNumCols()).asCSC(),
                        null));
    }
    ...
    public Matrix add(double alpha, Matrix matrix) {
        if (isDense() || matrix.isDense()) {
            DMatrixRMaj out = new DMatrixRMaj(getNumRows(), getNumCols());
            CommonOps_DDRM.add(1.0, this.asRMaj(), alpha, matrix.asRMaj(), out);
            return new Matrix(out).autoFormat();
        }
        return new Matrix(addMatrices(1, this.asCSC(), alpha, matrix.asCSC(), null)).autoFormat();
    }
    ...
    public void addEq(double alpha) {
        ...
        for (int row = 0; row < numRows; row++) {
            for (int col = 0; col < numCols; col++) {
                this.set(row, col, this.get(row, col) + alpha);
            }
        }
    }
        ...
        switch (op) {
            case "equal":
                if (Math.abs(source - 0) < tol) {
                    ...
                    for (int i = 0; i < numRows; i++) {
                        for (int j = 0; j < numCols; j++) {
                            if (Math.abs(this.get(i, j) - 0) < tol) this.set(i, j, target);
                        }
                    }
            ...
            case "notequal":
                if (Math.abs(source - 0) < tol) {
                    if (Math.abs(target - 0) < tol) {
                        this.zero();
                    } else {
                        // collect first: mutating while iterating non-zeros is unsafe
                        List<int[]> hits = new ArrayList<>();
                        Iterator<MatrixEntry> it = nonZeroIterator();
                        while (it.hasNext()) {
                            MatrixEntry e = it.next();
                            if (Math.abs(e.value - 0) >= tol || Double.isNaN(e.value)) {
                                hits.add(new int[]{e.row, e.col});
                            }
                        }
                        for (int[] rc : hits) {
                            this.set(rc[0], rc[1], target);
                        }
                    }
                } else if (Double.isNaN(source)) {
                    for (int row = 0; row < numRows; row++) {
    ...
    public int count(double val) {
        if (val == 0) {
            return this.getNumCols() * this.getNumRows() - this.getNonZeros();
        }
        int res = 0;
        Iterator<MatrixEntry> it = nonZeroIterator();
        while (it.hasNext()) {
            if (it.next().value == val) res++;
        }
        return res;
    }
    ...
    public Matrix countEachRow(double val) {
        Matrix res = wrapResult(delegate.countEachRow(val));

        // sparse storage does not store zeros: add the structural zeros the
        // delegate cannot see; dense storage already counted them
        if (val == 0 && isSparse()) {
            int numRows = this.getNumRows();
            int numCols = this.getNumCols();
            for (int i = 0; i < numRows; i++) {
    ...
    public void divide(double scalar, Matrix outputB, boolean flag) {
        if (isDense() || outputB.isDense()) {
            DMatrixRMaj out = new DMatrixRMaj(getNumRows(), getNumCols());
            if (flag) {
                CommonOps_DDRM.divide(asRMaj(), scalar, out);
            } else {
                CommonOps_DDRM.divide(scalar, asRMaj(), out);
            }
            outputB.setTo(new Matrix(out));
            return;
        }
        if (flag) divideMatrix(scalar, outputB.asCSC());
        else divideScalarByMatrix(scalar, outputB.asCSC());
    }
    ...
    public Ret.Eigs eigval() {
        ...
        // decompose() may modify its input: never hand it the live dense storage
        DMatrixRMaj matrix = isDense() ? asRMaj().copy() : asRMaj();

        EigenDecomposition_F64<DMatrixRMaj> eig = DecompositionFactory_DDRM.eig(matrix.getNumCols(), false);
    ...
    public List<org.apache.commons.math3.complex.Complex> eig() {
        ...
        EigenDecomposition_F64<DMatrixRMaj> eigDecomp = DecompositionFactory_DDRM.eig(matrix.getNumCols(), false);
     ...
     */
    public Ret.SVD svd() {
        // Empty matrix: MATLAB svd([]) returns empty factors. Commons-Math cannot
        // build a RealMatrix with a zero dimension, so short-circuit here.
        if (getNumRows() == 0 || getNumCols() == 0) {
            return new Ret.SVD(new Matrix(getNumRows(), 0), new Matrix(0, 1), new Matrix(getNumCols(), 0));
        }
        RealMatrix matrix = MatrixUtils.createRealMatrix(this.toArray2D());
    ...
    public Matrix expm_higham() {
        ...
        // balancing (LAPACK DGEBAL) not implemented, no-op here: see _kb/11-conventions-and-gotchas.md
        Matrix A = this.copy();
    ...
    /**
     * Performs element-wise multiplication with another matrix, as MATLAB's {@code .*}.
     * Neither operand is modified.
     *
     * @param B the matrix to multiply with
     * @return a new matrix holding the element-wise product
     */
    public Matrix elementMult(Matrix B) {
        return this.elementMult(B, Matrix.createLike(B));
    }

    /**
     * Performs in-place element-wise multiplication with another matrix, overwriting this
     * matrix with the product. Use {@link #elementMult(Matrix)} when the operands must
     * survive the call.
     *
     * @param B the matrix to multiply with
     * @return the updated matrix (this)
     */
    public Matrix elementMultEq(Matrix B) {
        this.setTo(this.elementMult(B, Matrix.createLike(B)));
        return this;
    }
    ...
    public Matrix elementMult(Matrix B, Matrix output) {
        if (isDense() || B.isDense() || (output != null && output.isDense())) {
            DMatrixRMaj m = new DMatrixRMaj(getNumRows(), getNumCols());
            CommonOps_DDRM.elementMult(asRMaj(), B.asRMaj(), m);
        ...
        DMatrix m;
        if (output == null) {
            m = elementMult(this.asCSC(), B.asCSC(), null);
        } else {
            m = elementMult(this.asCSC(), B.asCSC(), output.asCSC());
        }
        return new Matrix(m);
    }
    ...
    public Matrix elementPower(double t) {
        ...
        if (t == 0) {
            ...
            // zero entries map to Inf (0^t for t<0), so every element must be visited
            for (int i = 0; i < this.getNumRows(); i++) {
                for (int j = 0; j < this.getNumCols(); j++) {
                    res.set(i, j, FastMath.pow(this.get(i, j), t));
                }
            }
        } else if (t != 1) {
            Iterator<MatrixEntry> it = this.nonZeroIterator();
            while (it.hasNext()) {
                MatrixEntry e = it.next();
                res.set(e.row, e.col, FastMath.pow(e.value, t));
            }
        }
        return res;
    }
    ...
    public void expandMatrix(int rows, int cols, int nz_length) {
        ...
        if (isDense()) {
            DMatrixRMaj bigger = new DMatrixRMaj(rows, cols);
            CommonOps_DDRM.insert(asRMaj(), bigger, 0, 0);
            this.delegate = new DenseMatrix(bigger);
            return;
        }
        ...
        while (it.hasNext()) {
            MatrixEntry e = it.next();
            triplet.addItem(e.row, e.col, e.value);
        }
        asCSC().setTo(DConvertMatrixStruct.convert(triplet, (DMatrixSparseCSC) null));
    }
    ...
    public void expandMatrixToSquare() {
        ...
        int n = FastMath.max(this.getNumCols(), this.getNumRows());
        expandMatrix(n, n, this.getNonZeros());
    }
    ...
    public Matrix find() {
        ...
        while (it.hasNext()) {
            MatrixEntry e = it.next();
            res.set(count++, 0, e.col * this.getNumRows() + e.row);
        }
        return res;
    }
    ...
    public Matrix findNonNegative() {
        return wrapResult(delegate.findNonNegative());
    }
    ...
    public int[] getColIndexes() {
        // transitional: raw CSC access sparsifies dense-backed matrices in place
        toSparse();
    ...
    public ColumnView getColumnView(int j) {
        // for dense-backed matrices the view wraps a CSC snapshot
        return new ColumnView(asCSC(), j);
    }

    /**
     * Returns true if this matrix is backed by dense (row-major) storage.
     *
     * @return true if the delegate is dense
     */
    public boolean isDense() {
        return delegate.isDenseFormat();
    }

    /**
     * Returns true if this matrix is backed by sparse (CSC) storage.
     *
     * @return true if the delegate is sparse
     */
     ...
     * No-op if already dense.
     *
     * @return this matrix, dense-backed
     */
    public Matrix toDense() {
        if (!delegate.isDenseFormat()) {
            this.delegate = new DenseMatrix(delegate.toRMaj());
        }
        return this;
    }

    /**
     * Switches this matrix in place to sparse (CSC) storage, dropping
     * explicit zeros. No-op if already sparse.
     *
     * @return this matrix, sparse-backed
     */
    public Matrix toSparse() {
        if (delegate.isDenseFormat()) {
            this.delegate = new SparseMatrix(delegate.toCSC());
        }
        return this;
    }

    /**
     * Returns a dense (row-major) EJML view of this matrix regardless of the
     * backing storage: the live backing array when dense-backed, a conversion
     * copy when sparse-backed.
     *
     * @return dense EJML representation of this matrix
     */
    DMatrixRMaj asRMaj() {
        return delegate.toRMaj();
    }

    /**
     * Returns a CSC (sparse) EJML view of this matrix regardless of the
     * backing storage: the live backing structure when sparse-backed, a
     * conversion copy when dense-backed.
     *
     * @return CSC representation of this matrix
     */
    DMatrixSparseCSC asCSC() {
        return delegate.toCSC();
    }

    /**
     * Wraps a storage-layer result into a facade Matrix, preserving its format.
     */
    private static Matrix wrapResult(BaseMatrix res) {
        if (res.isDenseFormat()) {
            return new Matrix(res.toRMaj());
        }
        return new Matrix(res.toCSC());
    }

    /**
     * Iterates the non-zero entries of this matrix in column-major order,
     * independently of the backing storage format. The returned
     * {@link MatrixEntry} is reused across calls to {@code next()}.
     *
     * @return iterator over non-zero entries
     */
    public Iterator<MatrixEntry> nonZeroIterator() {
        return delegate.nonZeroIterator();
    }

    /**
     * Returns the live CSC backing structure, switching a dense-backed matrix
     * to sparse storage first so that reads and writes through the returned
     * reference stay coherent with this matrix.
     *
     * @return the live CSC data
     * @deprecated raw storage access; use {@link #nonZeroIterator()} for reads
     *             and the Matrix API for writes
     */
    @Deprecated
    ...
    /**
     * Replaces the backing storage. A CSC argument installs sparse storage, a
     * row-major argument installs dense storage.
     *
     * @param newData the new backing data (adopted, not copied)
     * @deprecated raw storage access; use {@link #setTo(Matrix)} instead
     */
    @Deprecated
    public void setData(DMatrix newData) {
        if (newData instanceof DMatrixRMaj) {
            DenseMatrix dm = new DenseMatrix(0, 0);
            dm.setData((DMatrixRMaj) newData);
            this.delegate = dm;
        } else if (isSparse()) {
            ((SparseMatrix) delegate).setData((DMatrixSparseCSC) newData);
        } else {
            SparseMatrix sm = new SparseMatrix(0, 0);
            sm.setData((DMatrixSparseCSC) newData);
            this.delegate = sm;
        }
    }
    ...
    public Matrix getRowsFrom(int row) {
        ...
        DMatrixSparseCSC src = asCSC();
        DMatrixSparseCSC newData = new DMatrixSparseCSC(newNumRows, src.getNumCols());
        for (int r = row; r < src.getNumRows(); r++) {
            int start = src.col_idx[r];
            int end = src.col_idx[r + 1];
            for (int i = start; i < end; i++) {
                int c = src.nz_rows[i];
                double value = src.nz_values[i];
    ...
    public Matrix getSlice(int r0, int r1, int c0, int c1) {
        if (isDense()) {
            DMatrixRMaj out = new DMatrixRMaj(r1 - r0, c1 - c0);
            CommonOps_DDRM.extract(asRMaj(), r0, r1, c0, c1, out, 0, 0);
            return new Matrix(out);
        }
        ...
        extractMatrix(this.asCSC(), r0, r1, c0, c1, sliceCsc, 0, 0);
    ...
    public Matrix getSlice(boolean[] rowFlags, boolean[] colFlags) {
        ...
        while (it.hasNext()) {
            MatrixEntry e = it.next();
            int dstCol = colMap[e.col];
            if (dstCol < 0) continue;
            int dstRow = rowMap[e.row];
            if (dstRow < 0) continue;
            dst.set(dstRow, dstCol, e.value);
        }
    ...
    public void growMaxColumns(int newmax, boolean preserve) {
        // capacity management is CSC-specific; dense storage needs none
        if (isSparse()) {
            asCSC().growMaxColumns(newmax, preserve);
        }
    }
    ...
    public void growMaxLength(int newmax, boolean preserve) {
        // capacity management is CSC-specific; dense storage needs none
        if (isSparse()) {
            asCSC().growMaxLength(newmax, preserve);
        }
    }
    ...
    public Matrix inv() {
        ...
        // Empty matrix: MATLAB inv([]) returns [].
        if (getNumRows() == 0) {
            return new Matrix(0, 0);
        }

        // singularity from the LU factorization, not det(): see _kb/11-conventions-and-gotchas.md
        DMatrixRMaj inverse = new DMatrixRMaj(this.getNumRows(), this.getNumCols());
        if (isDense()) {
            // invert() may modify its input: pass a working copy
            boolean ok = CommonOps_DDRM.invert(asRMaj().copy(), inverse);
            Matrix result = new Matrix(inverse);
            if (!ok || (result.hasNaN() || result.hasInfinite()) && this.isFinite()) {
        ...
        boolean ok = SparseMatrix.invertStatic(this.asCSC(), inverse);
        DConvertMatrixStruct.convert(inverse, thisinverse.asCSC());
        if (!ok || (thisinverse.hasNaN() || thisinverse.hasInfinite()) && this.isFinite()) {
            throw new RuntimeException("Matrix is singular and cannot be inverted");
        }
        return thisinverse;
    }
     ...
     */
    public Matrix pinv() {
        // Empty matrix: MATLAB pinv of an m-by-n empty matrix is the n-by-m empty matrix.
        if (getNumRows() == 0 || getNumCols() == 0) {
            return new Matrix(getNumCols(), getNumRows());
        }
        ...
        // Rank tolerance as in MATLAB pinv: max(m,n) * eps(largest singular value)
        double tol = Math.max(m, n) * Math.ulp(rank > 0 ? S.get(0, 0) : 1.0);
    ...
    public boolean isAssigned(int row, int col) {
        // dense storage materializes every element
        if (isDense()) {
            return true;
        }
        return asCSC().isAssigned(row, col);
    }
    ...
    public boolean isInteger() {
        Iterator<MatrixEntry> it = nonZeroIterator();
        while (it.hasNext()) {
            double v = it.next().value;
            if (FastMath.abs(v - FastMath.round(v)) > GlobalConstants.Zero) {
                return false;
            }
        }
        return true;
    ...
    public Matrix kron(Matrix b) {
        DMatrixRMaj A = this.asRMaj();
        DMatrixRMaj B = b.asRMaj();
    ...
    public Matrix krons(Matrix other) {
        DMatrixRMaj A = this.asRMaj();
        DMatrixRMaj B = other.asRMaj();
        DMatrixRMaj C = DenseMatrix.kronStatic(A, DenseMatrix.identityStatic(B.getNumRows()), null);
        DMatrixRMaj D = DenseMatrix.kronStatic(DenseMatrix.identityStatic(A.getNumRows()), B, null);
    ...
    public Matrix mult(Matrix B) {
        if (isDense() || B.isDense()) {
            DMatrixRMaj out = new DMatrixRMaj(getNumRows(), B.getNumCols());
            CommonOps_DDRM.mult(this.asRMaj(), B.asRMaj(), out);
            return new Matrix(out).autoFormat();
        }
        return new Matrix(multMatrix(B.asCSC(), null)).autoFormat();
    }
    ...
    public Matrix mult(Matrix B, Matrix out) {
        if (isDense() || B.isDense() || (out != null && out.isDense())) {
            Matrix res = this.mult(B);
            if (out != null) {
                out.setTo(res);
                return out;
            }
        ...
        if (out == null) {
            return new Matrix(multMatrix(B.asCSC(), null));
        } else {
            return new Matrix(multMatrix(B.asCSC(), out.asCSC()));
        }
    }
     ...
     */
    public int rank() {
        return MatrixFeatures_DDRM.rank(asRMaj());
    }
    ...
    public void remove(int row, int col) {
        if (isDense()) {
            asRMaj().set(row, col, 0.0);
        } else {
            asCSC().remove(row, col);
        }
    }
    ...
    public Matrix repmat(int rows, int cols) {
        ...
        if (isDense()) {
            ...
            DMatrixRMaj tile = asRMaj();
            DMatrixRMaj out = new DMatrixRMaj(n * rows, m * cols);
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
                    CommonOps_DDRM.insert(tile, out, i * n, j * m);
                }
            }
            return new Matrix(out);
        }
    ...
    public void reshape(int numRows, int numCols) {
        ...
        if (isDense()) {
            delegate.reshape(numRows, numCols);
        } else {
            asCSC().reshape(numRows, numCols);
        }
    }
    ...
    public void reshape(int numRows, int numCols, int arrayLength) {
        if (isDense()) {
            ...
            asCSC().reshape(numRows, numCols, arrayLength);
        }
    }
    ...
    public void scaleEq(double scalar, Matrix output) {
        if (isDense() || output.isDense()) {
            output.setTo(this.scale(scalar));
        } else {
            scaleMatrix(scalar, output.asCSC());
        }
    }
    ...
    public Map<String, Matrix> schur(String method, Integer iter) {
        ...
        try {
            ...
            // SchurTransformer is package-private: allow invoking its public methods
     ...
     * {@code [V,H] = hess(A)}: V is orthogonal, H is upper Hessenberg, and
     * {@code A = V*H*V'} (equivalently {@code V'*A*V = H}).
     *
     * @return a map containing the keys "V" (orthogonal matrix) and "H" (upper Hessenberg matrix)
     */
    public Map<String, Matrix> hess() {
        if (getNumRows() != getNumCols()) {
            throw new RuntimeException("Only square matrix can be decomposed");
        }
        int n = getNumRows();
        Map<String, Matrix> result = new HashMap<>();
        if (n <= 2 || this.getNonZeroLength() == 0) {
            // Any matrix of order <= 2 is already upper Hessenberg.
            result.put("V", Matrix.eye(n));
            result.put("H", this.copy());
            return result;
        }

        // HessenbergTransformer is package-private, accessed via reflection (mirrors schur()): see _kb/11-conventions-and-gotchas.md
        RealMatrix rm = MatrixUtils.createRealMatrix(this.toArray2D());
        RealMatrix vMat;
        RealMatrix hMat;
        try {
            Class<?> htClass = Class.forName("org.apache.commons.math3.linear.HessenbergTransformer");
            java.lang.reflect.Constructor<?> ctor = htClass.getDeclaredConstructor(RealMatrix.class);
            ctor.setAccessible(true);
            Object ht = ctor.newInstance(rm);

            java.lang.reflect.Method getP = htClass.getMethod("getP");
            java.lang.reflect.Method getH = htClass.getMethod("getH");
            getP.setAccessible(true);
            getH.setAccessible(true);

            vMat = (RealMatrix) getP.invoke(ht);
            hMat = (RealMatrix) getH.invoke(ht);
        } catch (Exception e) {
            throw new RuntimeException("Hessenberg decomposition failed", e);
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                double vv = vMat.getEntry(i, j);
                if (vv != 0.0) V.set(i, j, vv);
                double hv = hMat.getEntry(i, j);
                if (hv != 0.0) H.set(i, j, hv);
            }
        }
        result.put("V", V);
        result.put("H", H);
    ...
    /**
     * Computes the generalized Hessenberg-triangular decomposition of the pencil
     * (this, B), equivalent to MATLAB's {@code [LBAR,NBAR,W,V] = hess(A,B)}:
     * W and V are orthogonal, {@code W*A*V = LBAR} is upper Hessenberg and
     * {@code W*B*V = NBAR} is upper triangular.
     *
     * <p>Implements the standard Hessenberg-triangular reduction (Golub and Van
     * Loan, Alg. 7.7.1): B is first triangularized by a QR factorization whose
     * factor is applied to A, then Givens rotations annihilate A below the
     * subdiagonal, each paired with a column rotation that restores the
     * triangularity of B. Both transforms are accumulated, so the identities
     * above hold exactly.</p>
     *
     * @param B the second matrix of the pencil, square and of the same order
     * @return a map with keys "LBAR" (upper Hessenberg), "NBAR" (upper
     *         triangular), "W" and "V" (orthogonal)
     */
    public Map<String, Matrix> hess(Matrix B) {
        int n = getNumRows();
        if (getNumCols() != n || B.getNumRows() != n || B.getNumCols() != n) {
            throw new RuntimeException("Both matrices must be square and of the same order");
        }

        Matrix A = this.copy();
        Matrix C = B.copy();

        // Triangularize C via QR: C = Q*R, so Q'*C = R. Apply Q' to A as well.
        Map<String, Matrix> qr = C.qr();
        Matrix Q = qr.get("Q");
        Matrix W = Q.transpose();
        ...
        for (int j = 0; j < n - 2; j++) {
            for (int i = n - 1; i >= j + 2; i--) {
                // Row rotation on (i-1, i) annihilating A(i,j).
                double a = A.get(i - 1, j);
                double b = A.get(i, j);
                if (Math.abs(b) > 1e-300) {
                    double r = Math.hypot(a, b);
                    double c = a / r;
                    double s = b / r;
                    applyRowRotation(A, i - 1, i, c, s);
                    applyRowRotation(C, i - 1, i, c, s);
                    applyRowRotation(W, i - 1, i, c, s);
                }
                // The row rotation fills C(i,i-1); a column rotation on
                // (i-1, i) annihilates it and preserves the zeros in A.
                double ca = C.get(i, i);
                double cb = C.get(i, i - 1);
                if (Math.abs(cb) > 1e-300) {
                    double r = Math.hypot(ca, cb);
                    double c = ca / r;
                    double s = cb / r;
                    applyColRotation(A, i - 1, i, c, s);
                    applyColRotation(C, i - 1, i, c, s);
                    applyColRotation(V, i - 1, i, c, s);
                }
            }
        }

        // Clean the numerical dust below the Hessenberg/triangular profiles.
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i > j + 1) A.set(i, j, 0.0);
                if (i > j) C.set(i, j, 0.0);
            }
        }
    ...
    /** Applies the Givens rotation [c s; -s c] to rows (p,q) of M, in place. */
    private static void applyRowRotation(Matrix M, int p, int q, double c, double s) {
    ...
    /** Applies the Givens rotation [c s; -s c] to columns (p,q) of M, in place. */
    private static void applyColRotation(Matrix M, int p, int q, double c, double s) {
    ...
    public Map<String, ComplexMatrix> schurComplex() {
        ...
        while (k < n - 1) {
            ...
            if (Math.abs(subdiag) > 1e-14) {
                ...
                // unitary G diagonalizing the complex-eigenvalue 2x2 block: see _kb/03-api-layer.md ("Matrix.schurComplex")
                double vRe0 = b;
                double vIm0 = 0.0;
                ...
                // embed G as an n x n Givens rotation and apply T <- G^H*T*G, U <- U*G: see _kb/03-api-layer.md ("Matrix.schurComplex")
    ...
    public void set(int row, int col, double val) {
        if (isDense()) {
            ...
            DMatrixSparseCSC csc = asCSC();
            if (val == 0) {
                // avoid insert-then-remove of an explicit zero: see _kb/11-conventions-and-gotchas.md
                if (csc.nz_index(row, col) >= 0) {
                    csc.remove(row, col);
                }
            } else {
                csc.set(row, col, val);
    ...
    public void set(int row, int col, int val) {
        if (val == Integer.MAX_VALUE) {
            this.set(row, col, Inf);
        } else if (val == Integer.MIN_VALUE) {
            this.set(row, col, NegInf);
        } else {
    ...
    //    public Ret.Eigs eigvec() {
    //        if (this.data.getNumCols() != this.data.getNumRows()) {
    //            throw new RuntimeException("Only square matrix can be eigen decomposited");
    //        }
    //        DMatrixRMaj matrix = new DMatrixRMaj(this.data.getNumRows(), this.data.getNumCols());
    //        DConvertMatrixStruct.convert(this.data, matrix);
    //
    //
    //        EigenDecomposition_F64<DMatrixRMaj> eig = DecompositionFactory_DDRM.eig(matrix.getNumCols(),
    // true);
    //        eig.decompose(matrix);
    //
    //        Matrix D = new Matrix(1, data.getNumCols(), data.getNumCols());
    //        Matrix V = new Matrix(data.getNumRows(), data.getNumCols(), data.getNumRows());
    ...
    public void setTo(Matrix m) {
        // preserves the storage format of the destination
        if (isDense()) {
            asRMaj().setTo(m.asRMaj());
        } else {
            asCSC().setTo(m.asCSC());
        }
    }
    ...
    /**
     * Sorts the matrix values in place following MATLAB semantics: vectors are
     * sorted as a whole (zeros included), matrices are sorted column by column.
     *
    ...
    public DMatrixSparseCSC toDMatrixSparseCSC() {
        return isSparse() ? asCSC().copy() : asCSC();
    }
    ...
    public DMatrixSparseCSC toDMatrixSparseCSC(Matrix matrix) {
        return matrix.toDMatrixSparseCSC();
    }
    ...
    public Matrix transpose() {
        if (isDense()) {
            DMatrixRMaj out = new DMatrixRMaj(getNumCols(), getNumRows());
            CommonOps_DDRM.transpose(asRMaj(), out);
            return new Matrix(out);
        }
        Matrix res = new Matrix(0, 0);
        transposeMatrix(res.asCSC());
        return res;
    }
    ...
    public void unsafeSet(int row, int col, double val) {
        if (isDense()) {
            asRMaj().unsafe_set(row, col, val);
        } else {
            asCSC().unsafe_set(row, col, val);
        }
    }
