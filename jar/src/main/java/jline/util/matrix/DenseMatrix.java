...
public class DenseMatrix extends BaseMatrix {
    ...
    /**
     * Constructs a dense matrix with specified dimensions. The nzLength
     * parameter is ignored for dense storage and exists for signature
     * compatibility with the sparse constructor.
     *
     * @param numRows  the number of rows in the matrix
     * @param numCols  the number of columns in the matrix
     * @param nzLength ignored for dense matrices
     */
    public DenseMatrix(int numRows, int numCols, int nzLength) {
        setData(new DMatrixRMaj(numRows, numCols));
    }
     ...
     */
    @Override
    public DMatrixSparseCSC toCSC() {
        DMatrixSparseCSC csc = new DMatrixSparseCSC(getNumRows(), getNumCols());
        org.ejml.ops.DConvertMatrixStruct.convert(getData(), csc, 0.0);
        return csc;
    }
     ...
     */
    @Override
    public DMatrixRMaj toRMaj() {
        return getData();
    }

    /**
     * Iterates the non-zero values of this matrix in column-major order,
     * matching the ordering of the sparse iterator.
     */
    @Override
    public Iterator<MatrixEntry> nonZeroIterator() {
        final DMatrixRMaj data = getData();
        return new Iterator<MatrixEntry>() {
            ...
            private void findNext() {
                ...
                } while (j < data.numCols && data.unsafe_get(i, j) == 0.0);
                positioned = true;
            }
            ...
            public MatrixEntry next() {
                ...
                entry.row = i;
                entry.col = j;
                entry.value = data.unsafe_get(i, j);
                return entry;
            }
        };
    }
    ...
    protected BaseMatrix createNewInstance(int rows, int cols, int nzLength) {
        return new DenseMatrix(rows, cols, nzLength);
    }
     ...
     * Two dense matrices are equal if they have the same dimensions and all
     * corresponding elements agree within a 1e-10 tolerance (historical
     * behavior, to be unified with the sparse semantics in Phase 4 of the
     * storage refactoring).
     *
    ...
    public BaseMatrix findNonNegative() {
        // matches the sparse semantics: only non-zero entries qualify,
        // scanned in column-major order with row-major linear indices
    ...
    public String toString() {
        // header plus truncated grid, format relied upon by coverage tests
