...
public class SparseMatrix extends BaseMatrix {
    ...
    protected static boolean invertStatic(DMatrix input, DMatrixRMaj output) {
        return CommonOps_DSCC.invert((DMatrixSparseCSC) input, output);
    }
    ...
    protected void applyConditionalTransform(double source, double target, double tol, String operation) {
        ...
        for (int colIdx = 0; colIdx < data.getNumCols(); colIdx++) {
            switch (operation) {
                case "NaN":
    ...
    public DMatrixRMaj toRMaj() {
        DMatrixRMaj rmaj = new DMatrixRMaj(getNumRows(), getNumCols());
        org.ejml.ops.DConvertMatrixStruct.convert(getData(), rmaj);
        return rmaj;
    }
    ...
    public Iterator<MatrixEntry> nonZeroIterator() {
        final DMatrixSparseCSC data = getData();
        return new Iterator<MatrixEntry>() {
            ...
            public boolean hasNext() {
                return idx < data.nz_length;
            }

            @Override
            public MatrixEntry next() {
                while (idx >= data.col_idx[col + 1]) {
                    col++;
                }
                entry.row = data.nz_rows[idx];
                entry.col = col;
                entry.value = data.nz_values[idx];
                idx++;
                return entry;
            }
        };
    ...
    public BaseMatrix countEachRow(double val) {
        BaseMatrix res = createNewInstance(data.getNumRows(), 1, data.getNumRows());
        int[] colIndices = data.col_idx;
        int[] rowIndices = data.nz_rows;
        double[] values = data.nz_values;
    ...
    protected BaseMatrix createNewInstance(int rows, int cols, int nzLength) {
        return new SparseMatrix(rows, cols, nzLength);
    }
     ...
     * Two sparse matrices are equal if they have the same dimensions and all
    ...
    public BaseMatrix findNonNegative() {
        ...
        for (int colIdx = 0; colIdx < data.getNumCols(); colIdx++) {
            ...
            for (int i = col1; i < col2; i++) {
                if (values[i] >= 0) {
                    resultIndices.add(rowIndices[i] * data.getNumCols() + colIdx);
                }
            }
        }
    ...
    public double get(int row, int col) {
        // out-of-bounds access must throw (EJML IllegalArgumentException);
        // in-bounds unset positions return 0 from the CSC lookup itself
