/*
 * Vendored from imperial-qore/lsoda-java (BSD 3-Clause, Copyright (c) 2023,
 * imperial-qore), which LINE also consumes as the Maven artifact
 * lsoda-java:lsoda. This copy is in the SAME package, so the compiled class in
 * jar/target/classes SHADOWS the one in the dependency jar; the dependency is
 * still needed for odesolver.SupportingFunctions, odesolver.Utility (whose
 * ReturningValues is package-private, which is why this file cannot move to
 * another package), odesolver.exception.* and tools.Data.
 *
 * It exists for the changes marked "LINE fork" below. Everything else is
 * upstream, unmodified. Re-derive the forks when the artifact is upgraded rather
 * than dropping this file (see _kb/06-solver-catalog.md):
 *
 *   1. calJacobian: a floor on the finite-difference increment; without it the
 *      fluid solver stalls.
 *   2. {@link #setForceStiff}: start and stay on the BDF half instead of
 *      auto-switching from Adams. LSODA's Adams half bounds its step by the
 *      stability region only through pdlast, which is estimated from the
 *      CORRECTOR's convergence rate; AT A FIXED POINT the corrector converges
 *      before that estimate is taken, pdlast stays 0, the bound goes away and h
 *      grows to hmax. An order-12 Adams-Moulton at h*|lambda| = O(1) is outside
 *      its stability region, so the solution then wanders around the fixed point
 *      at the amplitude the error test tolerates rather than sitting on it: on a
 *      closed 2-station fluid model at tol=1e-4 that is 2.5e-5, and it GROWS with
 *      the horizon. The reference integrators for this path (MATLAB ode15s,
 *      scipy BDF) are A-stable and have no such mode.
 */
package odesolver;
...
public class LSODA implements FirstOrderIntegrator{
    ...
    // atol_j/rtol_j, the scale below which component j is negligible; NUMJAC's
    // "threshold", PER COMPONENT. Set from the tolerances of the current lsoda()
    // call; 1-based like every other work array here, null before the first one.
    private double[] jacThreshold = null;

    // for output
    private ArrayList<Double> tvec;
    private ArrayList<Double[]> yvec;

    // Instants the CALLER wants the trajectory at, increasing, or null. They are
    // added to the recorded grid from this integrator's own interpolant (intdy,
    // ODEPACK's dense output) as each step is accepted, which is what MATLAB's
    // ode solvers do when handed an output vector. It costs no extra steps and
    // no accuracy, and it is what lets SolverENV sum its exit average on the
    // sojourn quadrature grid instead of LINEARLY interpolating a step grid
    // that has no resolution there. Null leaves every other caller unchanged.
    private double[] outputTimes = null;
    private int outputIdx = 0;
    /** Per-accepted-step stopping test, or null. See setStopCondition. */
    private StepStopCondition stopCondition = null;

    // LINE fork: pin the integrator to the BDF (stiff) half, see the file header.
    private boolean forceStiff = false;

    /**
     * Whether to start on the BDF half and never switch back to Adams.
     *
     * @param value true to pin the stiff method
     */
    public void setForceStiff(boolean value) {
        this.forceStiff = value;
    }

    /** Whether this integrator is pinned to the BDF half. */
    public boolean isForceStiff() {
        return this.forceStiff;
    }
    ...
    private void calJacobian(double[] y) {
        ...
        if (r0 == 0.0)
            r0 = 1.0;
        // LINE fork, see the increment floor below: the RHS measured in units of
        // the tolerated error, at or below roundoff, is the fixed-point start
        // that makes ODEPACK's own increment meaningless.
        final boolean degenerateRhs = !(fac > sqrteta);
        double yj, r;
        for (int j = 1; j <= n; j++) {
            yj = y[j];
            ...
            // to ~1e-27 instead of tripping the guard, and any coordinate with
            // y_j = 0 (or |y_j| ~ 1e-9) is differenced with an increment of
            // 1e-31..1e-17. The column is then rounding noise divided by that
            // increment, i.e. entries of order 1e15, the Newton matrix is
            // meaningless, the corrector diverges (crate ~ 1e3), h collapses by
            // 0.25 per failure and the step count explodes. The floor below is
            // MATLAB NUMJAC's increment, sqrt(eps)*max(|y_j|, atol_j/rtol_j),
            // which is what keeps ode15s off this failure mode.
            //
            // IT APPLIES ONLY WHERE ODEPACK'S INCREMENT HAS DEGENERATED, which
            // `fac` reports: vmnorm(f, ewt) measures the RHS in units of the
            // error the integrator tolerates, so `fac` at roundoff IS the
            // fixed-point start this fork is about, and r0 underflows with it.
            // Applied unconditionally it also replaced perfectly well scaled
            // increments -- HIRES has atol/rtol = 1e-2 while y8 ~ 1.3e-4, and
            // differencing it at 1.5e-10 rather than its own 1.9e-12 moved
            // y8(200) by 6e-4 relative. See _kb/06-solver-catalog.md.
            if (degenerateRhs && jacThreshold != null && j < jacThreshold.length) {
                r = Math.max(r, sqrteta * Math.max(Math.abs(yj), jacThreshold[j]));
            }
            y[j] += r;
...
*/
        if (istate == 1 || istate == 3) {
            ...
            // LINE fork: NUMJAC's threshold, atol/rtol, for the Jacobian
            // increment floor in calJacobian. Taken from the tolerances of THIS
            // call rather than from the constructor, which some callers load in
            // the opposite order.
            //
            // PER COMPONENT, as NUMJAC's is. Reducing a vector atol to its max
            // over the smallest rtol is not conservative but destructive: it
            // lets the LARGEST component's scale set the increment of the
            // SMALLEST, which on Robertson (atol {1e-6,1e-10,1e-6}, rtol 1e-4)
            // differenced a species of size 5.7e-14 at 1.5e-10 and diverged the
            // integration by seven orders of magnitude.
            jacThreshold = new double[neq + 1];
            for (int i = 1; i <= neq; i++) {
                double atI = (itol == 2 || itol == 4) ? atol[i] : atol[1];
                double rtI = (itol > 2) ? rtol[i] : rtol[1];
                double th = (rtI > 0.0) ? atI / rtI : atI;
                if (!(th > 0.0) || Double.isInfinite(th)) {
                    th = 0.0; // no usable scale: keep ODEPACK's increment
                }
                jacThreshold[i] = th;
            }
...
//                    mxords = maxords;
        ...
        if (istate == 1) {
            ...
            meth = forceStiff ? 2 : 1; // LINE fork: pin the BDF half
            int nyh = n;
...
*/
        if (istate == 1) {
            tn = t;
            maxord = forceStiff ? mxords : mxordn; // LINE fork
            if (itask == 4 || itask == 5) {
                tcrit = tmax;
            ...
            nqu = 0;
            mused = 0;
            miter = forceStiff ? 2 : 0; // LINE fork: BDF corrects with a Jacobian
            ccmax = 0.3;
            maxcor = 3;
            msbp = 20;
            mxncf = 10;
...
*/
                ...
                if (itask == 1) {
                    if ((tn - tout) * h < 0.0) {
                        emitOutputTimes(tn, nq, tn, hu, h, yh, n);
                        recording(tn, y);
                        // THE WINDOW MAY BE OVER BEFORE ITS END TIME. A drift
                        // that has reached a fixed point cannot move again, so
                        // the rest of the span is known and stepping it is the
                        // stall this hook exists to end -- see
                        // StepStopCondition. The state is held out to `tout` so
                        // a stopped window is indistinguishable from one that
                        // ran, then the normal successful return is taken.
                        if (stopCondition != null && stopCondition.stop(tn, y)) {
                            if ((tout - tn) * h > 0.0) {
                                recording(tout, y);
                            }
                            istate = 2;
                    ...
                    emitOutputTimes(tout, nq, tn, hu, h, yh, n);
                    y = intdy(tout, 0, y, nq, tn, hu, ETA, h, yh, n);
                    iflag = (int) y[0];
                    if (iflag != 0) {
                        terminate();
...
*/
    ...
    private void stoda() {
        ...
        if (jstart == 0) {
            ...
            // LINE fork: leave elco holding the coefficients of the method actually
            // in use; upstream ends on cfode(1) because meth is always 1 here.
            if (meth == 2)
                cfode(2);
            resetCoeff();
        } // end if (jstart == 0)
 ...
 */
    ...
    private void methodSwitch(double dsm, double pnorm) {
        ...
        if (forceStiff) {
            return; // LINE fork: the BDF half is the whole method here
        }
     ...
     *
     * <p>Resets the emission pointer, so it must be called before each
     * {@code integrate}/{@code lsoda} call whose window it applies to. Pass
     * null to go back to recording steps only.</p>
     *
     * @param ts increasing output instants, or null
     */
    ...
    /**
     * A test consulted after every accepted step, ending the integration when
     * it returns true.
     *
     * <p>WHY NOT addStepHandler. That method is an empty stub here -- it
     * satisfies {@code FirstOrderIntegrator} and discards what it is given --
     * so a handler registered through it is silently never called. This is the
     * hook that works.</p>
     *
     * <p>Like {@link #setOutputTimes(double[])} this must be set before EACH
     * window, and set back to null when the window does not want it:
     * {@code stiffIntegratorFor} may hand back a shared instance, and a
     * condition left over from a previous window would apply to this one.</p>
     *
     * @param c the test, or null to step every window in full
     */
    ...
    /**
     * Record every requested instant up to {@code upTo} from the interpolant.
     *
     * @param upTo last instant the current Nordsieck history can answer for
     * @param nq   current method order
     * @param tn   time the history is centred on
     * @param hu   last successful step size
     * @param h    current step size
     * @param yh   Nordsieck history array
     * @param n    number of equations
     */
    private void emitOutputTimes(double upTo, int nq, double tn, double hu,
                                 double h, double[][] yh, int n) {
        if (outputTimes == null) {
            return;
        }
        // strictly below: an instant equal to the step time is recorded by the
        // step itself, and a duplicate row would show up as a zero-width
        // interval in every quadrature summed over this grid
        while (outputIdx < outputTimes.length && outputTimes[outputIdx] < upTo) {
            double tq = outputTimes[outputIdx];
            outputIdx++;
            double[] yq = intdy(tq, 0, new double[n + 1], nq, tn, hu, ETA, h, yh, n);
            if ((int) yq[0] != 0) {
                continue;   // outside the interpolant's range: skip, do not fail
            }
            yq[0] = 0;
            recording(tq, yq);
        }
