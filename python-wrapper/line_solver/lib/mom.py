...
def lib_mom_setup_linear_system(network_spec, moments_needed):
    ...
    """
    try:
        SetupLS = jpype.JPackage('jline').lib.mom.SetupLinearSystem
...
def lib_mom_solve(A, b):
    ...
    try:
        LinearSolver = jpype.JPackage('jline').lib.mom.LinearSolver
        result = LinearSolver.solve(A_jline, b_jline)
...
def lib_mom_solve_network(network_data):
    ...
    """
    try:
        MomSolver = jpype.JPackage('jline').lib.mom.MomSolver
