...
def _list_to_matrix_cell(mmap_list):
    """Convert a Python list of numpy arrays to a JAR MatrixCell."""
    MatrixCell = jpype.JPackage('jline').util.matrix.MatrixCell
    cell = MatrixCell(len(mmap_list))
    for i, mat in enumerate(mmap_list):
        cell.set(i, jlineMatrixFromArray(np.asarray(mat)))
    return cell


def _matrix_cell_to_list(cell):
    """Convert a JAR MatrixCell to a Python list of numpy arrays."""
    result = []
    for i in range(cell.size()):
        result.append(jlineMatrixToArray(cell.get(i)))
    return result
...
def m3afit_auto(mtrace, num_states=2, method=1, timescale=None, timescale_asy=None):
    ...
    """
    try:
        m3afit_auto_fn = jpype.JPackage('jline').lib.m3a.M3aFitKt.m3afit_auto
        ...
        S_arr = jpype.JArray(jpype.JDouble)(list(S))
        C_arr = jpype.JArray(jpype.JInt)(list(C))
...
def m3afit_compress(mmap, method=0, num_states=2, verbose=False):
    ...
    """
    try:
        m3afit_compress_fn = jpype.JPackage('jline').lib.m3a.M3aCompressKt.m3afit_compress
...
def mamap2m_fit_gamma_fb(M1, M2, M3, GAMMA, P, F, B):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Mamap2m_fit_gamma_fb_mmapKt.mamap2m_fit_gamma_fb

        P_arr = jpype.JArray(jpype.JDouble)(list(np.asarray(P).flatten()))
        F_arr = jpype.JArray(jpype.JDouble)(list(np.asarray(F).flatten()))
        B_arr = jpype.JArray(jpype.JDouble)(list(np.asarray(B).flatten()))
...
def mamap2m_fit_gamma_fb_mmap(mmap):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Mamap2m_fit_gamma_fb_mmapKt.mamap2m_fit_gamma_fb_mmap
...
def mamap22_fit_gamma_bs(M1, M2, M3, GAMMA, P, B, S):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Mamap22_fit_multiclassKt.mamap22_fit_gamma_bs

        P_mat = jlineMatrixFromArray(np.asarray(P).reshape(1, -1))
        B_mat = jlineMatrixFromArray(np.asarray(B).reshape(-1, 1))
        S_mat = jlineMatrixFromArray(np.asarray(S))
...
def mamap22_fit_gamma_fs(M1, M2, M3, GAMMA, P, F, S):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Mamap22_fit_multiclassKt.mamap22_fit_gamma_fs

        P_mat = jlineMatrixFromArray(np.asarray(P).reshape(1, -1))
        F_mat = jlineMatrixFromArray(np.asarray(F).reshape(-1, 1))
        S_mat = jlineMatrixFromArray(np.asarray(S))
...
def mamap22_fit_gamma_bs_mmap(mmap):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Mamap22_fit_multiclassKt.mamap22_fit_gamma_bs_mmap
...
def mamap22_fit_gamma_fs_mmap(mmap):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Mamap22_fit_multiclassKt.mamap22_fit_gamma_fs_mmap
...
def amap2_fit_gamma(M1, M2, M3, GAMMA):
    ...
    """
    try:
        fit_fn = jpype.JPackage('jline').api.mam.Amap2_fit_gammaKt.amap2_fit_gamma
...
def lib_m3a_fit_auto(moments):
    ...
    try:
        M3aFit = jpype.JPackage('jline').lib.m3a.M3aFit
        result = M3aFit.fitAuto(jlineMatrixFromArray(moments))

        return {
            'parameters': jlineMatrixToArray(result),
            'method': 'auto'
        }
    except Exception as e:
        raise RuntimeError(f"M3A auto fitting failed: {str(e)}")
...
def lib_m3a_fit_init(moments):
    ...
    try:
        M3aFit = jpype.JPackage('jline').lib.m3a.M3aFit
        result = M3aFit.fitInit(jlineMatrixFromArray(moments))
...
def lib_m3a_compress(arrival_process):
    ...
    arrival_jline = jlineMatrixFromArray(arrival_process)

    try:
        M3aCompressor = jpype.JPackage('jline').lib.m3a.M3A

        result = M3aCompressor.compressHyperExponential(arrival_jline)

        return {
            'compressed': jlineMatrixToArray(result),
