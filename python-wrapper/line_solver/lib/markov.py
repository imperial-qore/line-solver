...
def lib_markov_check_generator(Q):
    ...
    """
    Q = np.asarray(Q)
    Q_jline = jlineMatrixFromArray(Q)
...
def lib_markov_check_probability_matrix(P):
    ...
    """
    P = np.asarray(P)
    P_jline = jlineMatrixFromArray(P)
...
def lib_markov_check_probability_vector(pi):
    ...
    pi_jline = jlineMatrixFromArray(pi)

    try:
        result = jpype.JPackage('jline').lib.butools.BuToolsMarkov.checkProbVector(
            pi_jline
        )
        return bool(result)
    except Exception as e:
