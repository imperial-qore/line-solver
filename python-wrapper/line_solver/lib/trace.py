...
def lib_trace_acf(trace, max_lag):
    ...
    trace_jline = jlineMatrixFromArray(trace)
...
def lib_trace_bicov(trace1, trace2, max_lag):
    ...
    trace1_jline = jlineMatrixFromArray(trace1)
    trace2_jline = jlineMatrixFromArray(trace2)

    try:
        result = jpype.JPackage('jline').lib.kpctoolbox.Kpctoolbox.trace_bicov(
            trace1_jline, trace2_jline, max_lag
        )
        return jlineMatrixToArray(result)
    except Exception as e:
...
def lib_trace_gamma(trace, max_lag):
    ...
    try:
        result = jpype.JPackage('jline').lib.kpctoolbox.Kpctoolbox.trace_gamma(
            trace_jline, max_lag
        )
        return jlineMatrixToArray(result)
    except Exception as e:
...
def lib_trace_iat2bins(trace, num_bins):
    ...
    try:
        result = jpype.JPackage('jline').lib.kpctoolbox.Kpctoolbox.trace_iat2bins(
            trace_jline, num_bins
        )
        return jlineMatrixToArray(result)
    except Exception as e:
...
def lib_trace_idi(trace, num_intervals):
    ...
    try:
        result = jpype.JPackage('jline').lib.kpctoolbox.Kpctoolbox.trace_idi(
            trace_jline, num_intervals
        )
        return float(result)
    except Exception as e:
...
def lib_trace_skew(trace):
    ...
    try:
        result = jpype.JPackage('jline').lib.kpctoolbox.Kpctoolbox.trace_skew(
            trace_jline
        )
        return float(result)
    except Exception as e:
...
def lib_mtrace_mean(traces):
    ...
    traces_jline = [jlineMatrixFromArray(t) for t in traces]

    try:
        result = jpype.JPackage('jline').lib.kpctoolbox.Kpctoolbox.mtrace_mean(
            traces_jline
        )
        return float(result)
    except Exception as e:
