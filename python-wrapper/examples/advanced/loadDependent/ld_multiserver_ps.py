...
delay.setService(job_class2, Exp.fit_mean(2.0))

queue1.setService(job_class1, Exp.fit_mean(1.5))
queue1.setService(job_class2, Exp.fit_mean(2.5))
queue1.setNumberOfServers(c)

queue2.setService(job_class1, Exp.fit_mean(3.5))
queue2.setService(job_class2, Exp.fit_mean(4.5))
queue2.setNumberOfServers(c)
...
ms_t = MVA(model, 'exact').get_avg_table()
print('MVA (exact):')
print(ms_t)
...
queue1.setLoadDependence([min(i, c) for i in range(1, N + N // 2 + 1)])

queue2.setService(job_class1, Exp.fit_mean(3.5))
queue2.setService(job_class2, Exp.fit_mean(4.5))
# Multi-server with c servers using load dependence
queue2.setLoadDependence([min(i, c) for i in range(1, N + N // 2 + 1)])
...
print('\nNC (exact):')
lld_avg_table_nc = NC(ldmodel, method='exact').get_avg_table()
