...
queue.setService(job_class1, Exp.fit_mean(1.5))
queue.setService(job_class2, Exp.fit_mean(2.5))
...
queue.setLoadDependence([min(i, c) for i in range(1, N + N // 2 + 1)])
...
print('\nJMT Simulation:')
lld_avg_table_jmt = JMT(ldmodel, seed=23000, samples=5000).get_avg_table()
